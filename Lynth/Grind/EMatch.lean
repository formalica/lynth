...
structure Choice where
  ...
  /--
  Helper field for implementing `lynth_grind.ematch.diagnostics`.
...
This is used to implement `lynth_grind.ematch.diagnostics`.
-/
...
protected def _root_.Lean.Lynth.Grind.GenPatternInfo.assign? (genInfo : GenPatternInfo) (c : Choice) (x : Expr) : OptionT GoalM Choice := do
...
Annotate the conditions using `Lean.Grind.MatchCond`. See `MatchCond.lean`.
...
It also introduce `Lean.Grind.MatchCond` at equation conditions. See `MatchCond.lean`.
-/
private def annotateMatchEqnType (prop : Expr) (initApp : Expr) : M Expr := do
  annotateEqnTypeConds prop fun prop => do
    ...
    -- See comment at `Lean.Grind.EqMatch`
    let lhs ← markAsSimpMatchDiscrsOnly lhs
    return mkApp4 (mkConst ``Lean.Grind.EqMatch f.constLevels!) α lhs rhs initApp
...
/-- Similar to `reportIssue!`, but only reports issue if `lynth_grind.debug` is set to `true` -/
...
where
  go (proof prop : Expr) : M Unit := do
    ...
    if (← isMatchEqLikeDeclName thm.origin.key) then
      ...
      -- `Lean.Grind.PreMatchCond` which are not reducible.
      proof := mkExpectedPropHint proof prop
    else if (← isEqnThm thm.origin.key) then
      prop ← annotateEqnTypeConds prop
      -- We must add a hint because `annotateEqnTypeConds` introduces `Lean.Grind.PreMatchCond`
...
private def processUnassigned (mvars : Array Expr) (i : Nat) (v : Expr) (h : i < mvars.size) : OptionT (StateT Choice M) Unit := do
  ...
  if (← isDefEqD mvarIdType vType) then
    ...
    if let some heq ← withoutReportingMVarIssues <| proveEq? vType mvarIdType (abstract := true) then
      /-
      Some of the `cast`s will appear inside the `Lean.Grind.MatchCond` binders, and
      we want their proofs to be properly wrapped.
      -/
      let heq := mkApp2 (mkConst ``Lean.Grind.nestedProof) (← mkEq vType mvarIdType) heq
...
This is technique to ensure the metavariables ids do not depend on operations performed before invoking `lynth_grind`.
...
private def getLHS (args : Array Expr) (lhs : Nat) : MetaM Expr := do
  unless lhs < args.size do
    throwError "`lynth_grind` internal error, invalid variable in `grind_pattern` constraint"
...
private def checkDefEq (expectedResult : Bool) (levelParams : List Name) (us : List Level) (args : Array Expr) (lhs : Nat) (rhs : CnstrRHS) : GoalM Bool := do
  ...
  withNewMCtxDepth do
    ...
    **Note**: Recall that we have abstracted metavariables occurring in `rhs` after we elaborated it.
    ...
    /- **Note**: We used the guarded version to ensure type errors will not interrupt `lynth_grind`. -/
...
private def matchEqBwdPat (p : Expr) : M Unit := do
  let_expr Lean.Grind.eqBwdPattern pα plhs prhs := p | return ()
  let numParams  := (← read).thm.numParams
  let assignment := .replicate numParams unassigned
  let useMT      := (← read).useMT
  let gmt        := (← getThe Goal).ematch.gmt
...
def ematch' (extraThms : Array EMatchTheorem := #[]) : GoalM (Bool × InstanceMap) := do
  ...
  if (← getConfig).markInstances then
    modifyThe Lean.Lynth.Grind.State fun s => { s with instanceMap := s.instanceMap.insertMany map.toArray }
  return (progress, map)
...
def ematchOnly (thms : Array EMatchTheorem) : GoalM Bool := do
  ...
  let (_, s) ← go |>.run
  if (← getConfig).markInstances then
    modifyThe Lean.Lynth.Grind.State fun st => { st with instanceMap := st.instanceMap.insertMany s.instanceMap.toArray }
