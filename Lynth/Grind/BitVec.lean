...
congruence theorems `Lean.Lynth.Grind.eval_congr₁`/`Lean.Lynth.Grind.eval_congr₂` with an `Eq.refl` hypothesis
...
private def mkBVLit (n : Nat) (v : BitVec n) : GoalM Expr := do
  -- **Note**: `lynth_grind` uses `OfNat.ofNat` instead of `BitVec.ofNat` representation.
...
private def unaryOp (e : Expr) (eval : Expr → GoalM (Option Expr)) : GoalM Unit := do
  ...
  let proof := mkApp6 (mkConst ``Lean.Lynth.Grind.eval_congr₁ [1, 1]) (← inferType a) eType e.appFn! a aRoot b
...
private def binOp (e : Expr) (eval : Expr → Expr → GoalM (Option Expr)) : GoalM Unit := do
  ...
  internalize b 0
  let eType ← inferType e
  let proof := mkApp9 (mkConst ``Lean.Lynth.Grind.eval_congr₂ [1, 1, 1]) (← inferType a₁) (← inferType a₂) eType e.appFn!.appFn! a₁ r₁ a₂ r₂ b
...
lynth_builtin_grind_propagator propagateBVNot ↑Complement.complement :=
  unaryBV ``Complement.complement 3 (~~~ ·)
lynth_builtin_grind_propagator propagateBVClz ↑BitVec.clz := unaryBV ``BitVec.clz 2 BitVec.clz
lynth_builtin_grind_propagator propagateBVCpop ↑BitVec.cpop := unaryBV ``BitVec.cpop 2 BitVec.cpop

lynth_builtin_grind_propagator propagateBVMsb ↑BitVec.msb := fun e => do
  unless e.isAppOfArity ``BitVec.msb 2 do return ()
  unaryOp e fun r => do
    let some ⟨_, v⟩ ← getBitVecValue? r | return none
    some <$> mkBoolLit v.msb

lynth_builtin_grind_propagator propagateBVToNat ↑BitVec.toNat := fun e => do
  unless e.isAppOfArity ``BitVec.toNat 2 do return ()
  unaryOp e fun r => do
    let some ⟨_, v⟩ ← getBitVecValue? r | return none
    some <$> shareCommon (mkNatLit v.toNat)

lynth_builtin_grind_propagator propagateBVToInt ↑BitVec.toInt := fun e => do
  unless e.isAppOfArity ``BitVec.toInt 2 do return ()
  unaryOp e fun r => do
    let some ⟨_, v⟩ ← getBitVecValue? r | return none
    some <$> shareCommon (mkIntLit v.toInt)

lynth_builtin_grind_propagator propagateBVOfNat ↑BitVec.ofNat := fun e => do
...
lynth_builtin_grind_propagator propagateBVOfInt ↑BitVec.ofInt := fun e => do
...
lynth_builtin_grind_propagator propagateBVSetWidth ↑BitVec.setWidth := extendBV ``BitVec.setWidth BitVec.setWidth
lynth_builtin_grind_propagator propagateBVSignExtend ↑BitVec.signExtend := extendBV ``BitVec.signExtend BitVec.signExtend

lynth_builtin_grind_propagator propagateBVExtractLsb' ↑BitVec.extractLsb' :=
...
lynth_builtin_grind_propagator propagateBVReplicate ↑BitVec.replicate := fun e => do
...
lynth_builtin_grind_propagator propagateBVAnd ↑HAnd.hAnd := binBV ``HAnd.hAnd 6 (· &&& ·)
lynth_builtin_grind_propagator propagateBVOr ↑HOr.hOr := binBV ``HOr.hOr 6 (· ||| ·)
lynth_builtin_grind_propagator propagateBVXor ↑HXor.hXor := binBV ``HXor.hXor 6 (· ^^^ ·)

lynth_builtin_grind_propagator propagateBVAppend ↑HAppend.hAppend := fun e => do
...
lynth_builtin_grind_propagator propagateBVShiftLeft ↑BitVec.shiftLeft :=
  shiftBV ``BitVec.shiftLeft 3 BitVec.shiftLeft
lynth_builtin_grind_propagator propagateBVUShiftRight ↑BitVec.ushiftRight :=
  shiftBV ``BitVec.ushiftRight 3 BitVec.ushiftRight
lynth_builtin_grind_propagator propagateBVSShiftRight ↑BitVec.sshiftRight :=
  shiftBV ``BitVec.sshiftRight 3 BitVec.sshiftRight
lynth_builtin_grind_propagator propagateBVRotateLeft ↑BitVec.rotateLeft :=
  shiftBV ``BitVec.rotateLeft 3 BitVec.rotateLeft
lynth_builtin_grind_propagator propagateBVRotateRight ↑BitVec.rotateRight :=
...
lynth_builtin_grind_propagator propagateBVHShiftLeft ↑HShiftLeft.hShiftLeft :=
  hShiftBV ``HShiftLeft.hShiftLeft (· <<< ·)
lynth_builtin_grind_propagator propagateBVHShiftRight ↑HShiftRight.hShiftRight :=
  hShiftBV ``HShiftRight.hShiftRight (· >>> ·)

lynth_builtin_grind_propagator propagateBVGetLsbD ↑BitVec.getLsbD := getBitBV ``BitVec.getLsbD BitVec.getLsbD
lynth_builtin_grind_propagator propagateBVGetMsbD ↑BitVec.getMsbD := getBitBV ``BitVec.getMsbD BitVec.getMsbD

lynth_builtin_grind_propagator propagateBVGetElem ↑GetElem.getElem := fun e => do
  ...
  let proof := mkApp6 (mkConst ``Lean.Grind.getElem_congr f.constLevels!) coll idx elem valid inst x
