...
public def getLocalTheoremAnchors (goal : Goal) : GrindM (Array ExprWithAnchor) := do
  ...
  return entries
where
  go (found : Std.HashSet Lean.Lynth.Grind.Origin) (result : Array ExprWithAnchor) (thms : PArray EMatchTheorem)
      : GrindM (Std.HashSet Lean.Lynth.Grind.Origin × Array ExprWithAnchor) := do
    let mut found := found
