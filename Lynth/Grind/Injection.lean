...
/--
The `lynth_grind` tactic includes an auxiliary `injection?` tactic that is not intended for direct use by users.
...
- It does not introduce new propositions. Instead, the `lynth_grind` tactic preprocessor is responsible for introducing them.
