...
import Lynth.Grind.CastLike
import Lynth.Grind.Arith.Cutsat.Model
open Lean.Meta
public section
...
def Goal.ppENodeRef (goal : Goal) (e : Expr) : MetaM MessageData := do
  ...
  let u ← getLevel type
  let d := mkApp3 (mkConst ``Lean.Grind.node_def [u]) (toExpr n.idx) type e
  return m!"{d}"
...
private def isGadget (declName : Name) : Bool :=
  declName == ``Lean.Grind.nestedDecidable || declName == ``Lean.Grind.leftInv
...
`lynth_grind` cast-like operations, and arithmetic terms. Moreover, the
expression contains at least one cast-like application.
This kind of term is constructed by `lynth_grind` satellite solvers.
-/
...
def Arith.Cutsat.pp? (goal : Goal) : MetaM (Option MessageData) := do
  ...
  -- was interrupted because the `lynth_liaSteps` threshold was reached.
...
private def ppThresholds (c : Lean.Grind.Config) : M Unit := do
  let goal ← read
  ...
  if cutsat.steps ≥ c.liaSteps then
    msgs := msgs.push <| .trace { cls := `limit } m!"maximum number of steps performed by the `lynth_lia` solver has been reached, threshold: `(liaSteps := {c.liaSteps})`" #[]
...
def goalDiagToMessageData (goal : Goal) (config : Lean.Grind.Config) (header := "Goal diagnostics") (collapsedMain := true)
     : MetaM MessageData := do
  let (_, m) ← go goal |>.run #[]
  let gm := MessageData.trace { cls := `lynth_grind, collapsed := false } header m
  return gm
where
  go : M Unit := do
...
def goalToMessageData (goal : Goal) (config : Lean.Grind.Config) : MetaM MessageData := goal.withContext do
