...
public import Lynth.Grind.Util
public import Lean.Meta.Closure
import Lean.PrettyPrinter
import Lean.Meta.Tactic.ExposeNames
import Lean.Meta.Tactic.Simp.Diagnostics
import Lean.Meta.Tactic.Simp.Rewrite
import Lynth.Grind.MarkAccessible
import Lynth.Grind.Proj
import Lynth.Grind.ForallProp
import Lynth.Grind.CtorIdx
import Lynth.Grind.Intro
import Lynth.Grind.Solve
import Lynth.Grind.EMatch
import Lynth.Grind.MarkNestedSubsingletons
import Lynth.Grind.Internalize
import Lynth.Grind.SimpUtil
import Lynth.Grind.LawfulEqCmp
import Lynth.Grind.ReflCmp
import Lynth.Grind.PP
import Lynth.Grind.Core
...
/--
Returns the `ExtensionState` for the default `lynth_grind` attribute.
-/
def getDefaultExtensionState : MetaM ExtensionState := do
  let s := lynthGrindExt.getState (← getEnv)
  return s
...
/--
Returns the extensions used by the `lynth_lia` tactic.

`lynth_lia` keeps the structural part of the default `lynth_grind` attribute (cases types, `funCC`,
and extensionality theorems), but instead of the full `@[lynth_grind]` E-matching lemma set it
only uses the dedicated `@[lia]` lemma set. This lets `cutsat` benefit from a small amount
of instantiation (e.g. `Nat.max_def`) without pulling in everything tagged `@[lynth_grind]`.
-/
def getLiaExtensions : MetaM ExtensionStateArray := do
  return #[← getOnlyExtensionState, lynthLiaExt.getState (← getEnv)]
...
def mkParams (config : Lean.Grind.Config) (extensions : ExtensionStateArray) : MetaM Params := do
  let norm ← Lean.Lynth.Grind.getSimpContext config
  let normProcs ← Lean.Lynth.Grind.getSimprocs
  let symPrios ← getGlobalSymbolPriorities
  return { config, norm, normProcs, symPrios, extensions }

def mkDefaultParams (config : Lean.Grind.Config) : MetaM Params := do
  let ext ← getDefaultExtensionState
  mkParams config #[ext]

def mkOnlyParams (config : Lean.Grind.Config) : MetaM Params := do
...
-- We use it to make sure we don't have to reset the `simp` cache used in `lynth_grind`.
...
/--
Asserts extra facts provided as `lynth_grind` parameters.
-/
...
private def mkGlobalDiag (cs : Counters) (simp : Simp.Stats) (ss : PArray SplitDiagInfo) : MetaM (Option MessageData) := do
  ...
  unless simpMsgs.isEmpty do
    msgs := msgs.push <| .trace { cls := `lynth_grind} "Simplifier" simpMsgs
  if msgs.isEmpty then
    return none
  else
    return some <| .trace { cls := `lynth_grind } "Diagnostics" msgs
...
def Result.toMessageData (result : Result) : MetaM MessageData := do
  ...
  if result.config.verbose then
    ...
    unless issues.isEmpty do
      msgs := msgs ++ [.trace { cls := `lynth_grind } "Issues" issues.reverse.toArray]
...
def checkUnusedActivations (mvarId : MVarId) (counters : Counters) : GrindM Unit := do
  let threshold := Lean.Lynth.Grind.lynth_grind.unusedLemmaThreshold.get (← getOptions)
  ...
  unless unused.isEmpty do
    ...
    logWarning <| .trace { cls := `lynth_grind } "grind: activated but unused E-matching lemmas" data

/--
When `Config.revert := false`, we preprocess the hypotheses, and add them to the `lynth_grind` state.
...
private def initCore (mvarId : MVarId) : GrindM Goal := do
  ...
  appendTagSuffix mvarId `lynth_grind
...
partial def traceEMatchDiagsCompact (diag : PArray EMatchDiagInfo) : GrindM Unit := do
  if (← isTracingEnabledFor `lynth_grind.ematch.diagnostics.compact) then
  ...
  withTraceNode `lynth_grind.ematch.diagnostics.compact (fun _ => return m!"instances") do
...
def main (mvarId : MVarId) (params : Params) : MetaM Result := do profileitM Exception "grind" (← getOptions) do
  let params := if ((← getOptions).get? "lynth_grind.unusedLemmaThreshold".toName |>.getD 0) > 0 then
...
free variables created by `lynth_grind`, so the conversion is safe. Only metavariables at the current
  ...
  for mvarId in (e.collectMVars {}).result do
    if let some decl := mctx.findDecl? mvarId then
      if decl.depth == mctx.depth then
        throwError "`lynth_grind` failed, proof contains unresolved internal metavariable {Expr.mvar mvarId}"
  return e
where
...
/--
A helper combinator for executing a `lynth_grind`-based terminal tactic.
...
`lynth_grind`'s `isDefEq` calls. After `lynth_grind` finishes, delayed metavariable assignments are
...
def withProtectedMCtx [Monad m] [MonadControlT MetaM m] [MonadLiftT MetaM m]
    ...
    (config : Lean.Grind.Config) (mvarId : MVarId) (k : MVarId → m α) : m α := do
  /-
  **Note**: `instantiateGoalMVars` here also instantiates mvars occurring in hypotheses.
  This is particularly important when using `lynth_grind -revert`.
  -/
...
where
  ...
  finalize (val : Expr) : MetaM Expr := do
    ...
    -- `lynth_grind` proofs are often big, if `abstractProof` is true, we create an auxiliary theorem.
    let val ← if !config.abstractProof then
      pure val
    else if (← isProp type) then
      mkAuxTheorem type val (zetaDelta := true)
    else
      let auxName ← mkAuxDeclName `lynth_grind
