...
module
prelude
public import Lynth.Grind.Theorems
open Lean.Meta
public section
namespace Lean.Lynth.Grind

/-- Types that `lynth_grind` will case-split on. -/
...
inductive EMatchTheoremConstraint where
  ...
  | /--
    ...
    maxInsts (n : Nat)
  | /--
    It instructs `lynth_grind` to postpone the instantiation of the theorem until `e` is known to be `true`.
    -/
    guard (e : Expr)
  | /--
...
structure EMatchTheorem where
  ...
  origin       : Origin
  /-- The `kind` is used for generating the `patterns`. We save it here to implement `lynth_grind?`. -/
...
/-- A theorem marked with `@[lynth_grind inj]` -/
...
/-
**Note**: We currently have a single normalization and symbol priority sets for all `lynth_grind` attributes.
Reason: the E-match patterns must be normalized with respect to them. If we are using multiple
`lynth_grind` attributes, they patterns would have to be re-normalized using the union of all normalizers.

Alternative design: allow a single `lynth_grind` attribute per `lynth_grind` call. Cons: when creating a new
`lynth_grind` attribute users would have to carefully setup the normalizer to ensure all `lynth_grind` assumptions
are met. Cons: users would not be able to write `lynth_grind only [attr_1, attr_2]`.
-/
...
/--
`lynth_grind` is parametrized by a collection of `ExtensionState`. The motivation is to allow
