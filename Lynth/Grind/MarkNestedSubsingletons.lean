...
def isMarkedSubsingletonConst (e : Expr) : Bool := Id.run do
  ...
  return declName == ``Lean.Grind.nestedProof || declName == ``Lean.Grind.nestedDecidable

def isMarkedSubsingletonApp (e : Expr) : Bool :=
  /-
  Remark: we must check `e`s arity because we may have over-applied `Lean.Grind.nestedProof` applications.
...
where
  visit (e : Expr) : M Expr := do
    ...
    if (← isProp type) then
      let e' := mkApp2 (mkConst ``Lean.Grind.nestedProof) (← preprocess type) e
      modify fun s => s.insert { expr := e } e'
      return e'
    if let some p ← isDecidable type then
      let e' := mkApp2 (mkConst ``Lean.Grind.nestedDecidable) (← preprocess p) e
      modify fun s => s.insert { expr := e } e'
      return e'
  ...
  preprocess (e : Expr) : M Expr := do
    ...
    **Note**: We must use `instantiateMVars` here because this function is called using the result of `inferType`.
    ...
    module in `lynth_grind` assumes they have been expanded.
