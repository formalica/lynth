...
def checkMatchCondParent (e : Expr) (parent : Expr) : GoalM Bool := do
  let_expr Lean.Grind.MatchCond parent ← parent | return false
  let mut curr := parent
  repeat
...
/-- Checks invariants if `lynth_grind.debug` is enabled. -/
...
@[inherit_doc Lean.Lynth.Grind.checkInvariants]
public def Goal.checkInvariants (goal : Goal) (expensive := false) : GrindM Unit :=
  discard <| GoalM.run' goal <| Lean.Lynth.Grind.checkInvariants expensive
