...
/-- Returns `true` if `declName` is the name of a cast-like function used to implement `lynth_grind` solvers -/
public def isCastLikeDeclName (declName : Name) : Bool :=
  declName == ``Lean.Grind.Ring.OfSemiring.toQ ||
  declName == ``Lean.Grind.IntModule.OfNatModule.toQ ||
  declName == ``Lean.Grind.ToInt.toInt ||
