...
/--
Similar to `Lean.Lynth.Grind.preprocess`, but does not simplify `e` if
...
(e.g., manually applying an function induction principle before invoking `lynth_grind`).
-/
...
private def intro1 : GoalM FVarId := do
  ...
  let (name, type) ← match target with
    ...
    | .letE n d .. => pure (n, d)
    | _ => throwError "`lynth_grind` internal error, binder expected"
