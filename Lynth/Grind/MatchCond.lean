...
```
When instantiating the equations (and `match`-splitter), we wrap the conditions with the gadget `Lean.Grind.PreMatchCond`.
`Lean.Grind.PreMatchCond` uses the default reducibility setting and cannot be accidentally reduced by `simp`.
After `simp` is applied, it is replaced with `Lean.Grind.MatchCond` which is reducible.
This `Lean.Grind.MatchCond` is used for implementing truth-value propagation.
See the propagator `propagateMatchCond` below.
For example, given a condition `C` of the form `Lean.Grind.MatchCond (∀ (a : Nat),  t = S.mk1 a → False)`,
...
overlapping patterns is far from ideal for the `lynth_grind` module which operates with equivalence classes
...
A different representation could simplify `lynth_grind`, but it could add extra complexity to other modules.
-/
...
```
Lean.Grind.MatchCond (∀ y₁ y₂ y₃, t = .mk₁ y₁ → s = .mk₂ y₂ y₃ → False)
```
...
```
Lean.Grind.MatchCond (∀ y₁ y₂ y₃, a = .mk₁ y₁ → b = .mk₂ y₂ y₃ → False)
```
...
Then, if `lynth_grind` infers that `t = a` and `s = b`, it will detect that `l t s` and `l a b` are
...
def collectMatchCondLhssAndAbstract (matchCond : Expr) : GoalM (Array Expr × Expr) := do
  let_expr Lean.Grind.MatchCond e := matchCond | return (#[], matchCond)
...
/--
Returns `true` if `e` is a `Lean.Grind.MatchCond`, and it has been satisfied.
Recall that we use `Lean.Grind.MatchCond` to annotate conditional `match`-equations.
...
consequently produces conditional `match` equations. Thus, `lynth_grind` generates
the following auxiliary `Lean.Grind.MatchCond` terms for an application `f a b`:
- `Lean.Grind.MatchCond (∀ (n : Nat), a = S.mk1 n → False)`
- `Lean.Grind.MatchCond (∀ (s1 s2 : S), b = S.mk2 1 (s1.mk4 s2) → False)`
- `Lean.Grind.MatchCond (∀ (n : Bool), a = S.mk3 n → False)`
...
private partial def isSatisfied (e : Expr) : GoalM Bool := do
  let_expr Lean.Grind.MatchCond e ← e | return false
  let mut e := e
  repeat
...
private partial def mkMatchCondProof? (e : Expr) : GoalM (Option Expr) := do
  let_expr Lean.Grind.MatchCond f ← e | return none
...
example : z = a :: as → y = z → f x y > 0 := by
  grind [f.eq_def]
```
After `lynth_grind` unfolds `f`, it case splits on the `match`-expression producing
...
partial def tryToProveFalse (e : Expr) : GoalM Unit := do
  ...
  let_expr Lean.Grind.MatchCond body ← e | return ()
...
lynth_builtin_grind_propagator propagateMatchCondUp ↑Lean.Grind.MatchCond := fun e => do
...
lynth_builtin_grind_propagator propagateMatchCondDown ↓Lean.Grind.MatchCond := fun e => do
  if (← isEqTrue e) then
    unless (← isSatisfied e) do
      tryToProveFalse e
