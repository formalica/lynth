...
public import Lynth.Grind.Anchor
...
where
  go (cs : List SplitInfo) (c? : SplitCandidate) (cs' : List SplitInfo) : GoalM SplitCandidate := do
    ...
    if !(← checkAnchorRefs c) then
      /-
      **Note**: `lynth_grind`s context contains anchor references restricting the
...
/-- Constructs a major premise for the `cases` tactic used by `lynth_grind`. -/
private def mkCasesMajor (c : Expr) : GoalM Expr := do
  match_expr c with
  | And a b => return mkApp3 (mkConst ``Lean.Grind.or_of_and_eq_false) a b (← mkEqFalseProof c)
  | Eq _ a b =>
    if isMorallyIff c then
      if (← isEqTrue c) then
        return mkApp3 (mkConst ``Lean.Grind.of_eq_eq_true) a b (← mkEqTrueProof c)
      else
        return mkApp3 (mkConst ``Lean.Grind.of_eq_eq_false) a b (← mkEqFalseProof c)
    else
  ...
  for subgoal in subgoals do
    ...
    | .closed seq =>
      if let some falseProof ← getFalseProof? subgoal.mvarId then
        Lean.Lynth.Grind.assignFalseProof goal.mvarId falseProof
        return .closed seq
