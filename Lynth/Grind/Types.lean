...
public import Lynth.Grind.Attr
public import Lynth.Grind.CheckResult
...
public import Lynth.Grind.AttrSyntax
open Lean.Meta
public section
namespace Lean.Lynth.Grind
export Sym (isSameExpr hashPtrExpr ExprPtr shareCommon shareCommonInc)

-- Option readers mirroring core's `grind.*` / `backward.grind.*` options.
-- Core registers these via `register_builtin_option` in its own engine
-- modules; we read them by name with identical defaults instead, so this
-- port never needs to import (or re-register) core's engine state.
-- `set_option` from user code keeps working: it addresses the same names.
def grind.debug.get (o : Options) : Bool := o.get `grind.debug false
def grind.debug.proofs.get (o : Options) : Bool := o.get `grind.debug.proofs false
def grind.ematch.diagnostics.get (o : Options) : Bool := o.get `grind.ematch.diagnostics false
...
-- NOTE: plain `initialize` (not `builtin_initialize`): builtin initializers
-- never run under `lake env lean` (native objects don't load), leaving
-- `unreachable` values that crash the interpreter when forced.
initialize lynth_grind.warning : Lean.Option Bool ←
  Lean.Option.register "lynth_grind.warning".toName {
    defValue := false
    descr    := "generate a warning whenever `lynth_grind` is used"
  }

initialize lynth_grind.unusedLemmaThreshold : Lean.Option Nat ←
  Lean.Option.register "lynth_grind.unusedLemmaThreshold".toName {
    defValue := 0
...
/--
Anchors are used to reference terms, local theorems, and case-splits in the `lynth_grind` state.
We also use anchors to prune the search space when they are provided as `lynth_grind` parameters
...
We store the current source in the `lynth_grind` context.
-/
...
/--
Auxiliary type used to implement `lynth_grind.ematch.instance`.
-/
...
structure Context where
  ...
  /--
  If `cheapCases` is `true`, `lynth_grind` only applies `cases` to types that contain
  at most one minor premise.
  Recall that `lynth_grind` applies `cases` when introducing types tagged with `[grind cases eager]`,
  and at `Split.lean`
  ...
  /-
  If `reportMVarIssue` is `true`, `lynth_grind` reports an issue when internalizing metavariables.
  ...
  /-- Current source of tracking E-matching theorem instantiation (see `lynth_grind.ematch.instances`). -/
  ...
  debug        : Bool -- Cached `lynth_grind.debug (← getOptions)`
  ematchDiag   : Bool -- Cached `lynth_grind.ematch.diagnostics (← getOptions)`
...
structure State where
  ...
  /--
  Cached anchors (aka stable hash codes) for terms in the `lynth_grind` state.
  -/
...
/-- Returns `true` if `lynth_grind.debug` is set -/
@[inline] def isDebugEnabled : GrindM Bool :=
  return (← readThe Context).debug

/-- Returns `true` if `lynth_grind.ematch.diagnostics` is set -/
...
instance : MonadBacktrack SavedState GrindM where
  saveState      := Lean.Lynth.Grind.saveState
  restoreState s := s.restore

/--
`withoutReportingMVarIssues x` executes `x` without reporting metavariables found during internalization.
See comment at `Lean.Lynth.Grind.Context.reportMVarIssue` for additional details.
-/
def withoutReportingMVarIssues [MonadControlT GrindM m] [Monad m] : m α → m α :=
  mapGrindM <| withTheReader Lean.Lynth.Grind.Context fun ctx => { ctx with reportMVarIssue := false }
...
abbrev withSplitSource [MonadControlT GrindM m] [Monad m] (splitSource : SplitSource) : m α → m α :=
  mapGrindM <| withTheReader Lean.Lynth.Grind.Context fun ctx => { ctx with splitSource }
...
abbrev withEmatchDiagSource [MonadControlT GrindM m] [Monad m] (ematchDiagSource : EMatchDiagSource) : m α → m α :=
  mapGrindM <| withTheReader Lean.Lynth.Grind.Context fun ctx => { ctx with ematchDiagSource }

/-- Returns the user-defined configuration options -/
def getConfig : GrindM Lean.Grind.Config :=
  return (← readThe Context).config

/-- Returns extension states associate with `lynth_grind` attributes in use -/
def getExtensions : GrindM Lean.Lynth.Grind.ExtensionStateArray :=
...
def withCheapCasesOnly (k : GrindM α) : GrindM α :=
  withTheReader Lean.Lynth.Grind.Context (fun ctx => { ctx with cheapCases := true }) k
...
The `lynth_grind` core propagates equalities and disequalities to the theory solvers
...
structure ENode where
  ...
  See `Lean.Grind.Config.funCC` for additional details.
  -/
  funCC : Bool := true
  /-- Auxiliary field used to implement `lynth_grind.ematch.diagnostics` -/
...
private def congrHash (enodes : ENodeMap) (e : Expr) : UInt64 :=
  ...
  | Lean.Grind.nestedDecidable p _ => mixHash 13 (hashRoot enodes p)
...
private partial def isCongruent (enodes : ENodeMap) (e₁ e₂ : Expr) : Bool :=
  ...
  | Lean.Grind.nestedProof p₁ _ =>
    let_expr Lean.Grind.nestedProof p₂ _ := e₂ | false
    hasSameRoot enodes p₁ p₂
  | Lean.Grind.nestedDecidable p₁ _ =>
    let_expr Lean.Grind.nestedDecidable p₂ _ := e₂ | false
    hasSameRoot enodes p₁ p₂
  | Eq _ lhs₁ rhs₁ =>
...
Thus, minor changes may affect the proof found by `lynth_grind`. We found examples
...
One might think we can just reset them at the beginning of the `lynth_grind` search, but
...
If `check` is `true`, then `lynth_grind` attempts to prove `e` by asserting its negation and
...
structure DelayedTheoremInstance where
  ...
  /-- Sources for `lynth_grind.ematch.diagnostics` that generated this instance. -/
  sources    : List EMatchDiagNode
  deriving Inhabited

/-- E-matching related fields for the `lynth_grind` goal. -/
...
/-- Case splitting related fields for the `lynth_grind` goal. -/
...
/--
The `lynth_grind` state for a goal, excluding the metavariable.
...
/--
A `lynth_grind` goal, combining shared state with a specific metavariable.
...
def Goal.hasSameRoot (g : Goal) (a b : Expr) : Bool :=
  Lean.Lynth.Grind.hasSameRoot g.enodeMap a b

def Goal.isCongruent (g : Goal) (a b : Expr) : Bool :=
  Lean.Lynth.Grind.isCongruent g.enodeMap a b
...
This marks all existing local declarations as already processed by `lynth_grind`. Use this when
...
/--
Returns `true` if the goal has local declarations that have not yet been processed by `lynth_grind`.

A local declaration is "pending" if its index is greater than or equal to `nextDeclIdx`.
This is used to determine whether `lynth_grind` needs to internalize new hypotheses.
-/
...
def updateLastTag : GoalM Unit := do
  if (← isTracingEnabledFor `lynth_grind) then
    let currTag ← (← get).mvarId.getTag
    if currTag != (← getThe Lean.Lynth.Grind.State).lastTag then
      trace[grind] "working on goal `{currTag}`"
      modifyThe Lean.Lynth.Grind.State fun s => { s with lastTag := currTag }
...
def addNewRawFact (proof : Expr) (prop : Expr) (generation : Nat) (splitSource : SplitSource) (ematchDiagSource : EMatchDiagSource) : GoalM Unit := do
  if (← isDebugEnabled) then
    unless (← withGTransparency <| isDefEq (← inferType proof) prop) do
      throwError "`lynth_grind` internal error, trying to assert{indentExpr prop}\n\
...
def throwNonInternalizedExpr (e : Expr) : MetaM α :=
  throwError "internal `lynth_grind` error, term has not been internalized{indentExpr e}"
...
congruence closure. See `Lean.Grind.Config.funCC` for additional details.
-/
...
def pushEqCore (lhs rhs proof : Expr) (isHEq : Bool) : GoalM Unit := do
  if (← isDebugEnabled) then
    unless (← alreadyInternalized lhs) do
      throwError "`lynth_grind` internal error, lhs of new equality has not been internalized{indentExpr lhs}"
    unless (← alreadyInternalized rhs) do
      throwError "`lynth_grind` internal error, rhs of new equality has not been internalized{indentExpr rhs}"
    if proof != congrPlaceholderProof && proof != eqCongrSymmPlaceholderProof then
      ...
      unless (← withGTransparency <| isDefEq (← inferType proof) expectedType) do
        throwError "`lynth_grind` internal error, trying to assert equality{indentExpr expectedType}\n\
            with proof{indentExpr proof}\nwhich has type{indentExpr (← inferType proof)}\n\
            which is not definitionally equal with `reducible` transparency setting"
...
(Same body as core `Lean.MVarId.assignFalseProof`; defined here under our
own name so this module never depends on core's engine modules.)
-/
def assignFalseProof (mvarId : MVarId) (falseProof : Expr) : MetaM Unit := mvarId.withContext do
...
def closeGoal (falseProof : Expr) : GoalM Unit := do
  ...
  unless (← mvarId.isAssigned) do
    Lean.Lynth.Grind.assignFalseProof mvarId falseProof
...
/-!
`Action` is the *control interface* for `lynth_grind`’s search steps. It is defined in
...
/-- Result type for a `lynth_grind` Action -/
...
private initialize solverExtensionsRef : IO.Ref (Array (SolverExtension SolverExtensionState)) ← IO.mkRef #[]

/--
Registers a new solver extension for `lynth_grind`.
...
def registerSolverExtension {σ : Type} (mkInitial : IO σ) : IO (SolverExtension σ) := do
  unless (← initializing) do
    throw (IO.userError "failed to register `lynth_grind` solver, extensions can only be registered during initialization")
...
def SolverExtension.setMethods (ext : SolverExtension σ)
  ...
  unless ext.id < (← solverExtensionsRef.get).size do
    throw (IO.userError "failed to register `lynth_grind` solver methods, invalid solver id")
