...
module
prelude
public import Lynth.Grind.Injective
public import Lynth.Grind.Cases
public import Lynth.Grind.ExtAttr
public import Lean.Meta.Tactic.Simp.Attr
public import Lynth.Grind.Homo
...
initialize normExt : SimpExtension ← mkSimpExt
...
def getAttrKindCore (stx : Syntax) : CoreM AttrKind := do
  ...
  | `(Parser.Attr.grindMod|symbol $prio:prio) =>
    ...
    return .symbol prio
  | _ => throwError "unexpected `lynth_grind` theorem kind: `{stx}`"
...
def throwInvalidUsrModifier : CoreM α :=
  throwError "the modifier `usr` is only relevant in parameters for `lynth_grind only`"
...
/--
Auxiliary function for registering `lynth_grind`, `lynth_grind!`, `lynth_grind?`, and `lynth_grind!?` attributes.
`lynth_grind!` is like `lynth_grind` but selects minimal indexable subterms.
The `lynth_grind?` and `lynth_grind!?` are aliases for `lynth_grind` and `lynth_grind!` which displays patterns using `logInfo`.
...
private def mkGrindAttr (attrName : Name) (minIndexable : Bool) (showInfo : Bool) (ext : Extension) (ref : Name := by exact decl_name%) : IO Unit :=
  registerBuiltinAttribute {
    ...
    descr :=
      ...
      \
      The `lynth_grind` tactic utilizes annotated theorems to add instances of matching patterns into the local context during proof search.\
      For example, if a theorem `@[{attrName} =] theorem foo_idempotent : foo (foo x) = foo x` is annotated,\
      `lynth_grind` will add an instance of this theorem to the local context whenever it encounters the pattern `foo (foo x)`."
    ...
    add := fun declName stx attrKind => MetaM.run' do
      ...
      | .symbol prio =>
        unless attrName == `lynth_grind do
      ...
      | .unfold =>
        ...
        unless (← addDeclToUnfold normExt declName (post := false) (inv := false) (prio := eval_prio default) (attrKind := attrKind)) do
          throwError "cannot mark declaration to be unfolded by `lynth_grind`"
      | .homo =>
        unless attrName == `lynth_grind do
...
private def registerDefaultGrindAttr (minIndexable : Bool) (showInfo : Bool) : IO Unit :=
  mkGrindAttr `lynth_grind minIndexable showInfo
...
initialize extensionMapRef : IO.Ref ExtensionMap ← IO.mkRef {}
...
initialize lynthGrindExt : Extension ← registerAttr `lynth_grind

/--
Extension backing the `@[lia]` attribute. It uses the same recording mechanism as `@[lynth_grind]`
(see `registerAttr`), but provides a separate, smaller E-matching lemma set that the `lynth_lia`
tactic instantiates (e.g. `Nat.max_def`), without enabling the full `@[lynth_grind]` set.
-/
initialize lynthLiaExt : Extension ← registerAttr `lynth_lia
