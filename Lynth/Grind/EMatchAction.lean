...
The order in which things are asserted affects the proof found by `lynth_grind`.
  ...
  go (e : Expr) : StateM CollectState Unit := do
    if isMarkedSubsingletonApp e then
      ...
      **Note**: We can ignore nested proofs and decidable instances.
      They are not part of current `lynth_grind` proof.
      -/
      return ()
...
def mkInstantiateTactic (goal : Goal) (usedThms : Array EMatchTheorem) (approx : Bool) : GrindM TGrind := goal.withContext do
  ...
  let mut foundLocals : Std.HashSet Lean.Lynth.Grind.Origin := {}
      ...
      | .decl declName =>
        if let some fnName ← isEqnThm? declName then
          unless foundFns.contains fnName do
            ...
            let param ← Lean.Lynth.Grind.globalDeclToInstantiateParamSyntax declName thm.kind thm.minIndexable
...
def mkMask (map : EMatchTheoremIds) (thms : Array EMatchTheorem) : CoreM (Array Bool) := do
  ...
  for thm in thms do
    let some i := map.get? { thm } | throwError "`lynth_grind` internal error, theorem index not found"
    ...
    | .closed seq =>
      if (← getConfig).trace then
        ...
        let testMask (mask : Array Bool) : GrindM Bool := do
          checkSystem "`lynth_grind` `instantiate` tactic parameter optimizer"
