...
private def getCommInst : ACM Expr := do
  let some inst := (← getStruct).commInst? | throwError "`lynth_grind` internal error, `{(← getStruct).op}` is not commutative"
  return inst

private def getIdempotentInst : ACM Expr := do
  let some inst := (← getStruct).idempotentInst? | throwError "`lynth_grind` internal error, `{(← getStruct).op}` is not idempotent"
  return inst

private def getNeutralInst : ACM Expr := do
  let some inst := (← getStruct).neutralInst? | throwError "`lynth_grind` internal error, `{(← getStruct).op}` does not have a neutral element"
  return inst
...
private def mkContext (h : Expr) : ProofM Expr := do
  ...
  let h := mkLetOfMap (← get).seqDecls h `s (mkConst ``Lean.Grind.AC.Seq) fun p => toExpr <| p.renameVars varRename
  let h := mkLetOfMap (← get).exprDecls h `e (mkConst ``Lean.Lynth.Grind.AC.Expr) fun e => toExpr <| e.renameVars varRename
