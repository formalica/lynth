...
import Lynth.Grind.AC.DenoteExpr
import Lynth.Grind.AC.Proof
import Lean.Meta.Tactic.Grind.AC.Seq
import Lynth.Grind.AC.Inv
...
/-- Explicit basis-search helpers over `Lean.Grind.AC.Seq`.
...
private def findSimpA? (s : AC.Seq) : ACM (Option (EqCnstr × AC.SubseqResult)) := do
  for c in (← getStruct).basis do
    let r := c.lhs.subseq s
    unless r matches .false do return some (c, r)
  return none

private def findSimpAC? (s : AC.Seq) : ACM (Option (EqCnstr × AC.SubsetResult)) := do
  for c in (← getStruct).basis do
...
local macro "gen_cnstr_fns " cnstr:ident : command =>
  ...
  private def $(mkId `simplifyA) (c : $cnstr) : ACM $cnstr := do
    ...
    repeat
      ...
      if let some (c', r) ← findSimpA? c.lhs then
        c := c.simplifyLhsWithA c' r
        c ← c.cleanup
      else if let some (c', r) ← findSimpA? c.rhs then
  ...
  private def $(mkId `simplifyAC) (c : $cnstr) : ACM $cnstr := do
    ...
    repeat
      ...
      if (← checkMaxSteps) then return c
      if let some (c', r) ← findSimpAC? c.lhs then
        c := c.simplifyLhsWithAC c' r
        c ← c.cleanup
      else if let some (c', r) ← findSimpAC? c.rhs then
  ...
  while progress do
    ...
    if idempotent then
      ...
      if rhs != c.rhs then
        ...
        progress := true
    if comm then
      if let some (c', r) ← findSimpAC? c.rhs then
        c := c.simplifyRhsWithAC c' r
        progress := true
    else
      if let some (c', r) ← findSimpA? c.rhs then
        c := c.simplifyRhsWithA c' r
        progress := true
  return c
