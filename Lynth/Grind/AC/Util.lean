...
public import Lynth.Grind.ProveEq
...
protected def ACM.getStruct : ACM Struct := do
  ...
  if h : opId < s.structs.size then
    ...
    throwError "`lynth_grind` internal error, invalid structure id"
    ...
    let (neutralInst?, neutral?) ← do
      ...
      if let some identityInst ← synthInstance? identityType then
        ...
        **Note**: `Std.LawfulIdentity` instances may spell the identity element in a form
        that is not in `lynth_grind` normal form. Example: the `BitVec` instances use `0#n`
        (i.e., `BitVec.ofNat n 0`), but the `lynth_grind` normalizer represents bit-vector literals
        using `OfNat.ofNat`. Internalizing a non-canonical literal violates the invariant that
        interpreted nodes are canonical: `lynth_grind` assumes distinct interpreted nodes denote
