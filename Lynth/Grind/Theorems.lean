...
/-!
A collection of theorems. We use it to implement E-matching and injectivity theorems used by `lynth_grind`.
-/
...
inductive Origin where
  ...
  | fvar (fvarId : FVarId)
  /--
  A proof term provided directly to a call to `lynth_grind` where `ref`
...
When `lynth_grind` internalizes a term containing symbol `s`, we
...
def getProofForDecl (declName : Name) : MetaM Expr := do
  ...
  unless wasOriginallyTheorem (← getEnv) declName do
    unless (← isProp info.type) do
      throwError "invalid `lynth_grind` theorem `{.ofConstName declName}`, type is not a proposition"
