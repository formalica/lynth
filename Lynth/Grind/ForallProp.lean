...
import Lynth.Grind.Anchor
import Lynth.Grind.EqResolution
...
where
  propagateImpliesUp (a b : Expr) : GoalM Unit := do
    ...
    if (← isEqFalse a <&&> isProp b) then
      -- a = False → (a → b) = True
      pushEqTrue e <| mkApp3 (mkConst ``Lean.Grind.imp_eq_of_eq_false_left) a b (← mkEqFalseProof a)
    else if (← isEqTrue a <&&> isProp b) then
      -- a = True → (a → b) = b
      pushEq e b <| mkApp3 (mkConst ``Lean.Grind.imp_eq_of_eq_true_left) a b (← mkEqTrueProof a)
    else if (← isEqTrue b <&&> isProp a) then
      -- b = True → (a → b) = True
      pushEqTrue e <| mkApp3 (mkConst ``Lean.Grind.imp_eq_of_eq_true_right) a b (← mkEqTrueProof b)
    else if (← isEqFalse b <&&> isEqTrue e <&&> isProp a) then
      -- (a → b) = True → b = False → a = False
      pushEqFalse a <| mkApp4 (mkConst ``Lean.Grind.eq_false_of_imp_eq_true) a b (← mkEqTrueProof e) (← mkEqFalseProof b)
...
def propagateForallPropDown (e : Expr) : GoalM Unit := do
  ...
  if (← isEqFalse e) then
    if b.hasLooseBVars || !(← isProp a) then
      ...
      let proof := mkApp3 (mkConst ``Lean.Grind.of_forall_eq_false [u]) α (mkLambda n bi α p) (← mkEqFalseProof e)
      addNewRawFact proof prop (← getGeneration e) (.forallProp e) .other
    else
      let h ← mkEqFalseProof e
      pushEqTrue a <| mkApp3 (mkConst ``Lean.Grind.eq_true_of_imp_eq_false) a b h
      pushEqFalse b <| mkApp3 (mkConst ``Lean.Grind.eq_false_of_imp_eq_false) a b h
...
lynth_builtin_grind_propagator propagateExistsDown ↓Exists := fun e => do
  if (← isEqFalse e) then
...
- `Lean.Grind.imp_true_eq`
- `Lean.Grind.imp_false_eq`
- `Lean.Grind.forall_imp_eq_or`
- `Lean.Grind.true_imp_eq`
- `Lean.Grind.false_imp_eq`
- `Lean.Grind.imp_self_eq`
- `forall_true`
- `forall_false`
- `Lean.Grind.forall_or_forall`
- `Lean.Grind.forall_forall_or`
- `Lean.Grind.forall_and`
-/
def simpForall : Lean.Meta.Simp.Simproc := fun e => do
  let .forallE varName d b info := e | return .continue
  if !b.hasLooseBVars then
    match_expr d with
    | True => if (← isProp b) then return .done { expr := b, proof? := mkApp (mkConst ``Lean.Grind.true_imp_eq) b }
    | False => if (← isProp b) then return .done { expr := mkConst ``True, proof? := mkApp (mkConst ``Lean.Grind.false_imp_eq) b }
    | _ =>
    if let .forallE aName α pRaw info' := d then
      if (← pure pRaw.hasLooseBVars <&&> isProp d) then
        ...
        return .visit { expr, proof? := mkApp3 (mkConst ``Lean.Grind.forall_imp_eq_or [u]) α p q }
    else match_expr b with
    | True => if (← isProp d) then return .done { expr := mkConst ``True, proof? := mkApp (mkConst ``Lean.Grind.imp_true_eq) d }
    | False => if (← isProp d) then return .visit { expr := mkNot d, proof? := mkApp (mkConst ``Lean.Grind.imp_false_eq) d }
    | _ => if (← isProp d <&&> isDefEq d b) then
      return .done { expr := mkConst ``True, proof? := mkApp (mkConst ``Lean.Grind.imp_self_eq) d }
  else
    ...
    | True =>
      ...
      if (← isProp pTrue) then
        ...
        return .done { expr := pTrue, proof? := mkApp (mkConst ``Lean.Grind.forall_true) p }
    | False =>
      let p := mkLambda varName info d b
  ...
  if b.isApp && b.getAppNumArgs == 2 then
    ...
    if bDeclName == ``Or then
      ...
      if let some (bName, βRaw, qRaw) := isForallOrNot? left then
        ...
        return .visit { expr, proof? := mkApp4 (mkConst ``Lean.Grind.forall_forall_or [u, v]) α β p q }
        ...
        return .visit { expr, proof? := mkApp4 (mkConst ``Lean.Grind.forall_or_forall [u, v]) α β p q }
      ...
      let u ← getLevel d
      return .visit { expr, proof? := mkApp3 (mkConst ``Lean.Grind.forall_and [u]) d p q }
  -- None of the rules were applicable
  return .continue

simproc_pattern% ((a : _) → _) => simpForall
/--

Applies the following rewriting rules:
- `Lean.Grind.exists_or`
- `Lean.Grind.exists_and_left`
- `Lean.Grind.exists_and_right`
- `Lean.Grind.exists_prop`
- `Lean.Grind.exists_const`
-/
def simpExists : Lean.Meta.Simp.Simproc := fun e => do
  ...
  if b.isApp && b.getAppNumArgs == 2 then
    ...
    if bDeclName == ``Or then
      ...
      return .visit { expr, proof? := mkApp3 (mkConst ``Lean.Grind.exists_or u) α p q }
    else if bDeclName == ``And then
      let pRaw := b.appFn!.appArg!
      let qRaw := b.appArg!
      if !pRaw.hasLooseBVars then
        ...
        let u := ex.constLevels!
        return .visit { expr, proof? := mkApp3 (mkConst ``Lean.Grind.exists_and_left u) α p b }
      else if !qRaw.hasLooseBVars then
        let p := mkLambda x .default α pRaw
        let b := qRaw
        let expr := mkAnd (mkApp2 ex α p) b
        let u := ex.constLevels!
        return .visit { expr, proof? := mkApp3 (mkConst ``Lean.Grind.exists_and_right u) α p b }
  if !b.hasLooseBVars then
    if (← isProp α) then
      let expr := mkAnd α b
      return .visit { expr, proof? := mkApp2 (mkConst ``Lean.Grind.exists_prop) α b }
    else
      let u := ex.constLevels!
      let nonempty := mkApp (mkConst ``Nonempty u) α
      if let some nonemptyInst ← synthInstanceMeta? nonempty then
        return .visit { expr := b, proof? := mkApp3 (mkConst ``Lean.Grind.exists_const u) α nonemptyInst b }
  return .continue


simproc_pattern% (Exists _) => simpExists
