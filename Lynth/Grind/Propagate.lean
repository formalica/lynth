...
import Lynth.Grind.Ext
import Lynth.Grind.Diseq
...
-/
lynth_builtin_grind_propagator propagateAndUp ↑And := fun e => do
...
-/
lynth_builtin_grind_propagator propagateAndDown ↓And := fun e => do
  if (← isEqTrue e) then
    let_expr And a b := e | return ()
...
-/
lynth_builtin_grind_propagator propagateOrUp ↑Or := fun e => do
...
-/
lynth_builtin_grind_propagator propagateOrDown ↓Or := fun e => do
  if (← isEqFalse e) then
    let_expr Or a b := e | return ()
...
-/
lynth_builtin_grind_propagator propagateNotUp ↑Not := fun e => do
...
-/
lynth_builtin_grind_propagator propagateNotDown ↓Not := fun e => do
  let_expr Not a := e | return ()
  if (← isEqFalse e) then
...
def propagateBoolDiseq (eq : Expr) (a b : Expr) : GoalM Unit := do
  ...
  if (← isEqv b ff) then
    pushEqBoolTrue a <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_true_of_not_eq_false') a (← mkDiseqProofUsing a ff eq)
  else if (← isEqv b tt) then
    pushEqBoolFalse a <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_false_of_not_eq_true') a (← mkDiseqProofUsing a tt eq)
  else if (← isEqv a ff) then
    pushEqBoolTrue b <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_true_of_not_eq_false') b (← mkDiseqProofUsing b ff eq)
  else if (← isEqv a tt) then
    pushEqBoolFalse b <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_false_of_not_eq_true') b (← mkDiseqProofUsing b tt eq)

/-- Propagates `Eq` upwards -/
lynth_builtin_grind_propagator propagateEqUp ↑Eq := fun e => do
  ...
  if isSameExpr a' trueExpr then
    unless isSameExpr (← getRoot e) b' do
      pushEq e b <| mkApp3 (mkConst ``Lean.Grind.eq_eq_of_eq_true_left) a b (← mkEqProof a trueExpr)
  else if isSameExpr b' trueExpr then
    unless isSameExpr (← getRoot e) a' do
      pushEq e a <| mkApp3 (mkConst ``Lean.Grind.eq_eq_of_eq_true_right) a b (← mkEqProof b trueExpr)
  ...
  if α.isConstOf ``Bool then
    if (← isEqFalse e) then
      ...
      if isSameExpr a' tt && isSameExpr b' ff then
        pushEqFalse e <| mkApp4 (mkConst ``Lean.Grind.Bool.ne_of_eq_true_of_eq_false) a b (← mkEqBoolTrueProof a) (← mkEqBoolFalseProof b)
      else if isSameExpr a' ff && isSameExpr b' tt then
        pushEqFalse e <| mkApp4 (mkConst ``Lean.Grind.Bool.ne_of_eq_false_of_eq_true) a b (← mkEqBoolFalseProof a) (← mkEqBoolTrueProof b)
...
lynth_builtin_grind_propagator propagateEqDown ↓Eq := fun e => do
  if (← isEqTrue e) then
...
The following propagators for `BEq.beq` ensure `lynth_grind` does not assume this normalization
rule has been applied.
-/

lynth_builtin_grind_propagator propagateBEqUp ↑BEq.beq := fun e => do
  /-
  `lynth_grind` uses the normalization rule `Bool.beq_eq_decide_eq`, but it is only applicable if
  ...
  if (← isEqv a b) then
    ...
    pushEqBoolTrue e <| mkApp6 (mkConst ``Lean.Grind.beq_eq_true_of_eq u) α binst linst a b (← mkEqProof a b)
  else if let some h ← mkDiseqProof? a b then
    let some linst ← getLawfulBEqInst? u α binst | return ()
    pushEqBoolFalse e <| mkApp6 (mkConst ``Lean.Grind.beq_eq_false_of_diseq u) α binst linst a b h

lynth_builtin_grind_propagator propagateBEqDown ↓BEq.beq := fun e => do
  /- See comment above -/
  let_expr f@BEq.beq α binst a b := e | return ()
  let u := f.constLevels!
  if (← isEqBoolTrue e) then
    let some linst ← getLawfulBEqInst? u α binst | return ()
    pushEq a b <| mkApp6 (mkConst ``Lean.Grind.eq_of_beq_eq_true u) α binst linst a b (← mkEqProof e (← getBoolTrueExpr))
  else if (← isEqBoolFalse e) then
    let some linst ← getLawfulBEqInst? u α binst | return ()
    let eq ← shareCommon (mkApp3 (mkConst ``Eq [u.head!.succ]) α a b)
    internalize eq (← getGeneration a)
    pushEqFalse eq <| mkApp6 (mkConst ``Lean.Grind.ne_of_beq_eq_false u) α binst linst a b (← mkEqProof e (← getBoolFalseExpr))

/-- Propagates `EqMatch` downwards -/
lynth_builtin_grind_propagator propagateEqMatchDown ↓Lean.Grind.EqMatch := fun e => do
  if (← isEqTrue e) then
    let_expr Lean.Grind.EqMatch _ a b origin := e | return ()
    markCaseSplitAsResolved origin
    pushEq a b <| mkOfEqTrueCore e (← mkEqTrueProof e)

/-- Propagates `HEq` downwards -/
lynth_builtin_grind_propagator propagateHEqDown ↓HEq := fun e => do
  if (← isEqTrue e) then
    let_expr HEq _ a _ b := e | return ()
    pushHEq a b <| mkOfEqTrueCore e (← mkEqTrueProof e)

/-- Propagates `HEq` upwards -/
lynth_builtin_grind_propagator propagateHEqUp ↑HEq := fun e => do
  let_expr HEq _ a _ b := e | return ()
  if (← isEqv a b) then
...
lynth_builtin_grind_propagator propagateIte ↑ite := fun e => do
...
lynth_builtin_grind_propagator propagateDIte ↑dite := fun e => do
  let numArgs := e.getAppNumArgs
  if numArgs < 5 then return ()
  let c := e.getArg! 1 numArgs
  if (← isEqTrue c) then
    ...
    let r := p.expr
    let h₂ ← p.getProof
    let h := mkApp3 (mkAppRange (mkConst ``Lean.Lynth.Grind.dite_cond_eq_true' f.constLevels!) 0 5 args) r h₁ h₂
    ...
    let r := p.expr
    let h₂ ← p.getProof
    let h := mkApp3 (mkAppRange (mkConst ``Lean.Lynth.Grind.dite_cond_eq_false' f.constLevels!) 0 5 args) r h₁ h₂
    applyCongrFun e r h 5 args (ite := false)

lynth_builtin_grind_propagator propagateDecideDown ↓decide := fun e => do
  let root ← getRootENode e
  unless root.ctor do return ()
  let_expr decide p h := e | return ()
  if root.self.isConstOf ``true then
    pushEqTrue p <| mkApp3 (mkConst ``Lean.Grind.of_decide_eq_true) p h (← mkEqProof e root.self)
  else if root.self.isConstOf ``false then
    pushEqFalse p <| mkApp3 (mkConst ``Lean.Grind.of_decide_eq_false) p h (← mkEqProof e root.self)

lynth_builtin_grind_propagator propagateDecideUp ↑decide := fun e => do
  let_expr decide p h := e | return ()
  if (← isEqTrue p) then
    pushEq e (← getBoolTrueExpr) <| mkApp3 (mkConst ``Lean.Grind.decide_eq_true) p h (← mkEqTrueProof p)
  else if (← isEqFalse p) then
    pushEq e (← getBoolFalseExpr) <| mkApp3 (mkConst ``Lean.Grind.decide_eq_false) p h (← mkEqFalseProof p)

/-- `Bool` version of `propagateAndUp` -/
lynth_builtin_grind_propagator propagateBoolAndUp ↑Bool.and := fun e => do
  let_expr Bool.and a b := e | return ()
  if (← isEqBoolTrue a) then
    pushEq e b <| mkApp3 (mkConst ``Lean.Grind.Bool.and_eq_of_eq_true_left) a b (← mkEqBoolTrueProof a)
  else if (← isEqBoolTrue b) then
    pushEq e a <| mkApp3 (mkConst ``Lean.Grind.Bool.and_eq_of_eq_true_right) a b (← mkEqBoolTrueProof b)
  else if (← isEqBoolFalse a) then
    pushEqBoolFalse e <| mkApp3 (mkConst ``Lean.Grind.Bool.and_eq_of_eq_false_left) a b (← mkEqBoolFalseProof a)
  else if (← isEqBoolFalse b) then
    pushEqBoolFalse e <| mkApp3 (mkConst ``Lean.Grind.Bool.and_eq_of_eq_false_right) a b (← mkEqBoolFalseProof b)

/-- `Bool` version of `propagateAndDown` -/
lynth_builtin_grind_propagator propagateBoolAndDown ↓Bool.and := fun e => do
  if (← isEqBoolTrue e) then
    let_expr Bool.and a b := e | return ()
    let h ← mkEqBoolTrueProof e
    pushEqBoolTrue a <| mkApp3 (mkConst ``Lean.Grind.Bool.eq_true_of_and_eq_true_left) a b h
    pushEqBoolTrue b <| mkApp3 (mkConst ``Lean.Grind.Bool.eq_true_of_and_eq_true_right) a b h

/-- `Bool` version of `propagateOrUp` -/
lynth_builtin_grind_propagator propagateBoolOrUp ↑Bool.or := fun e => do
  let_expr Bool.or a b := e | return ()
  if (← isEqBoolFalse a) then
    pushEq e b <| mkApp3 (mkConst ``Lean.Grind.Bool.or_eq_of_eq_false_left) a b (← mkEqBoolFalseProof a)
  else if (← isEqBoolFalse b) then
    pushEq e a <| mkApp3 (mkConst ``Lean.Grind.Bool.or_eq_of_eq_false_right) a b (← mkEqBoolFalseProof b)
  else if (← isEqBoolTrue a) then
    pushEqBoolTrue e <| mkApp3 (mkConst ``Lean.Grind.Bool.or_eq_of_eq_true_left) a b (← mkEqBoolTrueProof a)
  else if (← isEqBoolTrue b) then
    pushEqBoolTrue e <| mkApp3 (mkConst ``Lean.Grind.Bool.or_eq_of_eq_true_right) a b (← mkEqBoolTrueProof b)

/-- `Bool` version of `propagateOrDown` -/
lynth_builtin_grind_propagator propagateBoolOrDown ↓Bool.or := fun e => do
  if (← isEqBoolFalse e) then
    let_expr Bool.or a b := e | return ()
    let h ← mkEqBoolFalseProof e
    pushEqBoolFalse a <| mkApp3 (mkConst ``Lean.Grind.Bool.eq_false_of_or_eq_false_left) a b h
    pushEqBoolFalse b <| mkApp3 (mkConst ``Lean.Grind.Bool.eq_false_of_or_eq_false_right) a b h

/-- `Bool` version of `propagateNotUp` -/
lynth_builtin_grind_propagator propagateBoolNotUp ↑Bool.not := fun e => do
  let_expr Bool.not a := e | return ()
  if (← isEqBoolFalse a) then
    pushEqBoolTrue e <| mkApp2 (mkConst ``Lean.Grind.Bool.not_eq_of_eq_false) a (← mkEqBoolFalseProof a)
  else if (← isEqBoolTrue a) then
    pushEqBoolFalse e <| mkApp2 (mkConst ``Lean.Grind.Bool.not_eq_of_eq_true) a (← mkEqBoolTrueProof a)
  else if (← isEqv e a) then
    closeGoal <| mkApp2 (mkConst ``Lean.Grind.Bool.false_of_not_eq_self) a (← mkEqProof e a)

/-- `Bool` version of `propagateNotDown` -/
lynth_builtin_grind_propagator propagateBoolNotDown ↓Bool.not := fun e => do
  let_expr Bool.not a := e | return ()
  if (← isEqBoolFalse e) then
    pushEqBoolTrue a <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_true_of_not_eq_false) a (← mkEqBoolFalseProof e)
  else if (← isEqBoolTrue e) then
    pushEqBoolFalse a <| mkApp2 (mkConst ``Lean.Grind.Bool.eq_false_of_not_eq_true) a (← mkEqBoolTrueProof e)
  else if (← isEqv e a) then
    closeGoal <| mkApp2 (mkConst ``Lean.Grind.Bool.false_of_not_eq_self) a (← mkEqProof e a)
