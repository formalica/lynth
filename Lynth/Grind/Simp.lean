...
/-- Simplifies the given expression using the `lynth_grind` simprocs and normalization theorems. -/
def simpCore (e : Expr) : GrindM Simp.Result := do profileitM Exception "grind simp" (← getOptions) do
  ...
  that have already been fully processed by `lynth_grind`. The idea was to use the `inShareCommon` test since
  the last step of the `lynth_grind` normalizer is `shareCommon`.
  However, this optimization was incorrect because there are other places in the `lynth_grind` code base that
  use `shareCommon`.
  -/
...
/--
Preprocesses `e` using `lynth_grind` normalization theorems and simprocs,
...
but ensures assumptions made by `lynth_grind` are satisfied.
-/
