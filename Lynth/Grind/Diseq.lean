...
def mkDiseqProofUsing (a b : Expr) (eq : Expr) : GoalM Expr := do
  ...
  let h ← if isSameExpr a c then
    ...
    pure <| mkApp6 (mkConst ``Lean.Grind.ne_of_ne_of_eq_left u) α a c d (← mkEqProof a c) h
  -- `h : a ≠ d
  if isSameExpr b d then
    return h
  else
    return mkApp6 (mkConst ``Lean.Grind.ne_of_ne_of_eq_right u) α b a d (← mkEqProof b d) h
...
def mkDiseqProof (a b : Expr) : GoalM Expr := do
 let some h ← mkDiseqProof? a b
   | throwError "internal `lynth_grind` error, failed to build disequality proof for{indentExpr a}\nand{indentExpr b}"
  return h
