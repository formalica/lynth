...
/-- Extension backing the `@[lynth_grind homo]` attribute. -/
initialize homoExt : Sym.Simp.SymSimpExtension ← Sym.Simp.mkSymSimpExt
...
-/
initialize homoSourceTypesExt : SimpleScopedEnvExtension Name NameSet ←
...
/-- Extension backing the `@[lynth_grind homo_pred]` attribute. -/
initialize homoPredExt : SimpleScopedEnvExtension (Name × HomoPredTheorem) HomoPredTheorems ←
  registerSimpleScopedEnvExtension {
    initial  := {}
