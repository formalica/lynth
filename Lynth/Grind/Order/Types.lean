...
structure Struct where
  ...
  /-- `id` of the `CommRing` (or `Ring`) structure in the `lynth_grind ring` module if available. -/
  ringId?            : Option Nat
...
/-- State for all order types detected by `lynth_grind`. -/
...
initialize orderExt : SolverExtension State ← registerSolverExtension (return {})
