...
import Lynth.Grind.PropagatorAttr
import Lynth.Grind.Order.Util
import Lynth.Grind.Order.Proof
open Lean.Meta
...
public def propagateEqTrue (c : Cnstr NodeId) (e : Expr) (u v : NodeId) (k k' : Weight) : OrderM Unit := do
  ...
  if let some he := c.h? then
    h := mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_trans_true) e c.e he h
  if let some (e', he) := (← get').termMapInv.find? { expr := e } then
    h := mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_trans_true) e' e he h
    pushEqTrue e' h
  else
    pushEqTrue e h
...
public def propagateEqFalse (c : Cnstr NodeId) (e : Expr) (u v : NodeId) (k k' : Weight) : OrderM Unit := do
  ...
  if let some he := c.h? then
    h := mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_trans_false) e c.e he h
  if let some (e', he) := (← get').termMapInv.find? { expr := e } then
    h := mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_trans_false) e' e he h
    pushEqFalse e' h
  else
    pushEqFalse e h
    ...
    | .eq u v =>
      ...
      unless (← isEqv ue' ve') do
        ...
        **Note**: We currently only support `Nat` originals. Thus `↑a` is actually
        ...
        `Rat`), `pushEq ue ve h` above is sufficient and `lynth_grind` core can derive
        the `Nat` equality via `norm_cast`/cast injectivity if needed.
        -/
        if (← inferType ue) == Int.mkType then
          pushEq ue' ve' <| mkApp7 (mkConst ``Lean.Lynth.Grind.Order.nat_eq) ue' ve' ue ve h₁ h₂ h
where
  /--
...
/--
Returns `true` if `e` is already `True` in the `lynth_grind` core.
...
/--
Returns `true` if `e` is already `False` in the `lynth_grind` core.
Recall that `e` may be an auxiliary term created for a term `e'` (see `cnstrsMapInv`).
-/
...
def assertIneqTrue (c : Cnstr NodeId) (e : Expr) (he : Expr) : OrderM Unit := do
  ...
  let h ←  if let some h := c.h? then
    pure <| mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_mp) e c.e h (mkOfEqTrueCore e he)
  else
...
def assertIneqFalse (c : Cnstr NodeId) (e : Expr) (he : Expr) : OrderM Unit := do
  ...
  trace[grind.order.assert] "¬ {← c.pp}"
  let h ←  if let some h := c.h? then
    pure <| mkApp4 (mkConst ``Lean.Lynth.Grind.Order.eq_mp_not) e c.e h (mkOfEqFalseCore e he)
  else
    pure <| mkOfEqFalseCore e he
  if (← isRing) then
    let declName := if c.kind matches .lt then
      ``Lean.Lynth.Grind.Order.le_of_not_lt_k
    else
      ``Lean.Lynth.Grind.Order.lt_of_not_le_k
    ...
    if (← isInt) && strict then
      h := mkApp6 (mkConst ``Lean.Lynth.Grind.Order.int_lt) (← getExpr c.v) (← getExpr c.u) (toExpr k') (toExpr (k'-1)) eagerReflBoolTrue h
      k'     := k' - 1
      strict := !strict
    addEdge c.v c.u { k := k', strict } h
  else if c.kind matches .lt then
    let h' ← mkLeLtLinearPrefix ``Lean.Lynth.Grind.Order.le_of_not_lt
    let h := mkApp3 h' (← getExpr c.u) (← getExpr c.v) h
    addEdge c.v c.u { strict := false } h
  else if (← hasLt) then
    let h' ← mkLeLtLinearPrefix ``Lean.Lynth.Grind.Order.lt_of_not_le
    let h := mkApp3 h' (← getExpr c.u) (← getExpr c.v) h
    addEdge c.v c.u { strict := true } h
  else
    let h' ← mkLeLinearPrefix ``Lean.Lynth.Grind.Order.le_of_not_le
    let h := mkApp3 h' (← getExpr c.u) (← getExpr c.v) h
    addEdge c.v c.u { strict := false } h
...
lynth_builtin_grind_propagator propagateLE ↓LE.le := propagateIneq
lynth_builtin_grind_propagator propagateLT ↓LT.lt := propagateIneq
  ...
  unless isSameExpr a b do
    ...
    if let some { e := a', h := h₁, α } ← getAuxTerm? a then
      ...
      if α == Int.mkType then
        let h := mkApp7 (mkConst ``Lean.Lynth.Grind.Order.of_nat_eq) a b a' b' h₁ h₂ h
        go a' b' h
      else
        let u ← getDecLevel α
        let inst ← synthInstance (mkApp (mkConst ``NatCast [u]) α)
        let h := mkApp9 (mkConst ``Lean.Lynth.Grind.Order.of_natCast_eq [u]) α inst a b a' b' h₁ h₂ h
        go a' b' h
    else
      go a b h
where
  ...
  go (a b h : Expr) : GoalM Unit := do
    ...
    OrderM.run id₁ do
      ...
      let v ← getNodeId b
      if (← isRing) then
        let h₁ := mkApp3 (← mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_of_eq_1_k) a b h
        let h₂ := mkApp3 (← mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_of_eq_2_k) a b h
        addEdge u v {} h₁
        addEdge v u {} h₂
      else
        let h₁ := mkApp3 (← mkLePreorderPrefix ``Lean.Lynth.Grind.Order.le_of_eq_1) a b h
        let h₂ := mkApp3 (← mkLePreorderPrefix ``Lean.Lynth.Grind.Order.le_of_eq_2) a b h
        addEdge u v {} h₁
        addEdge v u {} h₂
