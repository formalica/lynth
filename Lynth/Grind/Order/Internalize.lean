...
import Lynth.Grind.Arith.CommRing.Proof
import Lynth.Grind.Arith.Cutsat.Nat
import Lynth.Grind.Order.StructId
import Lynth.Grind.Order.Util
import Lynth.Grind.Order.Assert
    ...
    **Note**: `linarith` uses a virtual parent to mark auxiliary declarations used to encode
    ...
    if parent.isEq then
      ...
      **Note**: We currently ignore `•`. We may reconsider it in the future.
      ...
      | Lean.Grind.IntModule.OfNatModule.toQ _ _ _ => true
      | _ => false
  else
    false
...
def mkLeNorm0 (s : Struct) (ringInst : Expr) (lhs rhs : Expr) : Expr :=
  mkApp5 (mkConst ``Lean.Grind.CommRing.le_norm0 [s.u]) s.type ringInst s.leInst lhs rhs

def mkLtNorm0 (s : Struct) (ringInst : Expr) (lhs rhs : Expr) : Expr :=
  mkApp5 (mkConst ``Lean.Grind.CommRing.lt_norm0 [s.u]) s.type ringInst s.ltInst?.get! lhs rhs

def mkEqNorm0 (s : Struct) (ringInst : Expr) (lhs rhs : Expr) : Expr :=
  mkApp4 (mkConst ``Lean.Grind.CommRing.eq_norm0 [s.u]) s.type ringInst lhs rhs
...
def mkCommRingCnstr? (s : Struct) (kind : CnstrKind) (lhs rhs : Expr) : RingM (Option (Cnstr Expr)) := do
  ...
  let some p ← toPolyM? (lhs.sub rhs) | return none
...
def toOffsetTermCommRing? (e : Expr) : RingM (Option OffsetTermResult) := do
  ...
  let some p ← toPolyM? e | return none
  let k := p.getConst
  let p := p.addConst (-k)
...
def internalizeTerm (e : Expr) : OrderM Unit := do
  ...
  let x ← mkNode e
  let y ← mkNode r.a
  let h₁ ← mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_of_offset_eq_1_k
  let h₁ := mkApp4 h₁ e r.a (toExpr r.k) r.h
  addEdge x y { k := r.k } h₁
  let h₂ ← mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_of_offset_eq_2_k
  let h₂ := mkApp4 h₂ e r.a (toExpr r.k) r.h
  addEdge y x { k := -r.k } h₂
