...
def mkTransCoreProof (u v w : Expr) (strict₁ strict₂ : Bool) (h₁ h₂ : Expr) : OrderM Expr := do
  let h ← match strict₁, strict₂ with
    | false, false => mkLePreorderPrefix ``Lean.Lynth.Grind.Order.le_trans
    | false, true  => mkLeLtPreorderPrefix ``Lean.Lynth.Grind.Order.le_lt_trans
    | true,  false => mkLeLtPreorderPrefix ``Lean.Lynth.Grind.Order.lt_le_trans
    | true,  true  => mkLeLtPreorderPrefix ``Lean.Lynth.Grind.Order.lt_trans
...
def mkTransOffsetProof (u v w : Expr) (k₁ k₂ : Weight) (h₁ h₂ : Expr) : OrderM Expr := do
  let h ← match k₁.strict, k₂.strict with
    | false, false => mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_trans_k
    | false, true  => mkOrdRingPrefix ``Lean.Lynth.Grind.Order.le_lt_trans_k
    | true,  false => mkOrdRingPrefix ``Lean.Lynth.Grind.Order.lt_le_trans_k
    | true,  true  => mkOrdRingPrefix ``Lean.Lynth.Grind.Order.lt_trans_k
  let k  := k₁.k + k₂.k
...
def mkPropagateEqTrueProofCore (u v : Expr) (k : Weight) (huv : Expr) (k' : Weight) : OrderM Expr := do
  if k.strict == k'.strict then
    ...
    let h ← mkLeLtPrefix ``Lean.Lynth.Grind.Order.le_eq_true_of_lt
...
def mkPropagateEqTrueProofOffset (u v : Expr) (k : Weight) (huv : Expr) (k' : Weight) : OrderM Expr := do
  let declName := match k'.strict, k.strict with
    | false, false => ``Lean.Lynth.Grind.Order.le_eq_true_of_le_k
    | false, true  => ``Lean.Lynth.Grind.Order.le_eq_true_of_lt_k
    | true,  false => ``Lean.Lynth.Grind.Order.lt_eq_true_of_le_k
    | true,  true  => ``Lean.Lynth.Grind.Order.lt_eq_true_of_lt_k
...
def mkPropagateSelfEqTrueProofOffset (u : Expr) (k : Weight) : OrderM Expr := do
  let declName := match k.strict with
    | false => ``Lean.Lynth.Grind.Order.le_eq_true_k
    | true  => ``Lean.Lynth.Grind.Order.lt_eq_true_k
  let h ← mkOrdRingPrefix declName
  return mkApp3 h u (toExpr k.k) eagerReflBoolTrue

def mkPropagateSelfEqTrueProofCore (u : Expr) : OrderM Expr := do
  let h ← mkLePreorderPrefix ``Lean.Lynth.Grind.Order.le_eq_true
  return mkApp h u
...
def mkPropagateEqFalseProofCore (u v : Expr) (k : Weight) (huv : Expr) (k' : Weight) : OrderM Expr := do
  let declName := match k'.strict, k.strict with
    ...
    | false, true  => ``Lean.Lynth.Grind.Order.le_eq_false_of_lt
    | true,  false => ``Lean.Lynth.Grind.Order.lt_eq_false_of_le
    | true,  true  => ``Lean.Lynth.Grind.Order.lt_eq_false_of_lt
  let h ← mkLeLtPreorderPrefix declName
  return mkApp3 h u v huv

def mkPropagateEqFalseProofOffset (u v : Expr) (k : Weight) (huv : Expr) (k' : Weight) : OrderM Expr := do
  let declName := match k'.strict, k.strict with
    | false, false => ``Lean.Lynth.Grind.Order.le_eq_false_of_le_k
    | false, true  => ``Lean.Lynth.Grind.Order.le_eq_false_of_lt_k
    | true,  false => ``Lean.Lynth.Grind.Order.lt_eq_false_of_le_k
    | true,  true  => ``Lean.Lynth.Grind.Order.lt_eq_false_of_lt_k
  let h ← mkOrdRingPrefix declName
  return mkApp6 h u v (toExpr k.k) (toExpr k'.k) eagerReflBoolTrue huv
...
def mkPropagateSelfEqFalseProofOffset (u : Expr) (k : Weight) : OrderM Expr := do
  let declName := match k.strict with
    | false => ``Lean.Lynth.Grind.Order.le_eq_false_k
    | true  => ``Lean.Lynth.Grind.Order.lt_eq_false_k
  let h ← mkOrdRingPrefix declName
  return mkApp3 h u (toExpr k.k) eagerReflBoolTrue

def mkPropagateSelfEqFalseProofCore (u : Expr) : OrderM Expr := do
  let h ← mkLeLtPrefix ``Lean.Lynth.Grind.Order.lt_eq_false
  return mkApp h u
...
def mkSelfUnsatProofCore (u : Expr) (h : Expr) : OrderM Expr := do
  let hf ← mkLeLtPreorderPrefix ``Lean.Lynth.Grind.Order.lt_unsat
  return mkApp2 hf u h

def mkSelfUnsatProofOffset (u : Expr) (k : Weight) (h : Expr) : OrderM Expr := do
  let declName := if k.strict then
    ``Lean.Lynth.Grind.Order.lt_unsat_k
  else
    ``Lean.Lynth.Grind.Order.le_unsat_k
...
public def mkEqProofOfLeOfLeCore (u v : Expr) (h₁ : Expr) (h₂ : Expr) : OrderM Expr := do
  let h ← mkLePartialPrefix ``Lean.Lynth.Grind.Order.eq_of_le_of_le
  return mkApp4 h u v h₁ h₂

public def mkEqProofOfLeOfLeOffset (u v : Expr) (h₁ : Expr) (h₂ : Expr) : OrderM Expr := do
  let h ← mkLePartialPrefix ``Lean.Lynth.Grind.Order.eq_of_le_of_le_0
