...
mutual
  ...
  partial def mkEqCongrProof (lhs rhs : Expr) : GoalM Expr := withIncRecDepth do
    ...
    if !isSameExpr α₁ α₂ then
      return mkApp8 (mkConst ``Lean.Grind.heq_congr us) α₁ α₂ a₁ b₁ a₂ b₂ (← mkEqProofCore a₁ a₂ true) (← mkEqProofCore b₁ b₂ true)
    else
      return mkApp7 (mkConst ``Lean.Grind.eq_congr us) α₁ a₁ b₁ a₂ b₂ (← mkEqProofCore a₁ a₂ false) (← mkEqProofCore b₁ b₂ false)
  ...
  partial def mkEqCongrSymmProof (lhs rhs : Expr) : GoalM Expr := withIncRecDepth do
    ...
    let us := f.constLevels!
    if !isSameExpr α₁ α₂ then
      return mkApp8 (mkConst ``Lean.Grind.heq_congr' us) α₁ α₂ a₁ b₁ a₂ b₂ (← mkEqProofCore a₁ b₂ true) (← mkEqProofCore b₁ a₂ true)
    else
      return mkApp7 (mkConst ``Lean.Grind.eq_congr' us) α₁ a₁ b₁ a₂ b₂ (← mkEqProofCore a₁ b₂ false) (← mkEqProofCore b₁ a₂ false)
  ...
  private partial def mkHCongrProof (lhs rhs : Expr) (heq : Bool) : GoalM Expr := do
    ...
    if fInfo.getArity < numArgs then
      ...
      let throwUnsupported {α} : GoalM α :=
        throwError "`lynth_grind` currently cannot build congruence proofs for over-applied terms such as{indentExpr lhs}\nand{indentExpr rhs}"
  ...
  private partial def mkCongrProof (lhs rhs : Expr) (heq : Bool) : GoalM Expr := do
    if let .forallE _ p₁ q₁ _ := lhs then
      ...
      let numArgs := lhs.getAppNumArgs
      assert! rhs.getAppNumArgs == numArgs
      if numArgs == 2 && f.isConstOf ``Lean.Grind.nestedProof && g.isConstOf ``Lean.Grind.nestedProof then
        mkNestedProofCongr lhs rhs heq
      else if numArgs == 2 && f.isConstOf ``Lean.Grind.nestedDecidable && g.isConstOf ``Lean.Grind.nestedDecidable then
...
def mkEqProofImpl (a b : Expr) : GoalM Expr := do
  unless (← hasSameType a b) do
    throwError "internal `lynth_grind` error, `mkEqProof` invoked with terms of different types{indentExpr a}\nhas type{indentExpr (← inferType a)}\nbut{indentExpr b}\nhas type{indentExpr (← inferType b)}"
