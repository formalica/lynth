...
/-!
Given an auto-generated `lynth_grind` tactic script, collect params for
single shot `finish` or top-level `lynth_grind` tactic.
-/
...
/--
Given a `lynth_grind` tactic sequence, extracts parameters and builds an terminal `finish only` tactic.
-/
...
/--
Given a `lynth_grind` tactic sequence, extracts parameters and builds a `lynth_grind only` tactics.
...
The `extraParams` are additional parameters (e.g., term arguments from the original `lynth_grind?` call)
...
where
  mkTac (params : Array TParam) : CoreM (TSyntax `tactic) :=
    if params.isEmpty then
      `(tactic| lynth_grind $cfg:optConfig only)
    else
      `(tactic| lynth_grind $cfg:optConfig only [$params,*])
