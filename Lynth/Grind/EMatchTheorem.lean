...
module
prelude
public import Lynth.Grind.Extension
...
-- Local readers for core's `backward.grind.*` / `grind.param.*` options
-- (see `Lynth.Grind.Types` option readers; kept here to avoid an import
-- cycle, since this module sits below `Types` in the import graph).
def backward.grind.inferPattern.get (o : Options) : Bool :=
  o.get `backward.grind.inferPattern false
def backward.grind.checkInferPatternDiscrepancy.get (o : Options) : Bool :=
  o.get `backward.grind.checkInferPatternDiscrepancy false
def grind.param.codeAction.get (o : Options) : Bool := o.get `grind.param.codeAction false
/-!
## Design Note: Symbol Priorities and Extension State

We considered including `SymbolPriorities` in `ExtensionState` to allow each `lynth_grind` attribute/extension
to define its own symbol priorities. However, this design was rejected because E-match patterns are selected
with respect to symbol priorities. When using multiple `lynth_grind` attributes simultaneously
(e.g., `lynth_grind only [attr_1, attr_2]`), patterns would need to be re-selected using the union of all
symbol priorities and then re-normalized using the union of all normalizers, an expensive operation we want to avoid.

Instead, we use a single global `SymbolPriorities` set shared across all `lynth_grind` attributes.
...
private initialize symbolPrioExt : SimpleScopedEnvExtension SymbolPriorityEntry SymbolPriorities ←
...
def mkOffsetPattern (pat : Expr) (k : Nat) : Expr :=
  mkApp2 (mkConst ``Lean.Grind.offset) pat (mkRawNatLit k)
...
def isOffsetPattern? (pat : Expr) : Option (Expr × Nat) := Id.run do
  let_expr Lean.Grind.offset pat k := pat | none
...
def mkEqBwdPattern (u : List Level) (α : Expr) (lhs rhs : Expr) : Expr :=
  mkApp3 (mkConst ``Lean.Grind.eqBwdPattern u) α lhs rhs

def isEqBwdPattern (e : Expr) : Bool :=
  e.isAppOfArity ``Lean.Grind.eqBwdPattern 3

def isEqBwdPattern? (e : Expr) : Option (Expr × Expr) :=
  let_expr Lean.Grind.eqBwdPattern _ lhs rhs := e
    | none
  some (lhs, rhs)

def mkGenPattern (u : List Level) (α : Expr) (h : Expr) (x : Expr) (val : Expr) : Expr :=
  mkApp4 (mkConst ``Lean.Grind.genPattern u) α h x val

def mkGenHEqPattern (u : List Level) (α β : Expr) (h : Expr) (x : Expr) (val : Expr) : Expr :=
  mkApp5 (mkConst ``Lean.Grind.genHEqPattern u) α β h x val

/-- Generalized pattern information. See `Lean.Grind.genPattern` gadget. -/
...
the gadgets `Lean.Grind.genPattern` and `Lean.Grind.genHEqPattern`.
...
forall
  ...
  (val✝ : Nat)
  (heq_1 : a✝ = some val✝),
  g.match_1 motive (Lean.Grind.genPattern (Option Nat) heq_1 a✝ (some val✝)) h_1 h_2
  ≍ h_2 val✝ heq_1
```
The gadget is used to infer a `generalize` pattern. The following term is used
during E-matching `g.match_1 motive (Lean.Grind.genPattern (Option Nat) heq_1 a✝ (some val✝)) h_1 h_2`
when matching `Lean.Grind.genPattern (Option Nat) heq_1 a✝ (some val✝)` the matcher uses
...
```
pbind (Lean.Grind.genPattern h x (some a)) f = f a h
```
-/
private def detectGeneralizedPatterns? (type : Expr) : MetaM Expr := do
  -- `genPattern` is not necessarily referenced in the result but always importing it for `lynth_grind`
  -- uses is a solid approximation.
  recordExtraModUseFromDecl (isMeta := false) ``Lean.Grind.genPattern
  forallTelescopeReducing type fun hs resultType => do
...
-- Configuration for the `lynth_grind` normalizer. We want both `zetaDelta` and `zeta`
private def normConfig : Lean.Grind.Config := {}
...
def EMatchTheorem.getProofWithFreshMVarLevels (thm : EMatchTheorem) : MetaM Expr := do
  Lean.Lynth.Grind.getProofWithFreshMVarLevels thm
...
def mkGroundPattern (e : Expr) : Expr :=
  mkAnnotation `lynth_grind.ground_pat e

/--
Puts a parametric numeric literal in `lynth_grind` normal form.

`lynth_grind` represents `BitVec` and `Fin` literals using `OfNat.ofNat` (see
...
kinds here. Waiting on the port of `Sym.dsimp` to `lynth_grind`. The same hardcoding is in
...
def groundPattern? (e : Expr) : Option Expr :=
  annotation? `lynth_grind.ground_pat e
...
partial def ppPattern (pattern : Expr) : MessageData := Id.run do
  if let some e := groundPattern? pattern then
    ...
    | _ =>
      if pattern.isAppOfArity ``Lean.Grind.offset 2 then
...
private def saveSymbol (h : HeadIndex) : M Unit := do
  if let .const declName := h then
    if declName == ``Lean.Grind.genHEqPattern || declName == ``Lean.Grind.genPattern then
  ...
  e.forEach' fun e => do
    if e.isApp || e.isConst then
      ...
      if let .const declName _ := e.getAppFn then
        if declName == ``OfNat.ofNat || declName == ``Lean.Grind.nestedProof
           || declName == ``Lean.Grind.eqBwdPattern
           || declName == ``Lean.Grind.nestedDecidable || declName == ``ite then
          return false
    match e with
    | .const .. =>
...
inductive PatternArgKind where
  ...
  | /--
    ...
    **Motivation.** Consider the `ToInt` class used by the `lynth_grind` linear arithmetic module:
    ```
    class ToInt (α : Type) (range : outParam IntInterval) where ...
    instance : ToInt (Fin n) (.co 0 n) where ...
    @[lynth_grind =] theorem toInt_fin (x : Fin n) : ToInt.toInt x = x.val
    ```
    ...
    because the pattern generator treats it as relevant. However, the `lynth_grind` normalizer pushes
    `NatCast.natCast` inside arithmetic operations, rewriting `↑(n + 1)` to `↑n + 1`. So when
    `lynth_grind` processes `Fin (n + 1)`, the e-graph contains:
    ```
    ...
    xs.mapIdxM fun idx x => do
      if (← isProp x) then
        ...
        if h : idx < pinfos.size then
          /-
          We originally wanted to ignore types and type formers in `lynth_grind` and treat them as supporting elements.
  ...
  if !pattern.isApp && !pattern.isConst then
    ...
    | f@(.const declName _) =>
      if !(← isCandidateSymbol declName root) then
        return none
      if declName == ``Lean.Grind.genPattern || declName == ``Lean.Grind.genHEqPattern then
        return some f
      if inSupport then
...
private inductive ParentKind where
  | /-- No special treatment needed. -/
    regular
  | /--
    The parent pattern is the gadget `Lean.Grind.eqBwdPattern`.
    In this case, we do not collect the symbols in ground children pattern.
    -/
    eqBwdPattern
  | /--
    The parent pattern is the gadget `Lean.Grind.genPattern` and `Lean.Grind.genHEqPattern`.
...
private def toParentKind (f : Expr) : ParentKind :=
  match f with
  | .const declName _ =>
    if declName == ``Lean.Grind.eqBwdPattern then
      .eqBwdPattern
    else if declName == ``Lean.Grind.genHEqPattern || declName == ``Lean.Grind.genPattern then
      .genPattern
    else
      .regular
  | _ => .regular
    ...
    if !arg.hasLooseBVars then
      if arg.hasMVar then
        ...
        if !inSupport && parentKind matches .regular then
          /-
          **Note**: We ignore symbols in ground patterns if the parent is the auxiliary ``Lean.Grind.eqBwdPattern
...
If `normalizePattern` is true, it applies the `lynth_grind` simplification theorems and simprocs to the pattern.
-/
...
-- **TODO**: Delete. We should use custom grind attributes for implementing the `lynth_lia` tactic.
...
If `normalizePattern` is true, it applies the `lynth_grind` simplification theorems and simprocs to the
pattern.
-/
...
private def collectPatterns? (proof : Expr) (xs : Array Expr) (searchPlaces : Array Expr) (symPrios : SymbolPriorities) (minPrio : Nat)
  ...
  if backward.grind.checkInferPatternDiscrepancy.get (← getOptions) then
    ...
    if oldResult? != newResult? then
      logWarning m!"found discrepancy between old and new `lynth_grind` pattern inference procedures, old:{indentD (toPattern oldResult?)}\nnew:{indentD (toPattern newResult?)}"
...
We use this function to implement local forall expressions in a `lynth_grind` goal.
-/
...
since the theorem is already in the `lynth_grind` state and there is nothing to be instantiated.
-/
def mkEMatchTheoremWithKind?
  ...
  ```
  @[lynth_grind] theorem State.join_le_left (σ₁ σ₂ : State) : σ₁.join σ₂ ≼ σ₁ := by
  ```
  We do not want `State.le` to be unfolded and the abstraction exposed.

  That said, we must still reduce `[reducible]` definitions since `lynth_grind` unfolds them.
  -/
  withReducible <| forallTelescopeReducing type fun xs type => withDefault do
    let searchPlaces ← match kind with
      | .fwd =>
        ...
        if ps.isEmpty then
          throwError "invalid `lynth_grind` forward theorem, theorem `{origin.pp}` does not have propositional hypotheses"
        pure ps
      | .bwd _ => pure #[type]
...
private def ensureNoMinIndexable (minIndexable : Bool) : MetaM Unit := do
  if minIndexable then
    throwError "redundant modifier `!` in `lynth_grind` attribute"
...
def EMatchTheoremKind.toSyntax (kind : EMatchTheoremKind) : CoreM (TSyntax ``Parser.Attr.grindMod) :=
  ...
  | .user           => throwError "`lynth_grind` theorem kind is not a modifier"
...
private def save (ref : Syntax) (thm : EMatchTheorem) (isParam : Bool) (minIndexable : Bool) : SelectM Unit := do
  ...
  if (← get).thms.all fun thm' => thm.patterns != thm'.patterns then
    ...
    let suggestion : Tactic.TryThis.SuggestionText ← match isParam, minIndexable with
      | false, true  => pure <| Tactic.TryThis.SuggestionText.tsyntax (← `(attr|lynth_grind! $(← thm.kind.toSyntax)))
      | false, false => pure <| .tsyntax (← `(attr|lynth_grind $(← thm.kind.toSyntax)))
...
/-- Helper type for generating suggestions for `lynth_grind` parameters -/
...
def mkEMatchTheoremAndSuggest (ref : Syntax) (declName : Name) (prios : SymbolPriorities)
  ...
  if h₁ : 0 < s.thms.size then
    ...
    throwError "invalid `lynth_grind` theorem, failed to find an usable pattern using different modifiers"
