...
open Command Meta Lean.Lynth.Grind

def checkEMatchTheorem (declName : Name) : CoreM Unit := do
  unless (← Lean.Lynth.Grind.lynthGrindExt.isEMatchTheorem declName) do
    throwError "`{declName}` is not marked with the `@[lynth_grind]` attribute for theorem instantiation"
...
-/
def defaultConfig : Lean.Grind.Config := {
  splits       := 0
  lookahead    := false
  mbtc         := false
  ematch       := 20
...
def mkConfig (items : Array (TSyntax `Lean.Parser.Tactic.configItem)) : TermElabM Lean.Grind.Config := do
  elabConfigItems defaultConfig items

def mkParams (config : Lean.Grind.Config) : MetaM Params := do
  let params ← Lean.Lynth.Grind.mkDefaultParams config
...
def sum (cs : PHashMap Lean.Lynth.Grind.Origin Nat) : Nat := Id.run do
  let mut r := 0
  for (_, c) in cs do
    r := r + c
  return r

def thmsToMessageData (thms : PHashMap Lean.Lynth.Grind.Origin Nat) : MetaM MessageData := do
...
def analyzeEMatchTheorem (declName : Name) (params : Params) : MetaM Bool := do
  ...
  if s > params.config.min then
    if s >= params.config.instances then
      logInfo m!"instantiating `{declName}` triggers more than {s} additional `lynth_grind` theorem instantiations"
    else
      logInfo m!"instantiating `{declName}` triggers {s} additional `lynth_grind` theorem instantiations"
...
def getTheorems (prefixes? : Option (Array Name)) (inModule : Bool) : CoreM (List Name) := do
  ...
  let origins := (← Lean.Lynth.Grind.lynthGrindExt.getEMatchTheorems).getOrigins
  let env ← getEnv
