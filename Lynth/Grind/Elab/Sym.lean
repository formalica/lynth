...
import Lynth.Grind.Elab.SimprocDSL
...
private def evalIntroCore (internalize : Bool) (ids : TSyntaxArray `Lean.binderIdent) : GrindTacticM Unit := do
  ...
  let goal ←
    if ids.isEmpty then
      match (← liftSymM <| Lean.Lynth.Grind.Goal.introN goal 1 hygienic) with
      ...
      match (← liftSymM <| Lean.Lynth.Grind.Goal.intros goal names hygienic) with
      | .goal _ goal => pure goal
      | .failed => throwError "`intro` failed"
  let goal ← if internalize then liftGrindM <| Lean.Lynth.Grind.Goal.internalizeAll goal else pure goal
  replaceMainGoal [goal]

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symIntro] def evalSymIntro : GrindTactic := fun stx => do
...
private def evalIntrosCore (internalize : Bool) : GrindTacticM Unit := do
  ...
  let hygienic := tactic.hygienic.get (← getOptions)
  let goal ← getMainGoal
  match (← liftSymM <| Lean.Lynth.Grind.Goal.intros goal #[] hygienic) with
  | .goal _ goal =>
    let goal ← if internalize then liftGrindM <| Lean.Lynth.Grind.Goal.internalizeAll goal else pure goal
    replaceMainGoal [goal]
  | .failed => throwError "`intros` failed"

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symIntros] def evalSymIntros : GrindTactic := fun stx => do
  match stx with
...
@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symApply] def evalSymApply : GrindTactic := fun stx => do
  ensureSym
  ...
  | _ =>
    getOrCreateBackwardRuleFromTerm term
  match (← liftSymM <| Lean.Lynth.Grind.Goal.apply goal rule) with
  | .goals subgoals => replaceMainGoal subgoals
  | .failed => throwError "`apply` failed, rule is not applicable"

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symInternalize] def evalSymInternalize : GrindTactic := fun stx => do
  ensureSym
  let goal ← getMainGoal
  let num := if stx[1].isNone then 1 else stx[1][0].toNat
  let goal ← liftGrindM <| Lean.Lynth.Grind.Goal.internalize goal num
  replaceMainGoal [goal]

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symInternalizeAll] def evalSymInternalizeAll : GrindTactic := fun _ => do
  ensureSym
  let goal ← getMainGoal
  let goal ← liftGrindM <| Lean.Lynth.Grind.Goal.internalizeAll goal
  replaceMainGoal [goal]

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symByContra] def evalSymByContra : GrindTactic := fun _ => do
  ensureSym
  let goal ← getMainGoal
...
@[builtin_lynth_grind_tactic Parser.Tactic.Grind.symSimp] def evalSymSimp : GrindTactic := fun stx => withMainContext do
  ensureSym
