...
module
prelude
public import Lynth.Grind.Elab.Filter
import Lynth.Grind.PP
import Lynth.Grind.EMatchTheoremParam
import Lynth.Grind.Split
namespace Lean.Lynth.Grind.Elab
open Lean.Elab
open Lean.Elab.Tactic
open Lean.Meta
open Lean.Lynth.Grind
...
def ppAsserted? (filter : Filter) (collapsed := false) : GrindTacticM (Option MessageData) := do
    ...
    if facts.isEmpty then
      return none
    return some <| Lean.Lynth.Grind.ppExprArray `facts "Asserted facts" facts (collapsed := collapsed)

@[builtin_lynth_grind_tactic showAsserted] def evalShowAsserted : GrindTactic := fun stx => withMainContext do
...
def ppProps? (filter : Filter) (isTrue : Bool) (collapsed := false) : GrindTacticM (Option MessageData) := do
  ...
  if props.isEmpty then
    return none
  return some <| Lean.Lynth.Grind.ppExprArray `props s!"{if isTrue then "True" else "False"} propositions" props (collapsed := collapsed)
...
@[builtin_lynth_grind_tactic showTrue] def evalShowTrue : GrindTactic := fun stx => do
  let `(grind| show_true $[$filter?]?) := stx | throwUnsupportedSyntax
  showProps filter? true

@[builtin_lynth_grind_tactic showFalse] def evalShowFalse : GrindTactic := fun stx => do
...
@[builtin_lynth_grind_tactic showEqcs] def evalShowEqcs : GrindTactic := fun stx => withMainContext do
...
public def showState (filter : Filter) (isSilent := false) : GrindTacticM Unit := do
  ...
  logAt (← getRef) (severity := .information) (isSilent := isSilent) <| MessageData.trace { cls := `lynth_grind, collapsed := false } "Grind state" msgs

@[builtin_lynth_grind_tactic Parser.Tactic.Grind.showState] def evalShowState : GrindTactic := fun stx => withMainContext do
  let `(grind| show_state $[$filter?]?) := stx | throwUnsupportedSyntax
  let filter ← elabFilter filter?
  showState filter

@[builtin_lynth_grind_tactic showCases] def evalShowCases : GrindTactic := fun stx => withMainContext do
...
@[builtin_lynth_grind_tactic showLocalThms] def evalShowLocalThms : GrindTactic := fun _ => withMainContext do
...
@[builtin_lynth_grind_tactic showTerm] def evalShowTerm : GrindTactic := fun stx => do
