...
structure Context extends Tactic.Context where
  ctx     : Lean.Lynth.Grind.Context
  sctx    : Meta.Sym.Context
  methods : Lean.Lynth.Grind.Methods
  params  : Lean.Lynth.Grind.Params
  sym     : Bool := false

open Lean.Lynth.Grind (Goal)
...
structure State where
  ...
  grindState : Lean.Lynth.Grind.State
  goals      : List Goal
  cache      : Cache := {}
...
unsafe initialize grindTacElabAttribute : KeyedDeclsAttribute GrindTactic ←
  mkElabAttribute GrindTactic `builtin_lynth_grind_tactic `lynth_grind_tactic
    `Lean.Parser.Tactic.Grind `Lean.Lynth.Grind.Elab.GrindTactic "grind"
  ...
  withRef stx <| withIncRecDepth <| withFreshMacroScope <| match stx with
    | .node _ k _    =>
      if k == nullKind then
        ...
        let s ← Lean.Lynth.Grind.Elab.saveState
...
where
    ...
    @[inline] handleEx (s : SavedState) (failures : Array EvalTacticFailure) (ex : Exception) (k : Array EvalTacticFailure → GrindTacticM Unit) := do
      ...
      | .error .. =>
        ...
        let failures := failures.push ⟨ex, ← Lean.Lynth.Grind.Elab.saveState⟩
...
instance : MonadBacktrack SavedState GrindTacticM where
  saveState := Lean.Lynth.Grind.Elab.saveState
...
instance : MonadExcept Exception GrindTacticM where
  throw    := throw
  tryCatch := Lean.Lynth.Grind.Elab.tryCatchRestore
...
instance : OrElse (GrindTacticM α) where
  orElse := Lean.Lynth.Grind.Elab.orElse
...
/-
**Note**: Recall that `lynth_grind` uses the reducibility specified at `Config.reducible`
-/
def liftGrindM (k : GrindM α) : GrindTacticM α := do
  let ctx ← read
  let s ← get
  let ((a, grindState), symState) ← liftMetaM <| StateRefT'.run (((Lean.Lynth.Grind.withGTransparency k) ctx.methods.toMethodsRef ctx.ctx |>.run s.grindState) ctx.sctx) s.symState
...
def done : GrindTacticM Unit := do
  ...
  unless goals.isEmpty do
    ...
    let results ← liftGrindM do goals.mapM fun goal => Lean.Lynth.Grind.mkResult params (some goal)
...
def mkEvalTactic' (elaborator : Name) (params : Params) : TermElabM (Goal → TSyntax `grind → GrindM (List Goal)) := do
  ...
  let eval (goal : Goal) (stx : TSyntax `grind) : GrindM (List Goal) := do
    ...
    let grindCtx ← readThe Lean.Lynth.Grind.Context
  ...
  let (methods, ctx, sctx, state) ← liftMetaM <| GrindM.runAtGoal mvarId params' (evalTactic? := some evalTactic) fun goal => do
      let goals ←
        if sym then
          ...
          | .stuck gs => pure gs
      let methods ← getMethods
      let ctx ← readThe Lean.Lynth.Grind.Context
