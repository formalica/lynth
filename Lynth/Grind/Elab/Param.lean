...
import Lynth.Grind.Elab.Anchor
import Lean.Elab.SyntheticMVars
import Lynth.Grind.Types
...
/-!
`lynth_grind` parameter elaboration
-/

def _root_.Lean.Lynth.Grind.Params.insertCasesTypes (params : Lean.Lynth.Grind.Params) (declName : Name) (eager : Bool) : Lean.Lynth.Grind.Params :=
  { params with extensions := params.extensions.modify 0 fun ext => { ext with casesTypes := ext.casesTypes.insert declName eager } }

def _root_.Lean.Lynth.Grind.Params.eraseCasesTypes (params : Lean.Lynth.Grind.Params) (declName : Name) : CoreM Lean.Lynth.Grind.Params := do
  unless params.extensions.any fun ext => ext.casesTypes.contains declName do
    Lean.Lynth.Grind.throwNotMarkedWithGrindAttribute declName
  return { params with extensions := params.extensions.modify 0 fun ext => { ext with casesTypes := ext.casesTypes.erase declName } }

def _root_.Lean.Lynth.Grind.Params.insertFunCC (params : Lean.Lynth.Grind.Params) (declName : Name) : Lean.Lynth.Grind.Params :=
  { params with extensions := params.extensions.modify 0 fun ext => { ext with funCC := ext.funCC.insert declName } }

def _root_.Lean.Lynth.Grind.Params.containsEMatch (params : Lean.Lynth.Grind.Params) (declName : Name) : Bool :=
  params.extensions.any fun ext => ext.ematch.contains (.decl declName)

def _root_.Lean.Lynth.Grind.Params.isInjectiveTheorem (params : Lean.Lynth.Grind.Params) (declName : Name) : Bool :=
  params.extensions.any fun ext => ext.inj.contains (.decl declName)

def _root_.Lean.Lynth.Grind.Params.eraseEMatchCore (params : Lean.Lynth.Grind.Params) (declName : Name) : Lean.Lynth.Grind.Params :=
  { params with extensions := params.extensions.modify 0 fun ext => { ext with ematch := ext.ematch.erase (.decl declName) } }

def _root_.Lean.Lynth.Grind.Params.eraseEMatch (params : Lean.Lynth.Grind.Params) (declName : Name) : MetaM Lean.Lynth.Grind.Params := do
...
def _root_.Lean.Lynth.Grind.Params.eraseInj (params : Lean.Lynth.Grind.Params) (declName : Name) : Lean.Lynth.Grind.Params :=
  { params with extensions := params.extensions.modify 0 fun ext => { ext with inj := ext.inj.erase (.decl declName) } }

def _root_.Lean.Lynth.Grind.ExtensionStateArray.getKindsFor (s : Lean.Lynth.Grind.ExtensionStateArray) (origin : Lean.Lynth.Grind.Origin) : List Lean.Lynth.Grind.EMatchTheoremKind := Id.run do
  let mut result := []
  for ext in s do
    let s : Lean.Lynth.Grind.EMatchTheorems := ext.ematch
    let ks := s.getKindsFor origin
    unless ks.isEmpty do
      result := result ++ ks
  return result

public def _root_.Lean.Lynth.Grind.ExtensionStateArray.find (s : Lean.Lynth.Grind.ExtensionStateArray) (origin : Lean.Lynth.Grind.Origin) : List Lean.Lynth.Grind.EMatchTheorem := Id.run do
  let mut r := []
...
def warnRedundantEMatchArg (s : Lean.Lynth.Grind.ExtensionStateArray) (declName : Name) : MetaM Unit := do
  let minIndexable := false -- TODO: infer it
  let kinds ← match s.getKindsFor (.decl declName) with
    | [] => return ()
    | [k] => pure m!"@{k.toAttribute minIndexable}"
    | [.eqLhs gen, .eqRhs _]
    | [.eqRhs gen, .eqLhs _] => pure m!"@{(Lean.Lynth.Grind.EMatchTheoremKind.eqBoth gen).toAttribute minIndexable}"
    | ks =>
...
def parseModifier (s : String) : CoreM Lean.Lynth.Grind.AttrKind := do
...
def ensureNoMinIndexable (minIndexable : Bool) : MetaM Unit := do
  if minIndexable then
    throwError "redundant modifier `!` in `lynth_grind` parameter"

public def addEMatchTheorem (params : Lean.Lynth.Grind.Params) (id : Ident) (declName : Name)
    (kind : Lean.Lynth.Grind.EMatchTheoremKind)
    (minIndexable : Bool) (suggest : Bool := false) (warn := true) : MetaM Lean.Lynth.Grind.Params := do
  ...
  | .thm | .axiom | .ctor =>
    ...
    | .eqBoth gen =>
      ...
      let thm₁ ← Lean.Lynth.Grind.mkEMatchTheoremForDecl declName (.eqLhs gen) params.symPrios
      let thm₂ ← Lean.Lynth.Grind.mkEMatchTheoremForDecl declName (.eqRhs gen) params.symPrios
      if warn &&
    ...
    | _ =>
      if kind matches .eqLhs _ | .eqRhs _ then
        ensureNoMinIndexable minIndexable
      let thm ← if suggest && !Lean.Lynth.Grind.backward.grind.inferPattern.get (← getOptions) then
        Lean.Lynth.Grind.mkEMatchTheoremAndSuggest id declName params.symPrios minIndexable (isParam := true)
      else
        Lean.Lynth.Grind.mkEMatchTheoremForDecl declName kind params.symPrios (minIndexable := minIndexable)
  ...
  | .defn =>
    if (← isReducible declName) then
      throwError "`{.ofConstName declName}` is a reducible definition, `lynth_grind` automatically unfolds them"
    if !kind.isEqLhs && !kind.isDefault then
      throwError "invalid `lynth_grind` parameter, `{.ofConstName declName}` is a definition, the only acceptable (and redundant) modifier is '='"
    ensureNoMinIndexable minIndexable
    let some thms ← Lean.Lynth.Grind.mkEMatchEqTheoremsForDef? declName
  ...
  | _ =>
    throwError "invalid `lynth_grind` parameter, `{.ofConstName declName}` is not a theorem, definition, or inductive type"

def processAnchor (params : Lean.Lynth.Grind.Params) (val : TSyntax `hexnum) : CoreM Lean.Lynth.Grind.Params := do
  let anchorRefs := params.anchorRefs?.getD #[]
  let anchorRef ← Lean.Lynth.Grind.Elab.elabAnchorRef val
  return { params with anchorRefs? := some <| anchorRefs.push anchorRef }

def checkNoRevert (params : Lean.Lynth.Grind.Params) : CoreM Unit := do
  if params.config.revert then
    throwError "invalid `lynth_grind` parameter, only global declarations are allowed when `+revert` is used"

def processTermParam (params : Lean.Lynth.Grind.Params)
    (p : TSyntax `Lean.Parser.Tactic.grindParam)
    (mod? : Option (TSyntax `Lean.Parser.Attr.grindMod))
    (term : Term)
    (minIndexable : Bool)
    : TermElabM Lean.Lynth.Grind.Params := withRef p do
  checkNoRevert params
  let kind ← if let some mod := mod? then Lean.Lynth.Grind.getAttrKindCore mod else pure .infer
  let kind ← match kind with
    | .ematch .user | .cases _ | .intro | .inj | .ext | .symbol _ | .funCC | .norm .. | .unfold | .homo | .homoPred =>
      throwError "invalid `lynth_grind` parameter, only global declarations are allowed with this kind of modifier"
  ...
  unless (← isProp type) do
    throwError "invalid `lynth_grind` parameter, proof term expected"
  if type.isForall then
    let mkThm (kind : Lean.Lynth.Grind.EMatchTheoremKind) (idx : Nat) : MetaM Lean.Lynth.Grind.EMatchTheorem := do
      let id := ((`extra).appendIndexAfter idx)
      let some thm ← Lean.Lynth.Grind.mkEMatchTheoremWithKind? (.stx id p) levelParams proof kind params.symPrios (minIndexable := minIndexable)
        | throwError "invalid `lynth_grind` parameter, failed to infer patterns"
      return thm
    ...
    unless mod?.isNone do
      throwError "invalid `lynth_grind` parameter, modifier is redundant since the parameter type is not a `forall`{indentExpr type}"
    unless levelParams.isEmpty do
      throwError "invalid `lynth_grind` parameter, parameter type is not a `forall` and is universe polymorphic{indentExpr type}"
    return { params with extraFacts := params.extraFacts.push proof }

def processParam (params : Lean.Lynth.Grind.Params)
    (p : TSyntax `Lean.Parser.Tactic.grindParam)
    (mod? : Option (TSyntax `Lean.Parser.Attr.grindMod))
    (id : TSyntax `ident)
    (minIndexable : Bool)
  ...
  let declName ← try
    ...
    if (← resolveLocalName id.getId).isSome then
      throwErrorAt id "redundant parameter `{id}`, `lynth_grind` uses local hypotheses automatically"
    else if let some ext ← Lean.Lynth.Grind.getExtension? id.getId then
      if let some mod := mod? then
        throwErrorAt mod "invalid use of modifier in `lynth_grind` attribute `{id.getId}`"
  ...
  | .ematch .user =>
    unless only do
      withRef p <| Lean.Lynth.Grind.throwInvalidUsrModifier
    ensureNoMinIndexable minIndexable
    -- **Note**: This check is hard-coded to the default `lynth_grind` attribute. Possible improvement: `usr` modifier that specifies the attribute where
    -- the user pattern is coming from.
    let thms := (← Lean.Lynth.Grind.lynthGrindExt.getEMatchTheorems).find (.decl declName)
  ...
  | .cases eager =>
    ...
    ensureNoMinIndexable minIndexable
    withRef p <| Lean.Lynth.Grind.validateCasesAttr declName eager
    params := params.insertCasesTypes declName eager
  | .intro =>
    if let some info ← Lean.Lynth.Grind.isCasesAttrPredicateCandidate? declName false then
      if incremental then throwError "`cases` parameter are not supported here"
  ...
  | .inj =>
    let thm ← Lean.Lynth.Grind.mkInjectiveTheorem declName
    params := { params with extraInj := params.extraInj.push thm }
  | .ext =>
    throwError "`[grind ext]` cannot be set using parameters"
  | .infer =>
    if let some declName ← Lean.Lynth.Grind.isCasesAttrCandidate? declName false then
  ...
  | .norm .. => throwError "normalization theorems should be registered using the `@[lynth_grind norm]` attribute"
  | .homo => throwError "homomorphism rules should be registered using the `@[lynth_grind hom]` attribute"
  | .homoPred => throwError "homomorphism predicates should be registered using the `@[lynth_grind hom_pred]` attribute"
  | .unfold => throwError "declarations to be unfolded during normalization should be registered using the `@[lynth_grind unfold]` attribute"
  return params

/--
Elaborates `lynth_grind` parameters.
`incremental = true` for tactics such as `finish`, in this case, we disable some kinds of parameters
such as `- ident`.
-/
public def elabGrindParams (params : Lean.Lynth.Grind.Params) (ps : TSyntaxArray ``Parser.Tactic.grindParam)
    (only : Bool) (lax : Bool := false) (incremental := false) : TermElabM Lean.Lynth.Grind.Params := do
  let mut params := params
  for p in ps do
    try
      match p with
      | `(Parser.Tactic.grindParam| - $id:ident) =>
        if incremental then
          throwErrorAt p "invalid `-` occurrence, it can only used at the `lynth_grind` tactic entry point"
        let declName ← realizeGlobalConstNoOverloadWithInfo id
        Term.checkDeprecatedCore declName
        if let some declName ← Lean.Lynth.Grind.isCasesAttrCandidate? declName false then
          Lean.Lynth.Grind.ensureNotBuiltinCases declName
      ...
      | _ => throwError "unexpected `lynth_grind` parameter{indentD p}"
    catch ex =>
...
-/
public def withParams (params : Lean.Lynth.Grind.Params) (ps : TSyntaxArray ``Parser.Tactic.grindParam) (only : Bool)
  ...
  if !only && ps.isEmpty then
    ...
    liftGoalM do
      Lean.Lynth.Grind.assertExtra params
