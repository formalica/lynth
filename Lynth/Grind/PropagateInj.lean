...
def getInvFor? (f : Expr) (a : Expr) : GoalM (Option (Expr × Expr)) := do
  ...
  let inv := mkApp5 (mkConst ``Lean.Grind.leftInv info.us) info.α info.β f info.h nonEmpty
  let inv ← preprocessLight inv
  let args := inv.getAppArgs
  let heq := mkAppN (mkConst ``Lean.Grind.leftInv_eq info.us) args
...
lynth_builtin_grind_propagator propagateInj ↓Function.Injective := fun e => do
