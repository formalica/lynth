...
/--
`lynth_grind` always case-splits on the following types. Even when using `lynth_grind only`.
The goal is to reduce noise in the tactic generated by `lynth_grind?`
-/
...
even `lynth_grind only` case splits on.
...
def ensureNotBuiltinCases (declName : Name) : CoreM Unit := do
  if isBuiltinEagerCases declName then
    throwError "`{.ofConstName declName}` is marked as a built-in case-split for `lynth_grind` and cannot be erased"
...
/--
The `lynth_grind` tactic includes an auxiliary `cases` tactic that is not intended for direct use by users.
...
- It does not introduce new local declarations for each minor premise. Instead, the `lynth_grind` tactic preprocessor is responsible for introducing them.

- If the major premise type is an indexed family, auxiliary declarations and (heterogeneous) equalities are introduced.
  However, these equalities are not resolved using `unifyEqs`. Instead, the `lynth_grind` tactic employs union-find and
  congruence closure to process these auxiliary equalities. This approach avoids applying substitution to propositions
  that have already been internalized by `lynth_grind`.
-/
def cases (mvarId : MVarId) (e : Expr) : MetaM (List MVarId) := mvarId.withContext do
  ...
  let k (mvarId : MVarId) (fvarId : FVarId) (indices : Array FVarId) : MetaM (List MVarId) := do
    ...
    let recursor ← mkRecursorAppPrefix mvarId `lynth_grind.cases fvarId recursorInfo indicesExpr
    let lctx ← getLCtx
    ...
    for _ in *...recursorInfo.numMinors do
      let .forallE _ targetNew recursorTypeNew _ ← whnf recursorType
        | throwTacticEx `lynth_grind.cases mvarId "unexpected recursor type"
...
where
  throwInductiveExpected {α} (type : Expr) : MetaM α := do
    throwTacticEx `lynth_grind.cases mvarId m!"(non-recursive) inductive type expected at {e}{indentExpr type}"
