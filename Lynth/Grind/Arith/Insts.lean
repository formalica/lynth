...
module
prelude
public import Lynth.Grind.Arith.EvalNum
...
def getIsCharInst? (u : Level) (type : Expr) (semiringInst : Expr) : GoalM (Option (Expr × Nat)) := do withNewMCtxDepth do
  ...
  let charType := mkApp3 (mkConst ``Lean.Grind.IsCharP [u]) type semiringInst n
...
def getPowIdentityInst? (u : Level) (type : Expr) : GoalM (Option (Expr × Expr × Nat)) := do withNewMCtxDepth do
  ...
  let csInst ← mkFreshExprMVar (mkApp (mkConst ``Lean.Grind.CommSemiring [u]) type)
  let p ← mkFreshExprMVar (mkConst ``Nat)
  let powIdentityType := mkApp3 (mkConst ``Lean.Grind.PowIdentity [u]) type csInst p
...
def getNoZeroDivInst? (u : Level) (type : Expr) : GoalM (Option Expr) := do
  ...
  let noZeroDivType := mkApp2 (mkConst ``Lean.Grind.NoNatZeroDivisors [u]) type natModuleInst
