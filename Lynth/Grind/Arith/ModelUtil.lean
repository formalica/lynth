...
def isInterpretedTerm (e : Expr) : Bool :=
  ...
  || e.isAppOf ``Inv.inv || e.isAppOf ``NatCast.natCast || e.isIte || e.isDIte || e.isAppOf ``OfNat.ofNat || e.isAppOf ``Lean.Grind.ToInt.toInt
  || e.isAppOf ``Fin.val || e.isAppOf ``Lean.Grind.IntModule.OfNatModule.toQ || e matches .lit (.natVal _)
