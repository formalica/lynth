...
import Lynth.Grind.Arith.CommRing.NonCommRingM
import Lynth.Grind.Arith.CommRing.NonCommSemiringM
...
```
@[lynth_grind] def mask := 15

@[lynth_grind =] theorem foo (x : Nat) : x &&& 15 = x % 16 := by sorry
...
lynth_builtin_grind_propagator propagateNatAnd ↑HAnd.hAnd := propagateNatBinOp ``HAnd.hAnd ``Lean.Lynth.Grind.Nat.and_congr (· &&& ·)
lynth_builtin_grind_propagator propagateNatOr ↑HOr.hOr := propagateNatBinOp ``HOr.hOr ``Lean.Lynth.Grind.Nat.or_congr (· ||| ·)
lynth_builtin_grind_propagator propagateNatXOr ↑HXor.hXor := propagateNatBinOp ``HXor.hXor ``Lean.Lynth.Grind.Nat.xor_congr (· ^^^ ·)
lynth_builtin_grind_propagator propagateNatShiftLeft ↑HShiftLeft.hShiftLeft :=
  propagateNatBinOp ``HShiftLeft.hShiftLeft ``Lean.Lynth.Grind.Nat.shiftLeft_congr (· <<< ·)
lynth_builtin_grind_propagator propagateNatShiftRight ↑HShiftRight.hShiftRight :=
  propagateNatBinOp ``HShiftRight.hShiftRight ``Lean.Lynth.Grind.Nat.shiftRight_congr (· >>> ·)
...
that does not have good support in `lynth_grind ring`. That is, `lynth_grind ring`
...
`lynth_grind ring`. We need this propagator because have normalization rules for them, and users
were surprised when using `lynth_grind [zero_mul]` did not have any effect. In this scenario,
`lynth_grind` was correctly instantiating `zero_mul : 0*a = 0`, but the normalizer reduces the
instance to `True`.

Alternative approach: We improve the support for equality reasoning for non-commutative rings
and semirings in `lynth_grind ring`. For example, we could just replace equalities and keep renormalizing.
If we implement this feature, this propagator can be deleted.
-/
lynth_builtin_grind_propagator propagateMul ↑HMul.hMul := fun e => do
  ...
  if let some n ← isOfNat? aRoot then
    if n == 0 then
      pushEq e aRoot <| mkApp5 (mkConst ``Lean.Grind.Semiring.zero_mul_congr [u]) α₁ semiringInst a b (← mkEqProof a aRoot)
    else if n == 1 then
      pushEq e b <| mkApp5 (mkConst ``Lean.Grind.Semiring.one_mul_congr [u]) α₁ semiringInst a b (← mkEqProof a aRoot)
  else if let some n ← isOfNat? bRoot then
    if n == 0 then
      pushEq e bRoot <| mkApp5 (mkConst ``Lean.Grind.Semiring.mul_zero_congr [u]) α₁ semiringInst a b (← mkEqProof b bRoot)
    else if n == 1 then
      pushEq e a <| mkApp5 (mkConst ``Lean.Grind.Semiring.mul_one_congr [u]) α₁ semiringInst a b (← mkEqProof b bRoot)
