...
import Lynth.Grind.SynthInstance
...
def run? (type : Expr) (x : M α) : MetaM (Option α) := do
  ...
  let some fieldInst ← synthInstanceMeta? (mkApp (mkConst ``Lean.Grind.Field [u]) type) | return none
  let commRingInst := mkApp2 (mkConst ``Lean.Grind.Field.toCommRing [u]) type fieldInst
  let ringInst := mkApp2 (mkConst ``Lean.Grind.CommRing.toRing [u]) type commRingInst
  let semiringInst := mkApp2 (mkConst ``Lean.Grind.Ring.toSemiring [u]) type ringInst
  let isCharP := mkApp3 (mkConst ``Lean.Grind.IsCharP [u]) type semiringInst (mkNatLit 0)
...
def isAddInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``instHAdd [ctx.u]) ctx.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toAdd [ctx.u]) ctx.type ctx.semiringInst
  isDefEqI inst expectedInst

def isMulInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``instHMul [ctx.u]) ctx.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toMul [ctx.u]) ctx.type ctx.semiringInst
  isDefEqI inst expectedInst

def isSubInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``instHSub [ctx.u]) ctx.type <| mkApp2 (mkConst ``Lean.Grind.Ring.toSub [ctx.u]) ctx.type ctx.ringInst
  isDefEqI inst expectedInst

def isDivInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``instHDiv [ctx.u]) ctx.type <| mkApp2 (mkConst ``Lean.Grind.Field.toDiv [ctx.u]) ctx.type ctx.fieldInst
  isDefEqI inst expectedInst

def isNegInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Ring.toNeg [ctx.u]) ctx.type ctx.ringInst
  isDefEqI inst expectedInst

def isInvInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Field.toInv [ctx.u]) ctx.type ctx.fieldInst
  isDefEqI inst expectedInst

def isNPowInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Semiring.npow [ctx.u]) ctx.type ctx.semiringInst
  isDefEqI inst expectedInst

def isZPowInst (inst : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Field.zpow [ctx.u]) ctx.type ctx.fieldInst
  isDefEqI inst expectedInst

def isOfNatInst (inst : Expr) (n : Expr) : M Bool := do
  let ctx ← read
  let expectedInst := mkApp3 (mkConst ``Lean.Grind.Semiring.ofNat [ctx.u]) ctx.type ctx.semiringInst n
  isDefEqI inst expectedInst

def mkNatCastInst : M Expr := do
  let ctx ← read
  return mkApp2 (mkConst ``Lean.Grind.Semiring.natCast [ctx.u]) ctx.type ctx.semiringInst
...
def mkIntCastInst : M Expr := do
  let ctx ← read
  return mkApp2 (mkConst ``Lean.Grind.Ring.intCast [ctx.u]) ctx.type ctx.ringInst
...
partial def eval (e : Expr) : M (Rat × Expr) := do
  ...
  | HAdd.hAdd _ _ _ inst a b =>
    ...
    mkBin ``Lean.Grind.Field.NormNum.add_eq a b (← eval a) (← eval b) (· + ·)
  | HMul.hMul _ _ _ inst a b =>
    guard (← isMulInst inst)
    mkBin ``Lean.Grind.Field.NormNum.mul_eq a b (← eval a) (← eval b) (· * ·)
  | HSub.hSub _ _ _ inst a b =>
    guard (← isSubInst inst)
    mkBin ``Lean.Grind.Field.NormNum.sub_eq a b (← eval a) (← eval b) (· - ·)
  | HDiv.hDiv _ _ _ inst a b =>
    guard (← isDivInst inst)
    mkBin ``Lean.Grind.Field.NormNum.div_eq a b (← eval a) (← eval b) (· / ·)
  | Neg.neg _ inst a =>
    guard (← isNegInst inst)
    mkUnary ``Lean.Grind.Field.NormNum.neg_eq a (← eval a) (- ·)
  | Inv.inv _ inst a =>
    guard (← isInvInst inst)
    mkUnary ``Lean.Grind.Field.NormNum.inv_eq a (← eval a) (·⁻¹)
  | OfNat.ofNat _ n inst =>
    guard (← isOfNatInst inst n)
    let some v ← getNatValue? n | failure
    let ctx ← read
    let h := mkApp3 (mkConst ``Lean.Grind.Field.NormNum.ofNat_eq [ctx.u]) ctx.type ctx.fieldInst (toExpr v)
    return ((v : Rat), h)
  | NatCast.natCast _ inst n =>
    guard (← isNatCastInst inst)
    let some v ← getNatValue? n | failure
    let ctx ← read
    let h := mkApp3 (mkConst ``Lean.Grind.Field.NormNum.natCast_eq [ctx.u]) ctx.type ctx.fieldInst (toExpr v)
    return ((v : Rat), h)
  | IntCast.intCast _ inst n =>
    guard (← isIntCastInst inst)
    let some v ← getIntValue? n | failure
    let ctx ← read
    let h := mkApp3 (mkConst ``Lean.Grind.Field.NormNum.intCast_eq [ctx.u]) ctx.type ctx.fieldInst (toExpr v)
    return ((v : Rat), h)
  | HPow.hPow _ _ _ inst a b =>
    if (← isNPowInst inst) then
      ...
      let ctx ← read
      -- **Note**: Would be great to be able to use `Lean.Grind.Config.exp`, but we don't have access to it in the `MetaM` monad
      guard (← checkExponent n (warning := false))
      let v := v₁ ^ n
      let h := mkApp9 (mkConst ``Lean.Grind.Field.NormNum.npow_eq [ctx.u]) ctx.type ctx.fieldInst ctx.isChar0Inst
      ...
      let v := v₁ ^ n
      let h := mkApp9 (mkConst ``Lean.Grind.Field.NormNum.zpow_eq [ctx.u]) ctx.type ctx.fieldInst ctx.isChar0Inst
        a (toExpr n) (toExpr v₁) (toExpr v) eagerReflBoolTrue h₁
      return (v, h)
    else
      failure
...
public def normFieldExpr? (e : Expr) (type : Expr) : MetaM (Option (Expr × Expr)) := do
  ...
  run? type do
    ...
    if v.den == 1 then
      ...
      let h := mkApp7 (mkConst ``Lean.Grind.Field.NormNum.eq_int [ctx.u]) ctx.type ctx.fieldInst e (toExpr v) (toExpr v.num) eagerReflBoolTrue h
      return (r, h)
    else if v.num == 1 then
      let r ← mkNatCast v.den
      let r ← mkAppM ``Inv.inv #[r]
      let h := mkApp7 (mkConst ``Lean.Grind.Field.NormNum.eq_inv [ctx.u]) ctx.type ctx.fieldInst e (toExpr v) (toExpr v.den) eagerReflBoolTrue h
      return (r, h)
    else
      ...
      let r ← mkMul n d
      let h := mkApp8 (mkConst ``Lean.Grind.Field.NormNum.eq_mul_inv [ctx.u]) ctx.type ctx.fieldInst e (toExpr v) (toExpr v.num) (toExpr v.den) eagerReflBoolTrue h
      return (r, h)
