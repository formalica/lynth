...
/-- Explicit `RingM` helpers over `Lean.Grind.CommRing` types.
...
with identical bodies, operating on our `RingM` state. -/
@[inline] def toPolyM? (e : RingExpr) : RingM (Option Poly) := do
  toPoly e

@[inline] def mulConstM (p : Poly) (k : Int) : RingM Poly :=
  mulConst k p

@[inline] def mulMonM (p : Poly) (k : Int) (m : Mon) : RingM Poly :=
  mulMon k m p

@[inline] def mulM (p₁ p₂ : Poly) : RingM Poly := do
  mul p₁ p₂

@[inline] def combineM (p₁ p₂ : Poly) : RingM Poly :=
  combine p₁ p₂

def spolM (p₁ p₂ : Poly) : RingM Grind.CommRing.SPolResult := do
  match p₁, p₂ with
  | .add k₁ m₁ p₁, .add k₂ m₂ p₂ =>
    let m    := m₁.lcm m₂
    let m₁   := m.div m₁
...
def findInvNumeralVarMon? (m : Mon) : RingM (Option (Nat × Var)) := do
  match m with
  | .unit => return none
  | .mult pw m =>
    let e := (← getRing).vars[pw.x]!
    let_expr Inv.inv _ _ a := e | findInvNumeralVarMon? m
    let_expr OfNat.ofNat _ n _ := a | findInvNumeralVarMon? m
    let some n ← getNatValue? n | findInvNumeralVarMon? m
    return some (n, pw.x)

/-- Returns `some (val, x)` if `p` contains a variable `x` whose the denotation is `val⁻¹`. -/
def findInvNumeralVarPoly? (p : Poly) : RingM (Option (Nat × Var)) := do
  match p with
  | .num _ => return none
  | .add _ m p =>
    let some r ← findInvNumeralVarMon? m | findInvNumeralVarPoly? p
    return some r

def simpMPoly? (p₁ p₂ : Poly) : RingM (Option SimpResult) := do
  match p₂ with
  | .add k₂' m₂ p₂ =>
    ...
    let rec go? (p₁ : Poly) : RingM (Option SimpResult) := do
      ...
      | .add k₁' m₁ p₁ =>
        if m₂.divides m₁ && (!checkCoeff || k₂' ∣ k₁') then
          ...
          let k₁ := k₂'/g
          let k₂ := -k₁'/g
          let p  ← combineM (← mulMonM p₂ k₂ m₂) (← mulConstM p₁ k₁)
          return some { p, k₁, k₂, m₂ }
