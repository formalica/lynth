...
def mkPowFn (u : Level) (type : Expr) (semiringInst : Expr) : m Expr := do
  ...
  let inst' := mkApp2 (mkConst ``Lean.Grind.Semiring.npow [u]) type semiringInst
...
def mkNatCastFn (u : Level) (type : Expr) (semiringInst : Expr) : m Expr := do
  let inst' := mkApp2 (mkConst ``Lean.Grind.Semiring.natCast [u]) type semiringInst
...
def getAddFn : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``instHAdd [ring.u]) ring.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toAdd [ring.u]) ring.type ring.semiringInst
...
def getSubFn : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``instHSub [ring.u]) ring.type <| mkApp2 (mkConst ``Lean.Grind.Ring.toSub [ring.u]) ring.type ring.ringInst
...
def getMulFn : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``instHMul [ring.u]) ring.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toMul [ring.u]) ring.type ring.semiringInst
...
def getNegFn : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Ring.toNeg [ring.u]) ring.type ring.ringInst
...
def getIntCastFn : m Expr := do
  ...
  let inst' := mkApp2 (mkConst ``Lean.Grind.Ring.intCast [ring.u]) ring.type ring.ringInst
...
private def mkOne (u : Level) (type : Expr) (semiringInst : Expr) : m Expr := do
  let n := mkRawNatLit 1
  let ofNatInst := mkApp3 (mkConst ``Lean.Grind.Semiring.ofNat [u]) type semiringInst n
...
def getInvFn : m Expr := do
  ...
  let some fieldInst := ring.fieldInst?
    | throwError "`lynth_grind` internal error, type is not a field{indentExpr ring.type}"
  if let some invFn := ring.invFn? then return invFn
  let expectedInst := mkApp2 (mkConst ``Lean.Grind.Field.toInv [ring.u]) ring.type fieldInst
