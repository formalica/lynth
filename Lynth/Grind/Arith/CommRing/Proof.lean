...
private def throwNoNatZeroDivisors : RingM α := do
  throwError "`lynth_grind` internal error, `NoNatZeroDivisors` instance is needed, but it is not available for{indentExpr (← getRing).type}"

private def getPolyConst (p : Poly) : RingM Int := do
  let .num k := p
    | throwError "`lynth_grind` internal error, constant polynomial expected {indentExpr (← p.denoteExpr)}"
  return k
...
private abbrev getSContext : ProofM Expr := do
  let some sctx := (← read).sctx?
    | throwError "`lynth_grind` internal error, semiring context is not available"
  return sctx
...
private def getSemiringIdOf : RingM Nat := do
  let some semiringId := (← getCommRing).semiringId? | throwError "`lynth_grind` internal error, semiring is not available"
  return semiringId
...
private def mkSemiringAddRightCancelPrefix (declName : Name) : ProofM Expr := do
  ...
  let some addRightCancelInst ← SemiringM.run (← getSemiringIdOf) do getAddRightCancelInst?
    | throwError "`lynth_grind` internal error, `AddRightCancel` instance is not available"
  return mkApp4 (mkConst declName [semiring.u]) semiring.type semiring.semiringInst addRightCancelInst sctx

open Lean.Grind.CommRing in
partial def _root_.Lean.Lynth.Grind.Arith.CommRing.EqCnstr.toExprProof (c : EqCnstr) : ProofM Expr := caching c do
  match c.h with
  | .core a b lhs rhs =>
  ...
  | .coreS a b sa sb ra rb =>
    let h' ← mkSemiringPrefix ``Lean.Grind.Ring.OfSemiring.of_eq
  ...
  | .gcd a b c₁ c₂ =>
    let h ← mkStepBasicPrefix ``Lean.Grind.CommRing.eq_gcd
  ...
  | .numEq0 k c₁ c₂ =>
    let h ← mkStepBasicPrefix ``Lean.Grind.CommRing.eq_normEq0
...
private def derivToExprProof (d : PolyDerivation) : ProofM (Int × Poly × Expr) := do
  ...
  | .normEq0 p d c =>
    ...
    let h ← mkStepBasicPrefix ``Lean.Grind.CommRing.d_normEq0
    let h := mkApp9 h
...
private def mkContext (h : Expr) : ProofM Expr := do
  ...
  let h := mkLetOfMap (← get).polyDecls h `p (mkConst ``Lean.Grind.CommRing.Poly) fun p => toExpr <| p.renameVars varRename
  let h := mkLetOfMap (← get).monDecls h `m (mkConst ``Lean.Grind.CommRing.Mon) fun m => toExpr <| m.renameVars varRename
  let h := mkLetOfMap (← get).exprDecls h `e (mkConst ``Lean.Grind.CommRing.Expr) fun e => toExpr <| e.renameVars varRename
...
private def mkSemiringContext (h : Expr) : ProofM Expr := do
  ...
  let h := mkLetOfMap (← get).sexprDecls h `s (mkConst ``Lean.Grind.CommRing.Expr) fun s => toExpr <| s.renameVars varRename
...
def EqCnstr.setUnsat  (c : EqCnstr) : RingM Unit := do
  let h ← withProofContext do
    ...
    if let some (charInst, char) := ring.charInst? then
      ...
      return mkApp6 (mkConst ``Lean.Grind.CommRing.one_eq_zero_unsat [ring.u]) ring.type fieldInst (← getContext)
        (← mkPolyDecl c.p) eagerReflBoolTrue (← c.toExprProof)
    else
      throwError "`lynth_grind ring` internal error, unexpected unsat eq proof {← c.denoteExpr}"
  closeGoal h
...
def DiseqCnstr.setUnsat (c : DiseqCnstr) : RingM Unit := do
  let h ← withProofContext do
    ...
    let hne ← if let some (sa, sb) := c.ofSemiring? then
      let h ← mkSemiringAddRightCancelPrefix ``Lean.Grind.Ring.OfSemiring.of_diseq
...
def setSemiringDiseqUnsat (a b : Expr) (sa sb : SemiringExpr) : SemiringM Unit := do
  ...
  let h := mkApp3 (mkConst ``Lean.Grind.CommRing.eq_normS [semiring.u]) semiring.type semiring.commSemiringInst ctx
...
def setNonCommRingDiseqUnsat (a b : Expr) (ra rb : RingExpr) : NonCommRingM Unit := do
  ...
  let h := if let some (charInst, c) := ring.charInst? then
    mkApp5 (mkConst ``Lean.Grind.CommRing.Expr.eq_of_toPolyC_nc_eq [ring.u]) ring.type (toExpr c) ring.ringInst charInst ctx
  else
    mkApp3 (mkConst ``Lean.Grind.CommRing.Expr.eq_of_toPoly_nc_eq [ring.u]) ring.type ring.ringInst ctx
...
def setNonCommSemiringDiseqUnsat (a b : Expr) (sa sb : SemiringExpr) : NonCommSemiringM Unit := do
  ...
  let sb           := sb.renameVars varRename
  let ctx          ← toSContextExpr' vars
  let h := mkApp3 (mkConst ``Lean.Grind.CommRing.eq_normS_nc [semiring.u]) semiring.type semiring.semiringInst ctx
  let h := mkApp3 h (toExpr sa) (toExpr sb) eagerReflBoolTrue
  closeGoal (mkApp hne h)
...
def mkLeIffProof (leInst ltInst isPreorderInst orderedRingInst : Expr) (lhs rhs lhs' rhs' : RingExpr) : RingM Expr := do
  ...
  let ctx ← toContextExpr vars
  let h := mkApp6 (mkConst ``Lean.Grind.CommRing.le_norm_expr [ring.u]) ring.type ring.commRingInst leInst ltInst isPreorderInst orderedRingInst
...
def mkLtIffProof (leInst ltInst lawfulOrdLtInst isPreorderInst orderedRingInst : Expr) (lhs rhs lhs' rhs' : RingExpr) : RingM Expr := do
  ...
  let { lhs, rhs, lhs', rhs', vars } := norm ring.vars lhs rhs lhs' rhs'
  let ctx ← toContextExpr vars
  let h := mkApp7 (mkConst ``Lean.Grind.CommRing.lt_norm_expr [ring.u]) ring.type ring.commRingInst leInst ltInst lawfulOrdLtInst isPreorderInst orderedRingInst
  let h := mkApp6 h ctx (toExpr lhs) (toExpr rhs) (toExpr lhs') (toExpr rhs') eagerReflBoolTrue
...
def mkEqIffProof (lhs rhs lhs' rhs' : RingExpr) : RingM Expr := do
  ...
  let { lhs, rhs, lhs', rhs', vars } := norm ring.vars lhs rhs lhs' rhs'
  let ctx ← toContextExpr vars
  let h := mkApp2 (mkConst ``Lean.Grind.CommRing.eq_norm_expr [ring.u]) ring.type ring.commRingInst
  let h := mkApp6 h ctx (toExpr lhs) (toExpr rhs) (toExpr lhs') (toExpr rhs') eagerReflBoolTrue
...
def mkTermEqProof (e e' : RingExpr) : RingM Expr := do
  ...
  let ctx ← toContextExpr vars
  let h := mkApp2 (mkConst ``Lean.Grind.CommRing.Expr.eq_of_toPoly_eq [ring.u]) ring.type ring.commRingInst
...
def mkNonCommLeIffProof (leInst ltInst isPreorderInst orderedRingInst : Expr) (lhs rhs lhs' rhs' : RingExpr) : NonCommRingM Expr := do
  ...
  let { lhs, rhs, lhs', rhs', vars } := norm ring.vars lhs rhs lhs' rhs'
  let ctx ← toContextExpr vars
  let h := mkApp6 (mkConst ``Lean.Grind.CommRing.le_norm_expr_nc [ring.u]) ring.type ring.ringInst leInst ltInst isPreorderInst orderedRingInst
  let h := mkApp6 h ctx (toExpr lhs) (toExpr rhs) (toExpr lhs') (toExpr rhs') eagerReflBoolTrue
  let leFn := mkApp2 (mkConst ``LE.le [ring.u]) ring.type leInst
  let le   := mkApp2 leFn (← lhs.denoteExpr' vars) (← rhs.denoteExpr' vars)
  let le'  := mkApp2 leFn (← lhs'.denoteExpr' vars) (← rhs'.denoteExpr' vars)
...
def mkNonCommLtIffProof (leInst ltInst lawfulOrdLtInst isPreorderInst orderedRingInst : Expr) (lhs rhs lhs' rhs' : RingExpr) : NonCommRingM Expr := do
  ...
  let { lhs, rhs, lhs', rhs', vars } := norm ring.vars lhs rhs lhs' rhs'
  let ctx ← toContextExpr vars
  let h := mkApp7 (mkConst ``Lean.Grind.CommRing.lt_norm_expr_nc [ring.u]) ring.type ring.ringInst leInst ltInst lawfulOrdLtInst isPreorderInst orderedRingInst
  let h := mkApp6 h ctx (toExpr lhs) (toExpr rhs) (toExpr lhs') (toExpr rhs') eagerReflBoolTrue
  let ltFn := mkApp2 (mkConst ``LT.lt [ring.u]) ring.type ltInst
  let lt   := mkApp2 ltFn (← lhs.denoteExpr' vars) (← rhs.denoteExpr' vars)
  let lt'  := mkApp2 ltFn (← lhs'.denoteExpr' vars) (← rhs'.denoteExpr' vars)
...
def mkNonCommEqIffProof (lhs rhs lhs' rhs' : RingExpr) : NonCommRingM Expr := do
  ...
  let { lhs, rhs, lhs', rhs', vars } := norm ring.vars lhs rhs lhs' rhs'
  let ctx ← toContextExpr vars
  let h := mkApp2 (mkConst ``Lean.Grind.CommRing.eq_norm_expr_nc [ring.u]) ring.type ring.ringInst
  let h := mkApp6 h ctx (toExpr lhs) (toExpr rhs) (toExpr lhs') (toExpr rhs') eagerReflBoolTrue
  let eqFn := mkApp (mkConst ``Eq [Level.succ ring.u]) ring.type
  let eq   := mkApp2 eqFn (← lhs.denoteExpr' vars) (← rhs.denoteExpr' vars)
  let eq'  := mkApp2 eqFn (← lhs'.denoteExpr' vars) (← rhs'.denoteExpr' vars)
...
def mkNonCommTermEqProof (e e' : RingExpr) : NonCommRingM Expr := do
  ...
  let { lhs, lhs', vars, .. } := norm ring.vars e (.num 0) e' (.num 0)
  let ctx ← toContextExpr vars
  let h := mkApp2 (mkConst ``Lean.Grind.CommRing.Expr.eq_of_toPoly_nc_eq [ring.u]) ring.type ring.ringInst
  let h := mkApp4 h ctx (toExpr lhs) (toExpr lhs') eagerReflBoolTrue
  let eqFn := mkApp (mkConst ``Eq [Level.succ ring.u]) ring.type
  let expected := mkApp2 eqFn (← lhs.denoteExpr' vars) (← lhs'.denoteExpr' vars)
  return mkExpectedPropHint h expected
