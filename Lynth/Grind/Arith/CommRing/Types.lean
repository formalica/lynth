...
-/
abbrev SemiringExpr := Lean.Grind.CommRing.Expr
...
/-- State for all `CommRing` types detected by `lynth_grind`. -/
...
initialize ringExt : SolverExtension State ← registerSolverExtension (return {})
