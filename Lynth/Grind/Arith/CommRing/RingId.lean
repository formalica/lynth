...
where
  go? : GoalM (Option Nat) := do
    if type.isAppOfArity ``Lean.Grind.Ring.OfSemiring.Q 2 then
      goQ? type.appFn!.appArg! type.appArg!
    else
      goCore?

  /-
  Fast path for `lynth_grind`'s internal envelope type `Ring.OfSemiring.Q base`.
  -/
  goQ? (base semiringInst : Expr) : GoalM (Option Nat) := do
    ...
    let_expr Lean.Grind.CommSemiring.toSemiring _ commSemiringInst := semiringInst | goCore?
    let some u ← getDecLevel? base | return none
    let commRingInst := mkApp2 (mkConst ``Lean.Grind.CommRing.OfCommSemiring.ofCommSemiring [u]) base commSemiringInst
    let ringInst := mkApp2 (mkConst ``Lean.Grind.CommRing.toRing [u]) type commRingInst
    let semiringInstQ := mkApp2 (mkConst ``Lean.Grind.Ring.toSemiring [u]) type ringInst
    let commSemiringInstQ := mkApp2 (mkConst ``Lean.Grind.CommRing.toCommSemiring [u]) type semiringInstQ
    registerInstance (mkApp (mkConst ``Lean.Grind.CommRing [u]) type) commRingInst
    registerInstance (mkApp (mkConst ``Lean.Grind.Ring [u]) type) ringInst
    registerInstance (mkApp (mkConst ``Lean.Grind.Semiring [u]) type) semiringInstQ
    registerInstance (mkApp (mkConst ``Lean.Grind.CommSemiring [u]) type) commSemiringInstQ
    registerInstance (mkApp (mkConst ``Lean.Grind.NatModule [u]) type)
      (mkApp2 (mkConst ``Lean.Grind.Semiring.toNatModule [u]) type semiringInstQ)
    registerInstance (mkApp3 (mkConst ``HAdd [u, u, u]) type type type)
      (mkApp2 (mkConst ``instHAdd [u]) type (mkApp2 (mkConst ``Lean.Grind.Semiring.toAdd [u]) type semiringInstQ))
    registerInstance (mkApp3 (mkConst ``HMul [u, u, u]) type type type)
      (mkApp2 (mkConst ``instHMul [u]) type (mkApp2 (mkConst ``Lean.Grind.Semiring.toMul [u]) type semiringInstQ))
    registerInstance (mkApp3 (mkConst ``HSub [u, u, u]) type type type)
      (mkApp2 (mkConst ``instHSub [u]) type (mkApp2 (mkConst ``Lean.Grind.Ring.toSub [u]) type ringInst))
    registerInstance (mkApp (mkConst ``Neg [u]) type)
      (mkApp2 (mkConst ``Lean.Grind.Ring.toNeg [u]) type ringInst)
    registerInstance (mkApp3 (mkConst ``HPow [u, 0, u]) type Nat.mkType type)
      (mkApp2 (mkConst ``Lean.Grind.Semiring.npow [u]) type semiringInstQ)
    registerInstance (mkApp (mkConst ``NatCast [u]) type)
      (mkApp2 (mkConst ``Lean.Grind.Semiring.natCast [u]) type semiringInstQ)
    registerInstance (mkApp (mkConst ``IntCast [u]) type)
      (mkApp2 (mkConst ``Lean.Grind.Ring.intCast [u]) type ringInst)
    ...
    let addRightCancelInst? ← do
      ...
      synthInstance? (mkApp2 (mkConst ``Lean.Grind.AddRightCancel [u]) base addInst)
    let charInst? ← do
      ...
      let inst := mkApp5 (mkConst ``Lean.Grind.Ring.OfSemiring.instIsCharPQOfAddRightCancel [u])
    ...
    let noZeroDivInst? ← do
      ...
      pure (some (mkApp4 (mkConst ``Lean.Grind.Ring.OfSemiring.instNoNatZeroDivisorsQOfAddRightCancel [u])
  ...
  goCore? : GoalM (Option Nat) := do
    ...
    let commRing := mkApp (mkConst ``Lean.Grind.CommRing [u]) type
    ...
    let commSemiringInst := mkApp2 (mkConst ``Lean.Grind.CommRing.toCommSemiring [u]) type semiringInst
    trace_goal[grind.ring] "new ring: {type}"
    let charInst? ← getIsCharInst? u type semiringInst
    let noZeroDivInst? ← getNoZeroDivInst? u type
    trace_goal[grind.ring] "NoNatZeroDivisors available: {noZeroDivInst?.isSome}"
    let fieldInst? ← synthInstance? <| mkApp (mkConst ``Lean.Grind.Field [u]) type
...
where
  go? : GoalM (Option Nat) := do
    let u ← getDecLevel type
    let ring := mkApp (mkConst ``Lean.Grind.Ring [u]) type
...
where
  go? : GoalM (Option Nat) := do
    let u ← getDecLevel type
    let commSemiring := mkApp (mkConst ``Lean.Grind.CommSemiring [u]) type
    let some commSemiringInst ← synthInstance? commSemiring | return none
    let semiringInst := mkApp2 (mkConst ``Lean.Grind.CommSemiring.toSemiring [u]) type commSemiringInst
    let q ← shareCommon (← canon (mkApp2 (mkConst ``Lean.Grind.Ring.OfSemiring.Q [u]) type semiringInst))
    let some ringId ← getCommRingId? q
      | throwError "`lynth_grind` unexpected failure, failure to initialize ring{indentExpr q}"
...
where
  go? : GoalM (Option Nat) := do
    let u ← getDecLevel type
    let semiring := mkApp (mkConst ``Lean.Grind.Semiring [u]) type
