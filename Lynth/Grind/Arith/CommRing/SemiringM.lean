...
instance : MonadCanon SemiringM where
  ...
  synthInstance? e := Lean.Lynth.Grind.synthInstance? e
...
protected def SemiringM.getCommSemiring : SemiringM CommSemiring := do
  ...
  if h : semiringId < s.semirings.size then
    ...
    throwError "`lynth_grind` internal error, invalid semiringId"
...
protected def SemiringM.getCommRing : SemiringM CommRing := do
  ...
  if h : ringId < s.rings.size then
    ...
    throwError "`lynth_grind` internal error, invalid ringId"
...
def getToQFn : SemiringM Expr := do
  ...
  let toQFn ← canonExpr <| mkApp2 (mkConst ``Lean.Grind.Ring.OfSemiring.toQ [s.u]) s.type s.semiringInst
...
private def mkAddRightCancelInst? (u : Level) (type : Expr) : GoalM (Option Expr) := do
  ...
  let addRightCancel := mkApp2 (mkConst ``Lean.Grind.AddRightCancel [u]) type addInst
...
def getAddFn' : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``instHAdd [s.u]) s.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toAdd [s.u]) s.type s.semiringInst
...
def getMulFn' : m Expr := do
  ...
  let expectedInst := mkApp2 (mkConst ``instHMul [s.u]) s.type <| mkApp2 (mkConst ``Lean.Grind.Semiring.toMul [s.u]) s.type s.semiringInst
