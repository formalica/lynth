...
/-
**Note**: It is safe to use (the more efficient) structural instances tests here because `lynth_grind` uses the canonicalizer.
-/
open Structural in
lynth_builtin_grind_propagator propagatePower ↑HPow.hPow := fun e => do
  ...
  traverseEqc b fun bENode => do
    ...
    | HAdd.hAdd n₁ n₂ n₃ inst b₁ b₂ =>
      ...
      let some h ← mkSemiringThm ``Lean.Grind.Semiring.pow_add_congr α | return ()
