  ...
  if let some k ← toInt? a then
    if k == 0 then
      ...
      **Remark:** We have a normalization rule for `0⁻¹ = 0`, but we may still encounter `0⁻¹` for one of the following reasons:
      ...
      -/
      pushEq e a <| mkApp2 (mkConst ``Lean.Grind.Field.inv_zero [ring.u]) ring.type fieldInst
      return ()
    if (← hasChar) then
      ...
      if c == 0 then
        ...
        pushNewFact <| mkExpectedPropHint
          (mkApp5 (mkConst ``Lean.Grind.CommRing.inv_int_eq [ring.u]) ring.type fieldInst charInst (mkIntLit k) eagerReflBoolTrue)
          expected
      else if k % c == 0 then
        let expected ← mkEq e (← denoteNum 0)
        pushNewFact <| mkExpectedPropHint
          (mkApp6 (mkConst ``Lean.Grind.CommRing.inv_zero_eqC [ring.u]) ring.type (mkNatLit c) fieldInst charInst (mkIntLit k) eagerReflBoolTrue)
          expected
      else
        let expected ← mkEq (mkApp2 (← getMulFn) a e) (← denoteNum 1)
        pushNewFact <| mkExpectedPropHint
          (mkApp6 (mkConst ``Lean.Grind.CommRing.inv_int_eqC [ring.u]) ring.type (mkNatLit c) fieldInst charInst (mkIntLit k) eagerReflBoolTrue)
          expected
      return ()
  pushNewFact <| mkApp3 (mkConst ``Lean.Grind.CommRing.inv_split [ring.u]) ring.type fieldInst a
...
private def processPowIdentityVars : RingM Unit := do
  ...
  for i in [startIdx:vars.size] do
    ...
    let proof := mkApp5 (mkConst ``Lean.Grind.PowIdentity.pow_eq [ring.u])
