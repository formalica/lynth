...
module
prelude
public import Lynth.Grind.Arith.Util
import Init.Grind.Module.OfNatModule
import Lean.Data.RArray
import Lynth.Grind.Arith.Linear.ToExpr
import Lynth.Grind.Diseq
import Lynth.Grind.ProofUtil
import Lean.Meta.Sym.Arith.VarRename
import Lean.Meta.Sym.Arith.ToExpr
import Lynth.Grind.Arith.Linear.VarRename
...
private def mkContext (h : Expr) : ProofM Expr := do
  ...
  let h := mkLetOfMap exprDecls h `e (mkConst ``Lean.Grind.Linarith.Expr) fun e => toExpr <| e.renameVars varRename
  let h := mkLetOfMap polyDecls h `p (mkConst ``Lean.Grind.Linarith.Poly) fun p => toExpr <| p.renameVars varRename
...
private def mkRingContext (h : Expr) : ProofM Expr := do
  ...
  let varRename    := mkVarRename vars'
  let vars         := vars'.map fun x => vars[x]!
  let h := mkLetOfMap (← get).ringExprDecls h `re (mkConst ``Lean.Grind.CommRing.Expr) fun e => toExpr <| e.renameVars varRename
  let h := mkLetOfMap (← get).ringPolyDecls h `rp (mkConst ``Lean.Grind.CommRing.Poly) fun p => toExpr <| p.renameVars varRename
...
partial def RingIneqCnstr.toExprProof (c' : RingIneqCnstr) : ProofM Expr := do
  match c'.h with
  | .core e lhs rhs =>
    let h' ← if c'.strict then mkCommRingLawfulPreOrdThmPrefix ``Lean.Grind.CommRing.lt_norm else mkCommRingPreOrdThmPrefix ``Lean.Grind.CommRing.le_norm
  ...
  | .notCore e lhs rhs =>
    let h' ← mkCommRingLinOrdThmPrefix (if c'.strict then ``Lean.Grind.CommRing.not_le_norm else ``Lean.Grind.CommRing.not_lt_norm)
  ...
  | .cancelDen c val x n =>
    let h ← if c.strict then
      mkCommRingLawfulPreOrdThmPrefix ``Lean.Grind.CommRing.lt_mul
    else
      mkCommRingPreOrdThmPrefix ``Lean.Grind.CommRing.le_mul
    ...
    let h_eq_one := mkApp2 (← mkFieldChar0ThmPrefix ``Lean.Grind.CommRing.inv_int_eq') (toExpr val) eagerReflBoolTrue
    let h' ← if c.strict then
      mkCommRingLTThmPrefix ``Lean.Grind.CommRing.lt_cancel_var
    else
      mkCommRingLEThmPrefix ``Lean.Grind.CommRing.le_cancel_var
...
partial def RingEqCnstr.toExprProof (c' : RingEqCnstr) : ProofM Expr := do
  match c'.h with
  | .core a b la lb =>
    let h' ← mkCommRingThmPrefix ``Lean.Grind.CommRing.eq_norm
    return mkApp5 h' (← mkRingExprDecl la) (← mkRingExprDecl lb) (← mkRingPolyDecl c'.p) eagerReflBoolTrue (← mkEqProof a b)
  | .symm c =>
    let h ← c.toExprProof
    let h' ← mkCommRingThmPrefix ``Lean.Grind.CommRing.Stepwise.inv
    return mkApp4 h' (← mkRingPolyDecl c.p) (← mkRingPolyDecl c'.p) eagerReflBoolTrue h
  | .cancelDen c val x n =>
    let h ← mkCommRingThmPrefix ``Lean.Grind.CommRing.Stepwise.eq_mul
    let p₁ := c.p.mulConst (val^n)
    let p₁ ← mkRingPolyDecl p₁
    let h := mkApp5 h (← mkRingPolyDecl c.p) (toExpr (val^n)) p₁ eagerReflBoolTrue (← c.toExprProof)
    let h_eq_one := mkApp2 (← mkFieldChar0ThmPrefix ``Lean.Grind.CommRing.inv_int_eq') (toExpr val) eagerReflBoolTrue
    let h' ← mkCommRingThmPrefix ``Lean.Grind.CommRing.eq_cancel_var
    return mkApp7 h' (toExpr val) (← mkRingVarDecl x) p₁ (← mkRingPolyDecl c'.p) eagerReflBoolTrue h_eq_one h

partial def RingDiseqCnstr.toExprProof (c' : RingDiseqCnstr) : ProofM Expr := do
  match c'.h with
  | .core a b lhs rhs =>
    let h' ← mkCommRingThmPrefix ``Lean.Grind.CommRing.diseq_norm
    return mkApp5 h' (← mkRingExprDecl lhs) (← mkRingExprDecl rhs) (← mkRingPolyDecl c'.p) eagerReflBoolTrue (← mkDiseqProof a b)
  | .cancelDen c val x n =>
    let h ← mkCommRingNoNatDivThmPrefix ``Lean.Grind.CommRing.Stepwise.diseq_mul
    let p₁ := c.p.mulConst (val^n)
    let p₁ ← mkRingPolyDecl p₁
    let h := mkApp5 h (← mkRingPolyDecl c.p) (toExpr (val^n)) p₁ eagerReflBoolTrue (← c.toExprProof)
    let h_eq_one := mkApp2 (← mkFieldChar0ThmPrefix ``Lean.Grind.CommRing.inv_int_eq') (toExpr val) eagerReflBoolTrue
    let h' ← mkCommRingThmPrefix ``Lean.Grind.CommRing.diseq_cancel_var
    return mkApp7 h' (toExpr val) (← mkRingVarDecl x) p₁ (← mkRingPolyDecl c'.p) eagerReflBoolTrue h_eq_one h
...
partial def IneqCnstr.toExprProof (c' : IneqCnstr) : ProofM Expr := caching c' do
  match c'.h with
  | .core e lhs rhs =>
    let h ← if c'.strict then mkIntModLawfulPreOrdThmPrefix ``Lean.Grind.Linarith.lt_norm else mkIntModPreOrdThmPrefix ``Lean.Grind.Linarith.le_norm
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl rhs) (← mkPolyDecl c'.p) eagerReflBoolTrue (mkOfEqTrueCore e (← mkEqTrueProof e))
  | .notCore e lhs rhs =>
    let h ← mkIntModLinOrdThmPrefix (if c'.strict then ``Lean.Grind.Linarith.not_le_norm else ``Lean.Grind.Linarith.not_lt_norm)
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl rhs) (← mkPolyDecl c'.p) eagerReflBoolTrue (mkOfEqFalseCore e (← mkEqFalseProof e))
  | .ring c lhs' =>
    let h ← c.toExprProof
    let h' ← if c'.strict then mkCommRingLTThmPrefix ``Lean.Grind.CommRing.lt_int_module else mkCommRingLEThmPrefix ``Lean.Grind.CommRing.le_int_module
  ...
  | .coreOfNat e natStructId lhs rhs =>
    let h' ← OfNatModuleM.run natStructId do
      ...
      let h := if c'.strict then
        mkApp7 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_lt [ns.u]) ns.type ns.natModuleInst ns.leInst?.get! ns.ltInst?.get!
          ns.lawfulOrderLTInst?.get! ns.isPreorderInst?.get! ns.orderedAddInst?.get!
      else
        mkApp5 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_le [ns.u]) ns.type ns.natModuleInst ns.leInst?.get!
  ...
  | .notCoreOfNat e natStructId lhs rhs =>
    let h' ← OfNatModuleM.run natStructId do
      ...
      let (b', hb) ← ofNatModule b
      let h := if c'.strict then
        mkApp5 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_not_le [ns.u]) ns.type ns.natModuleInst ns.leInst?.get!
          ns.isPreorderInst?.get! ns.orderedAddInst?.get!
      else
        mkApp7 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_not_lt [ns.u]) ns.type ns.natModuleInst ns.leInst?.get! ns.ltInst?.get!
          ns.lawfulOrderLTInst?.get! ns.isPreorderInst?.get! ns.orderedAddInst?.get!
  ...
  | .combine c₁ c₂ =>
    let (pre, c₁, c₂) :=
      ...
      | true, true => (mkIntModLawfulPreOrdThmPrefix ``Lean.Grind.Linarith.lt_lt_combine, c₁, c₂)
      | true, false => (mkIntModLawfulPreOrdThmPrefix ``Lean.Grind.Linarith.le_lt_combine, c₂, c₁)
      | false, true => (mkIntModLawfulPreOrdThmPrefix ``Lean.Grind.Linarith.le_lt_combine, c₁, c₂)
      | false, false => (mkIntModPreOrdThmPrefix ``Lean.Grind.Linarith.le_le_combine, c₁, c₂)
    return mkApp6 (← pre) (← mkPolyDecl c₁.p) (← mkPolyDecl c₂.p) (← mkPolyDecl c'.p) eagerReflBoolTrue
      (← c₁.toExprProof) (← c₂.toExprProof)
  | .oneGtZero =>
    let s ← getStruct
    let h := mkApp8 (mkConst ``Lean.Grind.Linarith.zero_lt_one [s.u]) s.type (← getRingInst) (← getLEInst) (← getLTInst) (← getLawfulOrderLTInst) (← getIsPreorderInst) (← getOrderedRingInst) (← getContext)
    return mkApp3 h (← mkPolyDecl c'.p) eagerReflBoolTrue (← mkEqRefl (← getOne))
  | .ofEq a b la lb =>
    let h ← mkIntModPreOrdThmPrefix ``Lean.Grind.Linarith.le_of_eq
  ...
  | .ofEqOfNat a b natStructId la lb =>
    let h' ← OfNatModuleM.run natStructId do
      ...
      let (a', ha) ← ofNatModule a
      let (b', hb) ← ofNatModule b
      return mkApp9 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_eq [ns.u]) ns.type ns.natModuleInst
        a b a' b' ha hb (← mkEqProof a b)
    let h ← mkIntModPreOrdThmPrefix ``Lean.Grind.Linarith.le_of_eq
    return mkApp5 h (← mkExprDecl la) (← mkExprDecl lb) (← mkPolyDecl c'.p) eagerReflBoolTrue h'
  | .ringEq c lhs =>
    let h' ← c.toExprProof
    let h' := mkApp2 (← mkCommRingThmPrefix ``Lean.Grind.CommRing.eq_int_module) (← mkRingPolyDecl c.p) h'
    let h ← mkIntModPreOrdThmPrefix ``Lean.Grind.Linarith.le_of_eq
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl .zero) (← mkPolyDecl c'.p) eagerReflBoolTrue h'
  ...
  | .ofDiseqSplit c₁ fvarId h _ =>
    ...
    let h ← mkIntModLinOrdThmPrefix ``Lean.Grind.Linarith.diseq_split_resolve
...
partial def DiseqCnstr.toExprProof (c' : DiseqCnstr) : ProofM Expr := caching c' do
  match c'.h with
  | .core a b lhs rhs =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.diseq_norm
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl rhs) (← mkPolyDecl c'.p) eagerReflBoolTrue (← mkDiseqProof a b)
  | .ring c lhs =>
    let h' ← c.toExprProof
    let h' := mkApp2 (← mkCommRingThmPrefix ``Lean.Grind.CommRing.diseq_int_module) (← mkRingPolyDecl c.p) h'
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.diseq_norm
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl .zero) (← mkPolyDecl c'.p) eagerReflBoolTrue h'
  | .coreOfNat a b natStructId lhs rhs =>
    let h ← OfNatModuleM.run natStructId do
      ...
      let (a', ha) ← ofNatModule a
      let (b', hb) ← ofNatModule b
      return mkApp10 (mkConst ``Lean.Grind.IntModule.OfNatModule.of_diseq [ns.u]) ns.type ns.natModuleInst ns.addRightCancelInst?.get!
        a b a' b' ha hb (← mkDiseqProof a b)
    return mkApp5 (← mkIntModThmPrefix ``Lean.Grind.Linarith.diseq_norm)
      (← mkExprDecl lhs) (← mkExprDecl rhs) (← mkPolyDecl c'.p) eagerReflBoolTrue h
  | .neg c =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.diseq_neg
    return mkApp4 h (← mkPolyDecl c.p) (← mkPolyDecl c'.p) eagerReflBoolTrue (← c.toExprProof)
  | .subst k₁ k₂ c₁ c₂ =>
    let h ← mkIntModNoNatDivThmPrefix ``Lean.Grind.Linarith.eq_diseq_subst
  ...
  | .subst1 k c₁ c₂ =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.eq_diseq_subst1
...
partial def EqCnstr.toExprProof (c' : EqCnstr) : ProofM Expr := caching c' do
  match c'.h with
  | .core a b lhs rhs =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.eq_norm
  ...
  | .coreOfNat a b natStructId lhs rhs =>
    ...
    return mkApp5 h (← mkExprDecl lhs) (← mkExprDecl rhs) (← mkPolyDecl c'.p) eagerReflBoolTrue h'
  | .neg c =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.eq_neg
    return mkApp4 h (← mkPolyDecl c.p) (← mkPolyDecl c'.p) eagerReflBoolTrue (← c.toExprProof)
  | .coeff k c =>
    let h ← mkIntModNoNatDivThmPrefix ``Lean.Grind.Linarith.eq_coeff
    return mkApp5 h (← mkPolyDecl c.p) (← mkPolyDecl c'.p) (toExpr k) eagerReflBoolTrue (← c.toExprProof)
  | .subst x c₁ c₂ =>
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.eq_eq_subst
    return mkApp7 h (← mkVarDecl x) (← mkPolyDecl c₁.p) (← mkPolyDecl c₂.p) (← mkPolyDecl c'.p) eagerReflBoolTrue
      (← c₁.toExprProof) (← c₂.toExprProof)

partial def UnsatProof.toExprProofCore (h : UnsatProof) : ProofM Expr := do
  match h with
  | .lt c => return mkApp (← mkIntModPreThmPrefix ``Lean.Grind.Linarith.lt_unsat) (← c.toExprProof)
  | .diseq c => return mkApp (← mkIntModThmPrefix ``Lean.Grind.Linarith.diseq_unsat) (← c.toExprProof)
...
def propagateImpEq (c : EqCnstr) : LinearM Unit := do
  ...
  let h ← withProofContext do
    let h ← mkIntModThmPrefix ``Lean.Grind.Linarith.imp_eq
