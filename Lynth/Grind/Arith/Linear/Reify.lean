...
def reportInstIssue (e : Expr) : GoalM Unit := do
  reportIssue! "`lynth_grind linarith` term with unexpected instance{indentExpr e}"

private def assertNatCastNonneg (a : Expr) : LinearM Unit := do
  let s ← getStruct
  let h := mkApp8 (mkConst ``Lean.Grind.OrderedRing.natCast_nonneg [s.u]) s.type s.ringInst?.get!
    s.leInst?.get! s.ltInst?.get! s.lawfulOrderLTInst?.get! s.isPreorderInst?.get! s.orderedRingInst?.get! a
  addNewRawFact h (← inferType h) (← getGeneration a) (.ematch (.decl ``Lean.Grind.OrderedRing.natCast_nonneg)) .other
