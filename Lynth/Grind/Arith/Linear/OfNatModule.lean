...
def getNatStruct : OfNatModuleM NatStruct := do
  ...
  if h : natStructId < s.natStructs.size then
    ...
    throwError "`lynth_grind` internal error, invalid natStructId"
...
private partial def ofNatModule' (e : Expr) : OfNatModuleM (Expr × Expr) := do
  ...
  | HAdd.hAdd _ _ _ i a b =>
    if isAddInst ns i then
      ...
      let h := mkApp8 (mkConst ``Lean.Grind.IntModule.OfNatModule.add_congr [ns.u]) ns.type ns.natModuleInst a b a' b' ha hb
      pure (e', h)
    else
      mkOfNatModuleVar e
  | HSMul.hSMul _ _ _ i a b =>
    if isSMulInst ns i then
      ...
      let h := mkApp6 (mkConst ``Lean.Grind.IntModule.OfNatModule.smul_congr [ns.u]) ns.type ns.natModuleInst a b b' hb
      pure (e', h)
    else
      mkOfNatModuleVar e
  | Zero.zero _ i =>
    if isZeroInst ns i then
      let e' := s.zero
      let h := mkApp2 (mkConst ``Lean.Grind.IntModule.OfNatModule.toQ_zero [ns.u]) ns.type ns.natModuleInst
      pure (e', h)
    else
      mkOfNatModuleVar e
  | OfNat.ofNat _ _ _ =>
...
private def reifyVar (e : Expr) : ReifyM Lean.Grind.Linarith.Expr := do
...
private partial def reify (e : Expr) : ReifyM Lean.Grind.Linarith.Expr := do
  let ns ← getNatStruct
  match_expr e with
  | HAdd.hAdd _ _ _ i a b =>
    if isAddInst ns i then
...
def normNatModuleDiseq (a b : Expr) : OfNatModuleM Unit := ReifyM.run do
  ...
  if pa == pb then
    ...
    let heq := mkApp6 (mkConst ``Lean.Grind.Linarith.eq_normN [ns.u]) ns.type ns.natModuleInst ctx (toExpr ea) (toExpr eb) eagerReflBoolTrue
