...
import Lynth.Grind.Arith.Linear.Proof
...
def IneqCnstr.throwUnexpected (c : IneqCnstr) : LinearM α := do
  throwError "`lynth_grind linarith` internal error, unexpected{indentD (← c.denoteExpr)}"
...
private def checkIsNextVar (x : Var) : LinearM Unit := do
  if x != (← getStruct).assignment.size then
    throwError "`lynth_grind linarith` internal error, assigning variable out of order"
...
private def findCase (decVars : FVarIdSet) : SearchM Case := do
  repeat
    ...
    unless numCases > 0 do throwError "`lynth_grind linarith` internal cases, cases stack is empty"
