...
deriving instance Hashable for Lean.Grind.Linarith.Expr
...
inductive RingIneqCnstrProof where
  | core (e : Expr) (lhs rhs : Lean.Grind.CommRing.Expr)
  | notCore (e : Expr) (lhs rhs : Lean.Grind.CommRing.Expr)
...
inductive RingEqCnstrProof where
  | core (a b : Expr) (ra rb : Lean.Grind.CommRing.Expr)
  | symm (c : RingEqCnstr)
...
inductive EqCnstrProof where
  ...
  | coreCommRing (a b : Expr) (ra rb : Lean.Grind.CommRing.Expr) (p : Lean.Grind.CommRing.Poly) (lhs' : LinExpr)
...
structure Struct where
  ...
  current `lynth_grind` goal or not.
  -/
  ...
  can convert `UnsatProof` into a Lean term and close the current `lynth_grind` goal.
  -/
...
/-- State for all `IntModule` types detected by `lynth_grind`. -/
...
initialize linearExt : SolverExtension State ← registerSolverExtension (return {})
