...
import Lynth.Grind.OrderInsts
import Lynth.Grind.Arith.Cutsat.ToInt
import Lynth.Grind.Arith.CommRing.RingId
import Lynth.Grind.Arith.Linear.Var
import Lynth.Grind.Arith.Insts
...
private def mkRingInst? (u : Level) (type : Expr) (commRingInst? : Option Expr) : GoalM (Option Expr) := do
  if let some commRingInst := commRingInst? then
    return mkApp2 (mkConst ``Lean.Grind.CommRing.toRing [u]) type commRingInst
  else
    synthInstance? <| mkApp (mkConst ``Lean.Grind.Ring [u]) type
...
private def mkIntModuleInst? (u : Level) (type : Expr) (ringInst? : Option Expr) : GoalM (Option Expr) := do
  if let some ringInst := ringInst? then
    return some <| mkApp2 (mkConst ``Lean.Grind.Ring.toIntModule [u]) type ringInst
  else
    synthInstance? <| mkApp (mkConst ``Lean.Grind.IntModule [u]) type

private def mkSemiringInst? (u : Level) (type : Expr) (ringInst? : Option Expr) : GoalM (Option Expr) := do
  if let some ringInst := ringInst? then
    return some <| mkApp2 (mkConst ``Lean.Grind.Ring.toSemiring [u]) type ringInst
  else
    synthInstance? <| mkApp (mkConst ``Lean.Grind.Semiring [u]) type
...
private def mkOrderedRingInst? (u : Level) (type : Expr) (semiringInst? leInst? ltInst? preorderInst? : Option Expr) : GoalM (Option Expr) := do
  ...
  let isOrdType := mkApp5 (mkConst ``Lean.Grind.OrderedRing [u]) type semiringInst leInst ltInst preorderInst
...
private def mkNoNatZeroDivInst? (u : Level) (type : Expr) : GoalM (Option Expr) := do
  let natModuleType := mkApp (mkConst ``Lean.Grind.NatModule [u]) type
  let some natModuleInst ← synthInstance? natModuleType | return none
  synthInstance? <| mkApp2 (mkConst ``Lean.Grind.NoNatZeroDivisors [u]) type natModuleInst
...
where
  go? : GoalM (Option Nat) := do
    if type.isAppOfArity ``Lean.Grind.IntModule.OfNatModule.Q 2 then
      goQ? type.appFn!.appArg! type.appArg!
    else
      goCore?
  /-
  Fast path for `lynth_grind`'s internal envelope type `IntModule.OfNatModule.Q base`.
  ...
  goQ? (base natModuleInst : Expr) : GoalM (Option Nat) := do
    ...
    let orderedAddInst? ← match leInst?, isPreorderInst? with
      | some leInst, some isPreorderInst =>
        ...
        synthInstance? <| mkApp4 (mkConst ``Lean.Grind.OrderedAdd [u]) base addInst leInst isPreorderInst
      | _, _ => pure none
    ...
    let leInstQ? := ordPremises?.map fun (le, pre, ord) =>
      mkQOrderInst ``Lean.Grind.IntModule.OfNatModule.instLEQOfOrderedAdd le pre ord
    let ltInstQ? := ordPremises?.map fun (le, pre, ord) =>
      mkQOrderInst ``Lean.Grind.IntModule.OfNatModule.instLTQOfOrderedAdd le pre ord
    let isPreorderInstQ? := ordPremises?.map fun (le, pre, ord) =>
      mkQOrderInst ``Lean.Grind.IntModule.OfNatModule.instIsPreorderQ le pre ord
    let orderedAddInstQ? := ordPremises?.map fun (le, pre, ord) =>
      mkQOrderInst ``Lean.Grind.IntModule.OfNatModule.instOrderedAddQ le pre ord
    ...
    let isLinearInstQ? ← match ordPremises? with
      | some (le, _, ord) =>
        ...
        pure <| isLinearInst?.map fun lin =>
          mkQOrderInst ``Lean.Grind.IntModule.OfNatModule.instIsLinearOrderQ le lin ord
      | none => pure none
    let noNatDivInstQ? ← do
      ...
      let some addRightCancelInst ← synthInstance? (mkApp2 (mkConst ``Lean.Grind.AddRightCancel [u]) base addInst) | pure none
      let some noNatDivInst ← synthInstance? (mkApp2 (mkConst ``Lean.Grind.NoNatZeroDivisors [u]) base natModuleInst) | pure none
      pure <| some <| mkApp4 (mkConst ``Lean.Grind.IntModule.OfNatModule.instNoNatZeroDivisorsQOfAddRightCancel [u])
        base natModuleInst addRightCancelInst noNatDivInst
    let intModuleInst := mkApp2 (mkConst ``Lean.Grind.IntModule.OfNatModule.ofNatModule [u]) base natModuleInst
    let addCommGroupInst := mkApp2 (mkConst ``Lean.Grind.IntModule.toAddCommGroup [u]) type intModuleInst
    let addCommMonoidInst := mkApp2 (mkConst ``Lean.Grind.AddCommGroup.toAddCommMonoid [u]) type addCommGroupInst
    let zeroInst := mkApp2 (mkConst ``Lean.Grind.AddCommMonoid.toZero [u]) type addCommMonoidInst
    let ofNatZeroInst := mkApp2 (mkConst ``Zero.toOfNat0 [u]) type zeroInst
    let addInst := mkApp2 (mkConst ``instHAdd [u]) type (mkApp2 (mkConst ``Lean.Grind.AddCommMonoid.toAdd [u]) type addCommMonoidInst)
    let subInst := mkApp2 (mkConst ``instHSub [u]) type (mkApp2 (mkConst ``Lean.Grind.AddCommGroup.toSub [u]) type addCommGroupInst)
    let negInst := mkApp2 (mkConst ``Lean.Grind.AddCommGroup.toNeg [u]) type addCommGroupInst
    let zsmulInst := mkApp3 (mkConst ``instHSMul [0, u]) Int.mkType type (mkApp2 (mkConst ``Lean.Grind.IntModule.zsmul [u]) type intModuleInst)
    let nsmulInst := mkApp3 (mkConst ``instHSMul [0, u]) Nat.mkType type (mkApp2 (mkConst ``Lean.Grind.IntModule.nsmul [u]) type intModuleInst)
    ...
    registerInstance (mkApp (mkConst ``Lean.Grind.IntModule [u]) type) intModuleInst
  ...
  goCore? : GoalM (Option Nat) := do
    ...
    if (← getConfig).ring && ringId?.isSome && isPreorderInst?.isNone then
      /-
      If the type is a `Ring` **and** is not even a preorder **and** `lynth_grind ring` is enabled,
      we let `lynth_grind ring` process the equalities and disequalities. There is no
    ...
    let orderedAddInst? ← match leInst?, isPreorderInst? with
      | some leInst, some isPreorderInst =>
        synthInstance? <| mkApp4 (mkConst ``Lean.Grind.OrderedAdd [u]) type addInst leInst isPreorderInst
      | _, _ => pure none
    ...
    let fieldInst? ← getInst? ``Lean.Grind.Field u type
    ...
    let nsmulFn ← internalizeFn <| mkApp4 (mkConst ``HSMul.hSMul [0, u, u]) Nat.mkType type type nsmulInst
    ensureToFieldDefEq zeroInst addCommMonoidInst ``Lean.Grind.AddCommMonoid.toZero u type
    ensureToHomoFieldDefEq addInst addCommMonoidInst ``Lean.Grind.AddCommMonoid.toAdd ``instHAdd u type
    ensureToHomoFieldDefEq subInst addCommGroupInst ``Lean.Grind.AddCommGroup.toSub ``instHSub u type
    ensureToFieldDefEq negInst addCommGroupInst ``Lean.Grind.AddCommGroup.toNeg u type
    ensureToHomoFieldDefEq zsmulInst intModuleInst ``Lean.Grind.IntModule.zsmul ``instHSMul u type (some Int.mkType)
    ensureToHomoFieldDefEq nsmulInst intModuleInst ``Lean.Grind.IntModule.nsmul ``instHSMul u type (some Nat.mkType)
...
private def mkNatModuleInst? (u : Level) (type : Expr) : GoalM (Option Expr) := do
  synthInstance? <| mkApp (mkConst ``Lean.Grind.NatModule [u]) type
...
where
  go? : GoalM (Option Nat) := do
    ...
    let q ← shareCommon (← canon (mkApp2 (mkConst ``Lean.Grind.IntModule.OfNatModule.Q [u]) type natModuleInst))
    let some structId ← getStructId? q
      | throwError "`lynth_grind` unexpected failure, failure to initialize auxiliary `IntModule`{indentExpr q}"
    let leInst? ← getInst? ``LE u type
    let ltInst? ← getInst? ``LT u type
    let isPreorderInst? ← mkIsPreorderInst? u type leInst?
    let lawfulOrderLTInst? ← mkLawfulOrderLTInst? u type ltInst? leInst?
    ...
    let addRightCancelInst? ← synthInstance? <| mkApp2 (mkConst ``Lean.Grind.AddRightCancel [u]) type addInst'
    let toQFn ← internalizeFn <| mkApp2 (mkConst ``Lean.Grind.IntModule.OfNatModule.toQ [u]) type natModuleInst
    let zeroInst ← getInst ``Zero u type
    let zero ← internalizeConst <| mkApp2 (mkConst ``Zero.zero [u]) type zeroInst
