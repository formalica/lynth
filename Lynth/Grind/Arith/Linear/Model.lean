...
import Lynth.Grind.Arith.Linear.Reify
import Lynth.Grind.Arith.ModelUtil
...
private def toQ? (e : Expr) : Option Expr :=
  match_expr e with
  | Lean.Grind.IntModule.OfNatModule.toQ _ _ a => some a
  | _ => none
...
for communicating with the `lynth_grind` core.
...
in the `lynth_grind` core, the `linarith` module is notified that `a + b = 2*b` is true, and it is then
...
def mkModel (goal : Goal) (structId : Nat) : MetaM (Array (Expr × Rat)) := do
  ...
  traceModel `lynth_grind.linarith.model r
  return r
