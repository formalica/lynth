...
import Lynth.Grind.Arith.Cutsat.Norm
...
private def exprAsPoly (a : Expr) : GoalM Poly := do
  if let some k ← getIntValue? a then
    ...
    throwError "internal `lynth_grind` error, expression is not relevant to cutsat{indentExpr a}"
...
private def getKindAndType? (e : Expr) : GrindM (Option (SupportedTermKind × Expr)) :=
  ...
  | Lean.Grind.ToInt.toInt _ _ _ _ => return some (.toInt, Int.mkType)
...
private def internalizeInt (e : Expr) : GoalM Unit := do
  ...
  let p ← toPoly e
  trace[grind.debug.lia.internalize] "{aquote e}:= {← ppPoly p}"
  let x ← mkVar e
...
def internalize (e : Expr) (parent? : Option Expr) : GoalM Unit := do
  ...
  if let some (k, type) ← getKindAndType? e then
    ...
    `lynth_grind` would fail to solve
    ```
