...
import Lynth.Grind.Arith.EvalNum
open Lean.Meta
public section
...
private def throwMissingDecl (declName : Name) : MetaM Unit :=
  throwError "`lynth_grind cutsat`, unexpected missing declaration {.ofConstName declName}"
...
where
  toIntInterval? (rangeExpr : Expr) : GoalM (Option SymbolicIntInterval) := do
    ...
    | Lean.Grind.IntInterval.co lo hi =>
  ...
  go? : GoalM (Option Nat) := withNewMCtxDepth do
    ...
    let rangeExpr ← mkFreshExprMVar (mkConst ``Lean.Grind.IntInterval)
    let toIntType := mkApp2 (mkConst ``Lean.Grind.ToInt [u]) type rangeExpr
    ...
    let toInt := mkApp3 (mkConst ``Lean.Grind.ToInt.toInt [u]) type rangeExpr toIntInst
    let wrap := mkApp (mkConst ``Lean.Grind.IntInterval.wrap) rangeExpr
    let ofWrap0? ← if let .co lo hi := range then
      if lo.isZero then
        pure <| some <| mkApp3 (mkConst ``Lean.Grind.ToInt.of_eq_wrap_co_0) rangeExpr hi.val (← mkEqRefl rangeExpr)
      else pure none
      else pure none
    let ofEq := mkApp3 (mkConst ``Lean.Grind.ToInt.of_eq [u]) type rangeExpr toIntInst
    let ofDiseq := mkApp3 (mkConst ``Lean.Grind.ToInt.of_diseq [u]) type rangeExpr toIntInst
    let lowerThm? ← if let some lo := range.lo? then
      if let some lo' := lo.ival? then
        if lo' == 0 then
          pure <| some <| mkApp4 (mkConst ``Lean.Grind.ToInt.ge_lower0 [u]) type rangeExpr toIntInst eagerReflBoolTrue
        else
          pure <| some <| mkApp5 (mkConst ``Lean.Grind.ToInt.ge_lower [u]) type rangeExpr toIntInst lo.val eagerReflBoolTrue
      else
        -- Symbolic case
        let some_lo ← mkSome Int.mkType lo.val
        pure <| some <| mkApp5 (mkConst ``Lean.Grind.ToInt.ge_lower' [u]) type rangeExpr toIntInst lo.val (← mkEqRefl some_lo)
    else pure none
    let upperThm? ← if let some hi := range.hi? then
      if hi.isNumeral then
        pure <| some <| mkApp5 (mkConst ``Lean.Grind.ToInt.le_upper [u]) type rangeExpr toIntInst (← hi.mkIntNegSucc) eagerReflBoolTrue
      else
        -- Symbolic case
        let some_hi ← mkSome Int.mkType hi.val
        pure <| some <| mkApp5 (mkConst ``Lean.Grind.ToInt.le_upper' [u]) type rangeExpr toIntInst hi.val (← mkEqRefl some_hi)
    else pure none
...
private def mkOfLE : ToIntM (Option Expr × Option Expr) := do
  ...
  let toIntLE := mkApp4 (mkConst ``Lean.Grind.ToInt.LE [info.u]) info.type leInst info.rangeExpr info.toIntInst
  let some toIntLEInst ← synthInstance? toIntLE
    | reportMissingToIntAdapter info.type toIntLE; return (none, none)
  let ofLE := mkApp5 (mkConst ``Lean.Grind.ToInt.of_le [info.u]) info.type info.rangeExpr info.toIntInst leInst toIntLEInst
  let ofNotLE := mkApp5 (mkConst ``Lean.Grind.ToInt.of_not_le [info.u]) info.type info.rangeExpr info.toIntInst leInst toIntLEInst
...
private def mkOfLT : ToIntM (Option Expr × Option Expr) := do
  ...
  let toIntLT := mkApp4 (mkConst ``Lean.Grind.ToInt.LT [info.u]) info.type ltInst info.rangeExpr info.toIntInst
  let some toIntLTInst ← synthInstance? toIntLT
    | reportMissingToIntAdapter info.type toIntLT; return (none, none)
  let ofLT := mkApp5 (mkConst ``Lean.Grind.ToInt.of_lt [info.u]) info.type info.rangeExpr info.toIntInst ltInst toIntLTInst
  let ofNotLT := mkApp5 (mkConst ``Lean.Grind.ToInt.of_not_lt [info.u]) info.type info.rangeExpr info.toIntInst ltInst toIntLTInst
...
private def mkSimpleOpThmCore? (op : Expr) (opSuffix : Name) (thmName : Name) : ToIntM (Option Expr) := do
  ...
  let toIntOpName := ``Lean.Grind.ToInt ++ opSuffix
...
private def mkPowThm? : ToIntM (Option Expr) := do
  ...
  mkSimpleOpThmCore? op `Pow ``Lean.Grind.ToInt.pow_congr
...
def getZeroThm? : ToIntM (Option Expr) := do
  ...
  let thm? ← mkSimpleOpThm? ``Zero ``Lean.Grind.ToInt.zero_eq
...
private def mkOfNatThm? : ToIntM (Option Expr) := do
  ...
  let toIntOfNat := mkApp4 (mkConst ``Lean.Grind.ToInt.OfNat [info.u]) info.type ofNatInst info.rangeExpr info.toIntInst
  let some toIntOfNatInst ← synthInstance? toIntOfNat
    | reportMissingToIntAdapter info.type toIntOfNat; return none
  return mkApp5 (mkConst ``Lean.Grind.ToInt.ofNat_eq [info.u]) info.type info.rangeExpr info.toIntInst ofNatInst toIntOfNatInst
...
private def isWrap (e : Expr) : Option Expr :=
  match_expr e with
  | Lean.Grind.IntInterval.wrap _ a => some a
  | _ => none
  ...
  | .co lo hi =>
    if lo.isZero then
      ...
      if (← hasNumericLoHi) then
        ...
        internalize b' (← getGeneration b)
        return (b', h)
  | _ => throwError "`lynth_grind cutsat`, `ToInt` interval not supported yet"
...
def assertToIntBounds (e : Expr) (x : Var) : GoalM Unit := do
  let_expr Lean.Grind.ToInt.toInt α _ _ a := e | return ()
  ToIntM.run α do
  let info ← getInfo
  let i := info.range
