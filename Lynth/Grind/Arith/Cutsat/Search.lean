...
import Lynth.Grind.Arith.Cutsat.DvdCnstr
import Lynth.Grind.Arith.Cutsat.ReorderVars
import Lynth.Grind.Arith.Cutsat.Proof
...
private def checkIsNextVar (x : Var) : GoalM Unit := do
  if x != (← get').assignment.size then
    throwError "`lynth_grind` internal error, assigning variable out of order"
  ...
  go (xs : List Var) : GoalM Unit := do
    ...
    | x :: xs =>
      let some c := (← get').elimEqs[x]!
        | throwError "`lynth_grind` internal error, eliminated variable must have equation associated with it"
      ...
      let some v ← evalPoly? c.p | c.throwUnexpected
      let v := (-v) / a
      traceAssignment x v
...
def getBestLower? (x : Var) (dvd? : Option DvdCnstr) : GoalM (Option (Rat × LeCnstr)) := do
  ...
  for c in s.lowers[x]! do
    ...
    let some v ← evalPoly? p | c.throwUnexpected
    let lower' := v / (-k)
...
def DvdCnstr.getSolutions? (c : DvdCnstr) : SearchM (Option DvdSolution) := do
  ...
  let some b ← evalPoly? p | c.throwUnexpected
  if b.den != 1 then
...
def resolveRealLowerUpperConflict (c₁ c₂ : LeCnstr) : GoalM Bool := do
  ...
  if (← satisfiedLePoly p) != .false then
  ...
  | some (lower, c₁), some (upper, c₂) =>
    ...
    if lower > upper then
      let .true ← resolveRealLowerUpperConflict c₁ c₂
        | throwError "`lynth_grind` internal error, conflict resolution failed"
      return ()
    -- `lower ≤ upper` here
...
private def searchAssignmentMain : SearchM Unit := do
  repeat
    ...
    if (← isInconsistent) then
      -- `lynth_grind` state is inconsistent
      return ()
...
private def traceActiveCnstrs : GoalM Unit := do
  unless (← isTracingEnabledFor `lynth_grind.debug.lia.search.cnstrs) do return ()
  let s ← get'
...
def searchAssignment : GoalM Unit := do
  ...
  if !(← getConfig).qlia && !precise then
    ...
    if (← isInconsistent) then return ()
  -- If the search was interrupted (`lynth_liaSteps` threshold), the assignment is partial.
  unless (← hasAssignment) do return ()
  -- TODO: constructing a model is only worth if `lynth_grind` will **not** continue searching.
  assignElimVars
...
def check : GoalM Bool := do profileitM Exception "grind cutsat" (← getOptions) do
  if (← hasAssignment) then
    ...
    -- A previous search reached the `lynth_liaSteps` threshold. The solver is disabled to
    -- ensure `lynth_grind` does not keep restarting the (bounded) search at every iteration.
    return false
  else
    searchAssignment
    return true
