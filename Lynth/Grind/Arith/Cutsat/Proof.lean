...
import Lynth.Grind.Arith.Cutsat.CommRing
import Lynth.Grind.Arith.Cutsat.Util
import Lynth.Grind.Arith.Cutsat.Nat
...
private def getVarOf (e : Expr) : ProofM Var := do
  let some x := (← getVarMap).find? { expr := e } | throwError "`lynth_grind` internal error, missing cutsat variable{indentExpr e}"
  return x
...
private partial def mkMulEqProof (x : Var) (a? : Option Expr) (cs : Array (Expr × Int × EqCnstr)) (c' : EqCnstr) : ProofM Expr := do
  ...
  | .none =>
    throwError "`lynth_grind` internal error, cutsat failed to construct proof for multiplication equality"
where
...
private partial def EqCnstr.toExprProofImpl (c' : EqCnstr) : ProofM Expr := caching c' do
  ...
  | .commRingNorm c e p =>
    let h := mkApp4 (mkConst ``Lean.Grind.CommRing.norm_int) (← getRingContext) (← mkRingExprDecl e) (← mkRingPolyDecl p) eagerReflBoolTrue
  ...
  | .defnCommRing e e' p re rp p' =>
    let h := mkApp4 (mkConst ``Lean.Grind.CommRing.norm_int) (← getRingContext) (← mkRingExprDecl re) (← mkRingPolyDecl rp) eagerReflBoolTrue
    let x ← getVarOf e
...
private partial def DvdCnstr.toExprProof (c' : DvdCnstr) : ProofM Expr := caching c' do
  ...
  | .cooper₂ s =>
    ...
    let p₁ := c₁.p
    let p₂ := c₂.p
    let some c₃ := c₃? | throwError "`lynth_grind` internal error, unexpected `cooper₂` proof"
...
private partial def CooperSplit.toExprProof (s : CooperSplit) : ProofM Expr := caching s do
  ...
  | .last hs _ =>
    ...
    unless hs.size + 1 == n do
      throwError "`lynth_grind` internal error, unexpected number of cases at `CopperSplit` hs.size: {hs.size}, n: {n}, left: {left}\nc₁: {← c₁.pp}\nc₂: {← c₂.pp}\nc₃: {← if let some c₃ := c₃? then c₃.pp else pure ""}"
