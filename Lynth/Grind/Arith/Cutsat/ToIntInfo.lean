...
def SymbolicIntInterval.wrap (i : SymbolicIntInterval) (x : Expr) : MetaM Expr := do
  ...
  | .ci _ => throwError "`lynth_grind` internal error, `.ci` interval support has not been implemented yet"
  | .io _ => throwError "`lynth_grind` internal error, `.io` interval support has not been implemented yet"
  | .ii => return x
