...
import Lynth.Grind.Arith.IsRelevant
import Lynth.Grind.Util
import Lynth.Grind.Beta
import Lynth.Grind.MatchCond
import Lynth.Grind.Simp
import Lynth.Grind.MarkNestedSubsingletons
import Lynth.Grind.PropagateInj
...
private def checkAndAddSplitCandidate (e : Expr) : GoalM Unit := do
  ...
  | .forallE _ d _ _ =>
    ...
    if (← getConfig).splitImp then
      ...
      -- to make `lynth_grind` "stronger" by disabling a feature.
      addSplitCandidate (.imp e (h ▸ rfl) currSplitSource)
  | _ => pure ()
...
private def pushCastHEqs (e : Expr) : GoalM Unit := do
  ...
  | f@Eq.rec α a motive v b h => pushHEq e v (mkApp6 (mkConst ``Lean.Grind.eqRec_heq f.constLevels!) α a motive v b h)
  | f@Eq.ndrec α a motive v b h => pushHEq e v (mkApp6 (mkConst ``Lean.Grind.eqNDRec_heq f.constLevels!) α a motive v b h)
  | f@Eq.recOn α a motive b h v => pushHEq e v (mkApp6 (mkConst ``Lean.Grind.eqRecOn_heq f.constLevels!) α a motive b h v)
  | _ => return ()
...
where
  ...
  We added this extra case to make sure the user is not surprised by `lynth_grind` failing at
  ```
  example (x y : Nat)
      : 0 < match x, y with
            | 0, 0   => 1
  ...
  internalizeSimpleMatchCondImp : GoalM Unit := do
    let_expr Lean.Grind.MatchCond e := matchCond | return ()
...
This `isDefEq` feature was negatively affecting `lynth_grind` until we added an
...
by `simp` (i.e., in the `lynth_grind` preprocessor), and `lynth_grind` would never see
these facts.
-/
...
This feature enables `lynth_grind` to solve examples such as:
```lean
  ...
  go : GoalM Unit := do
    ...
    | .sort .. =>
      ...
      **Note**: It may seem wasteful to create ENodes for sorts, but it is useful for the E-matching module.
      ...
      unless they are `lynth_grind` gadgets.
      -/
      mkENode' e generation
    | .fvar .. =>
      mkENode' e generation
    ...
    | .mvar .. =>
      mkENode' e generation
    | .mdata .. =>
      reportIssue! "unexpected metadata found during internalization{indentExpr e}\n`lynth_grind` uses a pre-processing step that eliminates metadata"
      mkENode' e generation
    | .proj .. =>
      reportIssue! "unexpected kernel projection term during internalization{indentExpr e}\n`lynth_grind` uses a pre-processing step that folds them as projection applications, the pre-processor failed to fold this term"
      mkENode' e generation
    | .app .. =>
      if (← isNonParametricLitValue e) then
      ...
      else if e.isAppOfArity ``Lean.Grind.MatchCond 1 then
        ...
        if args.size == 2 && f.isConstOf ``Lean.Grind.nestedProof then
        ...
        else if args.size == 2 && f.isConstOf ``Lean.Grind.nestedDecidable then
