...
example [DecidableEq α] (s₁ s₂ : Set α) (a₁ a₂ : α) :
  ...
  fail_if_success lynth_grind -- should fail
  open Set in lynth_grind -- scoped pattern was activated, it should work now
...
example : xs = [a, b] → foo x xs = x ++ [a, b] ++ x := by
  fail_if_success lynth_grind -- should fail, `local grind_pattern` is not active anymore
