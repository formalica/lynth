...
-/
macro_rules | `(tactic| decreasing_trivial) => `(tactic| lynth_grind -mbtc)
...
def siftDown (a : Array Int) (root : Nat) (e : Nat) (h : e ≤ a.size := by lynth_grind) : Array Int :=
...
@[grind] theorem siftDown_size : (siftDown a root e h).size = a.size := by
   fun_induction siftDown <;> lynth_grind
...
def heapsort (a : Array Int) : Array Int :=
  ...
  go a a.size
where
  go (a : Array Int) (e : Nat) (h : e ≤ a.size := by lynth_grind) : Array Int :=
    -- Another annoying `_ :`
    if _ : e > 1 then
      let e := e - 1
      let a := a.swap e 0
