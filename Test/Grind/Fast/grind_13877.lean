...
theorem reflTransGen_iff_eq (h : ∀ b, ¬r a b) : ReflTransGen r a b ↔ b = a where
  mp h := by induction h with lynth_grind
  mpr := by lynth_grind
...
/--
info: Try these:
  [apply] lynth_grind only [= reflTransGen_iff_eq, EqvGen.rel, #9242, #14e8, #d806, #f38a, #e48b, #5525, #3442, #ae98,
    EqvGen]
  [apply] lynth_grind only [= reflTransGen_iff_eq, EqvGen.rel, EqvGen]
  [apply] lynth_grind [EqvGen] =>
