...
class Category (obj : Type u) : Type max u (v + 1) where
  ...
  id_comp : ∀ {X Y : obj} (f : Hom X Y), comp (id X) f = f := by lynth_grind
  /-- Identity morphisms are right identities for composition. -/
  comp_id : ∀ {X Y : obj} (f : Hom X Y), comp f (id Y) = f := by lynth_grind
  /-- Composition in a category is associative. -/
  assoc : ∀ {W X Y Z : obj} (f : Hom W X) (g : Hom X Y) (h : Hom Y Z), comp (comp f g) h = comp f (comp g h) := by lynth_grind
...
structure Functor (C : Type u₁) [Category.{v₁} C] (D : Type u₂) [Category.{v₂} D] : Type max v₁ v₂ u₁ u₂ where
  ...
  map_id : ∀ X : C, map (𝟙 X) = 𝟙 (obj X) := by lynth_grind
  /-- A functor preserves composition. -/
  map_comp : ∀ {X Y Z : C} (f : X ⟶ Y) (g : Y ⟶ Z), map (f ≫ g) = (map f) ≫ (map g) := by lynth_grind
...
structure NatTrans [Category.{v₁, u₁} C] [Category.{v₂, u₂} D] (F G : Functor C D) : Type max u₁ v₂ where
  ...
  naturality : ∀ ⦃X Y : C⦄ (f : X ⟶ Y), F.map f ≫ app Y = app X ≫ G.map f := by lynth_grind
...
structure Iso {C : Type u} [Category.{v} C] (X Y : C) where
  hom : X ⟶ Y
  inv : Y ⟶ X
  hom_inv_id : hom ≫ inv = 𝟙 X := by lynth_grind
  inv_hom_id : inv ≫ hom = 𝟙 Y := by lynth_grind
...
theorem ext ⦃α β : X ≅ Y⦄ (w : α.hom = β.hom) : α = β :=
  suffices α.inv = β.inv by lynth_grind [Iso]
  calc
    α.inv = α.inv ≫ β.hom ≫ β.inv := by lynth_grind
    _     = β.inv                 := by lynth_grind
...
structure Equiv (α : Sort _) (β : Sort _) where
  ...
  protected left_inv : LeftInverse invFun toFun := by lynth_grind
  protected right_inv : RightInverse invFun toFun := by lynth_grind
...
theorem map_hom_inv_id (F : C ⥤ D) :
    F.map e.hom ≫ F.map e.inv = 𝟙 _ := by lynth_grind

@[grind =]
theorem map_inv_hom_id (F : C ⥤ D) :
    F.map e.inv ≫ F.map e.hom = 𝟙 _ := by lynth_grind
...
theorem naturality_app {F G : Functor C (Functor D E)} (T : F ⟶ G) (Z : D) {X Y : C} (f : X ⟶ Y) :
  ...
  lynth_grind -- this is done manually in Mathlib!
  -- rw [← comp_app]
  -- rw [T.naturality f]
  -- rw [comp_app]
...
theorem congr_app {α β : F ⟶ G} (h : α = β) (X : C) : α.app X = β.app X := by lynth_grind
...
instance hom_app_isIso (α : F ≅ G) (X : C) : IsIso (α.hom.app X) :=
  ⟨⟨α.inv.app X, ⟨by lynth_grind, by lynth_grind⟩⟩⟩

instance inv_app_isIso (α : F ≅ G) (X : C) : IsIso (α.inv.app X) :=
  ⟨⟨α.hom.app X, ⟨by lynth_grind, by lynth_grind⟩⟩⟩
...
theorem naturality_1' (α : F ⟶ G) (f : X ⟶ Y) {_ : IsIso (α.app X)} :
    inv (α.app X) ≫ F.map f ≫ α.app Y = G.map f := by lynth_grind
...
instance isIso_app_of_isIso (α : F ⟶ G) [IsIso α] (X) : IsIso (α.app X) :=
  ⟨⟨(inv α).app X, ⟨by lynth_grind, by lynth_grind⟩⟩⟩
...
def ofComponents (app : ∀ X : C, F.obj X ≅ G.obj X)
    (naturality : ∀ {X Y : C} (f : X ⟶ Y),
      F.map f ≫ (app Y).hom = (app X).hom ≫ G.map f := by lynth_grind) :
    F ≅ G where
...
theorem ofComponents.app (app : ∀ X : C, F.obj X ≅ G.obj X) (naturality) (X) :
    (ofComponents app naturality).app X = app X := by lynth_grind
...
theorem isIso_map_iff {F₁ F₂ : C ⥤ D} (e : F₁ ≅ F₂) {X Y : C} (f : X ⟶ Y) :
  ...
  · lynth_grind (ematch := 6)
  · lynth_grind (ematch := 7)
