...
theorem exists_prime_factor :
  ...
  · lynth_grind [Nat.dvd_refl]
  -- ... or it has a non-trivial divisor with a prime factor
  · obtain ⟨k, _⟩ : ∃ k, 1 < k ∧ k < n ∧ k ∣ n := by
      simp_all [IsPrime]
    obtain ⟨p, _, _⟩ := exists_prime_factor k (by lynth_grind)
    lynth_grind [Nat.dvd_trans]
...
theorem factorial_pos : ∀ n, 0 < n ! := by
  intro n; induction n <;>
    lynth_grind [factorial, Nat.mul_pos_iff_of_pos_left]

/-- ... and divided by its constituent factors -/
theorem dvd_factorial : ∀ n, ∀ k ≤ n, 0 < k → k ∣ n ! := by
  intro n; induction n <;>
    lynth_grind [Nat.dvd_mul_right, Nat.dvd_mul_left_of_dvd, factorial]
...
theorem InfinitudeOfPrimes : ∀ n, ∃ p > n, IsPrime p := by
  intro n
  have : 1 < n ! + 1 := by lynth_grind [factorial_pos]
  obtain ⟨p, hp, _⟩ := exists_prime_factor (n ! + 1) this
  suffices ¬p ≤ n by lynth_grind
  intro (_ : p ≤ n)
  have : 1 < p := hp.1
  have : p ∣ n ! := dvd_factorial n p ‹p ≤ n› (by lynth_grind)
  have := Nat.dvd_sub ‹p ∣ n ! + 1› ‹p ∣ n !›
  lynth_grind [Nat.add_sub_cancel_left, Nat.dvd_one]
