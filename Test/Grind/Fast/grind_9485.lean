...
example (f g h : G' G) :
    f + g + h = f + (g + h) := by
  lynth_grind -- should work without extra options

example (f g h : G' G) :
    f + g + h = f + (g + h) := by
  lynth_grind [Lean.Grind.AddCommMonoid.add_assoc] -- should work too

example (f g h : G' G) :
    f + g + h = f + (g + h) := by
  lynth_grind -linarith [Lean.Grind.AddCommMonoid.add_assoc] -- should work too
