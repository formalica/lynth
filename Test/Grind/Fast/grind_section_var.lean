...
def Id.instLawfulMonad' : LawfulMonad Id := by
  apply LawfulMonad.mk' <;> (simp only [Functor.map, id, Functor.mapConst, Seq.seq, SeqRight.seqRight, SeqLeft.seqLeft, bind, pure]; lynth_grind)
...
-/
def Id.instLawfulMonad'' : LawfulMonad Id := by
  apply LawfulMonad.mk' <;> (simp only [Functor.map, id, Functor.mapConst, Seq.seq, SeqRight.seqRight, SeqLeft.seqLeft, bind, pure]; lynth_grind +revert)
