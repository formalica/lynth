...
example [Max α] [Std.IdempotentOp (α := α) max] {p : α → Bool} {o : Option α} :
    max (o.filter p) o = o := by lynth_grind

example [Max α] [Std.IdempotentOp (α := α) max] {o : Option α} {p : (a : α) → o = some a → Bool} :
    max (o.pfilter p) o = o := by lynth_grind

example [Max α] {o o' : Option α} : (max o o').isSome = (o.isSome || o'.isSome) := by lynth_grind

example [Max α] {o o' : Option (Option α)} : (max o o').join = max o.join o'.join := by lynth_grind
