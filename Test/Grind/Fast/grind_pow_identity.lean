...
example (x y : Fin 2) : (x + y)^2 = x + y := by lynth_grind
example (x : Fin 2) : x^2 = x := by lynth_grind
example (x y : Fin 2) : x^2 + y^2 = x + y := by lynth_grind
example (x y : Fin 2) : x * y + x * y = 0 := by lynth_grind
example (x y : Fin 2) : (x + y)^2 = x^2 + y^2 := by lynth_grind

-- Higher powers reduced by PowIdentity
example (x : Fin 2) : x^4 = x := by lynth_grind
example (x : Fin 2) : x^8 = x := by lynth_grind
