...
where
  go (t : Tree β) (acc : List (Nat × β))
    ...
    induction t generalizing acc <;> lynth_grind [toListTR.go, toList]

@[local csimp] theorem Tree.toList_eq_toListTR_csimp
                 : @Tree.toList = @Tree.toListTR := by
  lynth_grind [toList_eq_toListTR]
...
theorem Tree.forall_insert_of_forall
  ...
  induction h₁ <;> lynth_grind [ForallTree.node, ForallTree.leaf]

theorem Tree.bst_insert_of_bst
        {t : Tree β} (h : BST t) (key : Nat) (value : β)
        : BST (t.insert key value) := by
  induction h <;> lynth_grind [BST.node, BST.leaf, ForallTree.leaf, forall_insert_of_forall]
