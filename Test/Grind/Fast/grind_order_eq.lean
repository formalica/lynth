...
example [CommRing α] [LE α] [LT α] [LawfulOrderLT α] [IsPartialOrder α] [OrderedRing α]
  ...
  lynth_grind -mbtc -lia -linarith (splits := 0)
...
example
  ...
  lynth_grind -lia -linarith -ring (splits := 0)

example
    : a = b → a ≤ b + 1 := by
  lynth_grind -lia -linarith -ring (splits := 0) only
...
example (j k : Nat) (h : j = k) : (j + 1 : Rat) = (k + 1 : Rat) := by lynth_grind
