...
def idxToCount' [BEq α] (xs : List α) (i : Nat) (hi : i < xs.length) : Nat := go xs i hi 0 where
  ...
  | _ :: _, 0, _, acc => acc
  | x :: xs, i + 1, hi, acc =>
    bif x == xs[i]'(by lynth_grind) then go xs i (by lynth_grind) (acc + 1) else go xs i (by lynth_grind) acc
...
def idxToCount'' [BEq α] (xs : List α) (i : Nat) (hi : i < xs.length) : Nat := go xs i hi 0 where
  ...
  | _ :: _, 0, _, acc => acc
  | x :: xs, i + 1, hi, acc =>
    bif x == xs[i]'(by lynth_grind +revert) then go xs i (by lynth_grind +revert) (acc + 1) else go xs i (by lynth_grind +revert) acc
