...
theorem getElem_singleton {a : α} {i : Nat} (h : i < 1) : [a][i] = a := by lynth_grind
...
theorem exists_mem_cons {p : α → Prop} {a : α} {l : List α} :
    (∃ x, ∃ _ : x ∈ a :: l, p x) ↔ p a ∨ ∃ x, ∃ _ : x ∈ l, p x := by lynth_grind

theorem getElem?_of_mem {a} {l : List α} (h : a ∈ l) : ∃ i : Nat, l[i]? = some a := by
  let ⟨n, _, e⟩ := getElem_of_mem h
  exact ⟨n, by lynth_grind⟩

/-! ### any / all -/

theorem any_eq {l : List α} : l.any p = decide (∃ x, x ∈ l ∧ p x) := by lynth_grind

theorem all_eq {l : List α} : l.all p = decide (∀ x, x ∈ l → p x) := by lynth_grind
...
theorem getLast!_nil [Inhabited α] : ([] : List α).getLast! = default := by lynth_grind

theorem getLast!_eq_getLast?_getD [Inhabited α] {l : List α} : getLast! l = (getLast? l).getD default := by
  cases l with lynth_grind
...
theorem map_inj_right {f : α → β} (w : ∀ x y, f x = f y → x = y) : map f l = map f l' ↔ l = l' := by
  induction l generalizing l' with lynth_grind [cases List]
...
theorem append_of_mem {a : α} {l : List α} : a ∈ l → ∃ s t : List α, l = s ++ a :: t
  | .head l => ⟨[], l, rfl⟩
  | .tail b h => let ⟨s, t, h'⟩ := append_of_mem h; ⟨b::s, t, by lynth_grind⟩

theorem mem_iff_append {a : α} {l : List α} : a ∈ l ↔ ∃ s t : List α, l = s ++ a :: t :=
  ⟨append_of_mem, by lynth_grind⟩
...
theorem getElem_append_right' (l₁ : List α) {l₂ : List α} {i : Nat} (hi : i < l₂.length) :
    l₂[i] = (l₁ ++ l₂)[i + l₁.length]'(by lynth_grind) := by
  lynth_grind

theorem getElem_of_append {l : List α} (eq : l = l₁ ++ a :: l₂) (h : l₁.length = i) :
    l[i]'(by lynth_grind) = a := by
  lynth_grind
...
theorem append_eq_append_iff {ws xs ys zs : List α} :
  ...
  | nil => lynth_grind
  | cons a as ih => cases ys with lynth_grind

theorem concat_append {a : α} {l₁ l₂ : List α} : concat l₁ a ++ l₂ = l₁ ++ a :: l₂ := by lynth_grind
...
theorem flatMap_eq_foldl {f : α → List β} {l : List α} :
  ...
  suffices ∀ l', l' ++ l.flatMap f = l.foldl (fun acc a => acc ++ f a) l' by lynth_grind
  intro l'
  induction l generalizing l' with lynth_grind
...
theorem getElem?_reverse' : ∀ {l : List α} {i j}, i + j + 1 = length l →
    l.reverse[i]? = l[j]?
  | [], _, _, _ => by lynth_grind
  | a::l, i, 0, h => by lynth_grind
  | a::l, i, j+1, h => by lynth_grind

theorem getElem?_reverse {l : List α} {i} (h : i < length l) :
    l.reverse[i]? = l[l.length - 1 - i]? := by lynth_grind

theorem getElem_reverse {l : List α} {i} (h : i < l.reverse.length) :
    l.reverse[i] = l[l.length - 1 - i]'(by lynth_grind) := by
  lynth_grind
...
theorem foldlM_pure [Monad m] [LawfulMonad m] {f : β → α → β} {b : β} {l : List α} :
  ...
  induction l generalizing b with lynth_grind
...
theorem foldl_eq_foldr_reverse {l : List α} {f : β → α → β} {b : β} :
    l.foldl f b = l.reverse.foldr (fun x y => f y x) b := by lynth_grind

theorem foldr_eq_foldl_reverse {l : List α} {f : α → β → β} {b : β} :
    l.foldr f b = l.reverse.foldl (fun x y => f y x) b := by lynth_grind

theorem foldl_assoc {op : α → α → α} [ha : Std.Associative op]
    {l : List α} {a₁ a₂} : l.foldl op (op a₁ a₂) = op a₁ (l.foldl op a₂) := by
  induction l generalizing a₁ a₂ with lynth_grind
...
theorem elem_cons_self [BEq α] [LawfulBEq α] {a : α} : (a::as).elem a = true := by lynth_grind
...
theorem getElem?_replace [LawfulBEq α] {l : List α} {i : Nat} :
  ...
  induction l generalizing i with cases i with lynth_grind

theorem getElem_replace [LawfulBEq α] {l : List α} {i : Nat} (h : i < l.length) :
    (l.replace a b)[i]'(by lynth_grind) = if l[i] == a then if a ∈ l.take i then a else b else l[i] := by
  lynth_grind
...
theorem replace_take {l : List α} {i : Nat} :
  ...
  | nil => lynth_grind
  | cons x xs ih => cases i with lynth_grind

theorem replace_replicate_ne [LawfulBEq α] {a b c : α} (h : !b == a) :
    (replicate n a).replace b c = replicate n a := by
  lynth_grind [replace_of_not_mem]
