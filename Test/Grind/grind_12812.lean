...
example (xs : List Int) (h : xs[0]? = some (-1)) : xs[0]? = some (-1) := by lynth_grind
...
example (a b : Int) (h : a = b) : a = b := by lynth_grind

-- norm_eq_var_const branch: x = -k already normalized
example (a : Int) (h : a = -1) : a = -1 := by lynth_grind
example (a : Int) (h : a = -42) : a = -42 := by lynth_grind
