...
macro_rules | `(tactic| get_elem_tactic_extensible) => `(tactic| lynth_grind)
...
structure IndexMap (α : Type u) (β : Type v) [BEq α] [Hashable α] where
  ...
  private size_keys' : keys.size = values.size := by lynth_grind
  private WF1 : ∀ (i : Nat) (h1 : i < keys.size) (h2 : keys[i] ∈ indices), indices[keys[i]] = i := by lynth_grind
  private WF2 : ∀ (a : α) (h1 : a ∈ indices) (h2 : indices[a] < keys.size), keys[indices[a]] = a := by lynth_grind
  private WF3 : ∀ (a : α) (h1 : a ∈ indices), indices[a] < keys.size := by lynth_grind
  private WF4 : ∀ (i : Nat) (h1 : i < keys.size), keys[i] ∈ indices := by lynth_grind
...
private theorem getElem_indices_lt {h : a ∈ m} : m.indices[a] < m.size := by
  have : m.indices[a]? = some m.indices[a] := by lynth_grind
  lynth_grind
...
example (m : IndexMap α β) (a : α) (h : a ∈ m) :
    m.getIdx (m.findIdx a) = m[a] := by
  lynth_grind => instantiate only [getIdx, findIdx, = getElem_def]
...
/--
info: Try these:
  [apply] lynth_grind only [= mem_indices_of_mem, insert, = getElem_def, = getElem?_neg, = getElem?_pos,
  ...
  [apply] lynth_grind only [= mem_indices_of_mem, insert, = getElem_def, = getElem?_neg, = getElem?_pos,
    = Array.getElem_set, size, = HashMap.mem_insert, = HashMap.getElem_insert, = Array.getElem_push,
    usr getElem_indices_lt, usr WF2, ← WF3, WF']
  [apply] lynth_grind =>
    instantiate only [= mem_indices_of_mem, insert, = getElem_def]
    instantiate only [= getElem?_neg, = getElem?_pos]
    cases #dbaf
...
example (m : IndexMap α β) (a a' : α) (b : β) (h : a' ∈ m.insert a b) :
    (m.insert a b)[a'] = if h' : a' == a then b else m[a'] := by
  lynth_grind only [= mem_indices_of_mem, insert, = getElem_def, = getElem?_neg, = getElem?_pos,
    = Array.getElem_set, size, = HashMap.mem_insert, = HashMap.getElem_insert, = Array.getElem_push,
    usr getElem_indices_lt, usr WF2, ← WF3, WF', #dbaf, #f969]
