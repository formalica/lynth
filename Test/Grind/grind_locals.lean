...
-/
#guard_msgs in
example (n : Nat) : foo n = n + 1 := by lynth_grind

-- Test that grind +locals can use the equation `foo n = n + 1`
example (n : Nat) : foo n = n + 1 := by lynth_grind +locals
...
/--
info: Try these:
  [apply] lynth_grind only [foo]
  [apply] lynth_grind => instantiate only [foo]
-/
#guard_msgs in
example (n : Nat) : foo n = n + 1 := by lynth_grind? +locals
...
example : isZero 0 = true := by lynth_grind +locals
example (n : Nat) : isZero (n + 1) = false := by lynth_grind +locals
