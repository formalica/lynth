...
`lake env lean -Dlinter.all=false -DElab.inServer=true Test/Grind/<file>.lean`
(the upstream harness flags; warnings/info output is fine, it is not
diffed — upstream `.out.expected` files are not copied). Files that fail
are removed from this directory (see MANIFEST for the take list, not a fail list).
