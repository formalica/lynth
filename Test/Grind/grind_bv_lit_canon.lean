...
example (f g : Nat → BitVec 2) (h : ∀ n, f n = g n ||| 1#2) : f 0 = g 0 ||| 1#2 := by lynth_grind

example (f g : Nat → BitVec 2) (h : ∀ n, f n = g n &&& 1#2) : f 0 = g 0 &&& 1#2 := by lynth_grind

example (f g : Nat → BitVec 2) (h : ∀ n, f n = g n ||| 0#2) : f 0 = g 0 ||| 0#2 := by lynth_grind

example (f g : Nat → BitVec 4) (h : ∀ n, f n = g n ||| 1#4) : f 0 = g 0 ||| 1#4 := by lynth_grind

-- mixed spellings across hypothesis and goal
example (f g : Nat → BitVec 2) (h : ∀ n, f n = g n ||| 1#2) :
    f 0 = g 0 ||| (1 : BitVec 2) := by lynth_grind

example (f g : Nat → BitVec 2) (h : ∀ n, f n = g n ||| (1 : BitVec 2)) :
    f 0 = g 0 ||| 1#2 := by lynth_grind
